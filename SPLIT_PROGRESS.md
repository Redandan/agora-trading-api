...
- `scripts/prepare_profit_live_blocker_source_refresh.ps1` orchestrates the
  ...
  23-step plan without invoking SSH or child refreshes; normal execution runs
  ...
  refreshed trailing dry-run preflight packet, the audit-backed
  `profit-operator-next-action-board-latest.log`, and
  `profit-operator-authorization-request-latest.log`, so the exact next
  operator review authorization and its trailing preflight stay in the same
  refreshed evidence set. It
