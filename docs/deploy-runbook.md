...
- If OCO/grid reconciliation finds closed Grid residual inventory, run `.\scripts\prepare_closed_grid_residual_disposition_packet_ssh.ps1 -RequireReady` before discussing cleanup. It emits `CLOSED_GRID_RESIDUAL_DISPOSITION_PACKET`, `closed_grid_residual_disposition_packet`, and `closed_grid_residual_disposition_status=READY_FOR_CLOSED_GRID_RESIDUAL_OPERATOR_DECISION_NOT_MUTATION` only when production read-only DB/MCP/log evidence is complete. It keeps `closed_grid_residual_cleanup_allowed=false`, `exchange_sell_allowed=false`, `db_write_allowed=false`, `order_allowed=false`, `grid_mutation_allowed=false`, `telegram_send_allowed=false`, and `deploy_or_env_change_allowed=false`. Treat `exactAuthorizationTextForNextPlan` as permission to draft a separate execution plan only; no DB cleanup, exchange sell, order, OCO/grid/fund/Earn/Telegram mutation, deploy, or production env change is authorized by this packet.
...
- Invokes only `prepare_grid_capital_override_review_packet_ssh.ps1`.
- Emits `GRID_OPEN_AUTHORIZATION_BUNDLE_PACKET`,
...
- For a more aggressive but still read-only operator review path, run:
  ...
  selected aggressive path, next order-capable candidate, current grid10
  blocker evidence when a fresh grid blocker board log is supplied, current
  blockers, candidate env diff, risk acceptance conditions, verification
  commands, and rollback requirements that must be reviewed for a separate
  env/deploy decision. `order_capable_execution_now_allowed=false` means this
