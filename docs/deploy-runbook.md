...
- Hourly orchestrator, market indicator collection, and meta-control attribution default off in code and the tracked template; enable `META_CONTROL_HOURLY_ORCHESTRATOR_ENABLED=true`, `META_CONTROL_INDICATOR_HISTORY_ENABLED=true`, and `META_CONTROL_ATTRIBUTION_ENABLED=true` only after this service should own external indicator API collection, `market_indicator_history` writes, K-line gap backfills, attribution writes, and wide-TP Telegram scans.
...
- ML protection scans, edge staleness alerts, and auto-retrain default off in code and the tracked template; enable `META_CONTROL_ML_PROTECTION_ENABLED=true`, `META_CONTROL_ML_EDGE_WATCHER_ENABLED=true`, or `META_CONTROL_ML_AUTORETRAIN_ENABLED=true` only after the deployed trading service should own those ML automation writes and Telegram alerts.
...
- Market flip analysis and auto-escalation default off in code and the tracked template; set `META_CONTROL_MARKET_FLIP_ANALYSIS_ENABLED=true` or `META_CONTROL_MARKET_FLIP_AUTO_ESCALATE_ENABLED=true` only after this service should own pending flip AI analysis, event status updates, audit writes, and Telegram escalation.
