...
accelerator, broad order-capable scope packet, and legacy custom-Grid operator
scripts were retired on 2026-07-23. Historical references later in this
runbook are acceptance records, not current commands. Any future live-capable
change requires a new narrowly scoped review based on current evidence.
...
- When production intentionally enables public market WebSocket auto-subscribe,
  keep `MARKET_WS_AUTO_SUBSCRIBE_PROVIDERS=okx` unless Binance WS access from
  the host has been explicitly verified; Binance Spot can return HTTP 451 from
  restricted regions. The K-line gap detector uses the same DB-derived
  subscription resolver as WebSocket auto-subscribe, so enabling this lane can
  write `md_kline`, publish `KlineClosedEvent`, run live signal evaluation, and
...
- `runScoreBuyTradingViewProfitOptimizationReport` uses
  ...
  stress gates. The final evaluated candidate keeps fixed 10 USDT
  `LIVE_ONE_ORDER_PER_BAR` buys and only reduces 25% when inventory net return
  first reaches `-12%`; a later BUY rearms the reduction. Exposure is capped at
  250 USDT concurrent cost basis and released basis may be redeployed. Reports
  show cumulative gross buys, maximum/remaining cost basis, realized and
  unrealized PnL, all fees, emergency reductions, inventory drawdown, and
  capital loss on cumulative gross buys. The drawdown gate uses the larger of
  the last two risk measures. This candidate is rejected and the report emits
  `NONE_NO_PROVEN_EDGE_STOP_TUNING`; neither tool authorizes live promotion.
...
- Trading signal source policy defaults to TradingView primary. Keep
  `TRADING_SIGNAL_SOURCE_PRIMARY=TRADINGVIEW` and
  `TRADING_LEGACY_LIVE_EVALUATOR_ENABLED=false` so K-line close events do not
  run the legacy `LiveSignalEvaluator`; use `LEGACY` plus explicit evaluator
  enablement only for rollback or a deliberate shadow investigation. For a
  no-paid-TradingView replacement,
  `TRADING_SIGNAL_SOURCE_PRIMARY=LOCAL_TRADINGVIEW` plus
  `TRADINGVIEW_LOCAL_ENABLED=true` evaluates local ScoreBuy TradingView parity
  ...
  `bt_decision_audit`, and `bt_runtime_decision_evidence` with
...
- A legacy strategy can be run beside `LOCAL_TRADINGVIEW` only through the
  default-off secondary allowlist. Use
  `TRADING_LEGACY_SECONDARY_EVALUATOR_ENABLED=true`,
  `TRADING_LEGACY_SECONDARY_ALLOWED_STRATEGY_IDS=<id>`, and
  `TRADING_LEGACY_SECONDARY_MAX_NOTIONAL_USDT=10.0` for a reviewed micro-live
  lane such as strategy 508. Do not switch `TRADING_SIGNAL_SOURCE_PRIMARY` back
  to `LEGACY` for this case, because that disables the local TradingView primary
  lane and evaluates all enabled legacy strategies instead of the named
  allowlist.
...
OKX Native Spot Grid verification is read-only unless a separately authorized
create/stop request explicitly sets the production mutation gates. The removed
custom Grid tools and packet scripts are not part of the deploy runbook.

After a relevant deploy, verify:

1. `getOkxNativeSpotGridStatus` for provider inventory and lifecycle.
2. `getOkxNativeSpotGridAcceptanceEvidence` for per-order complete fill
   coverage, signed fees, completed buy/sell pairs, terminal state, and base
   residual.
3. `getOkxNativeSpotGridFunctionalSafetyEvidence` for zero legacy executable
   activity, restart rediscovery, and duplicate/order-safety evidence.

The detailed acceptance contract is maintained in
`docs/okx-native-spot-grid-migration.md`. Historical `bt_grid` and
`bt_grid_level` rows remain read-only attribution evidence while the shared DB
is retained; do not drop or rewrite them from this service.
...
decision, preflight, activation review, post-opt-in readiness, observation
status, next-execution blocker, strategy485 risk reduction/escalation,
EntryDedup semantics, DataFreshness replay blocker/collector activation,
TP/SL/OCO feasibility, strategy574/TinyLive governance, governance relaxation,
and final live blocker audit lanes. It then writes the audit-backed
...
- The brief emits `exit_side_operator_action_proposals`, splitting the ready
  exit-side lane into trailing-stop rollout and strategy 485 risk-reduction
  proposal skeletons with ids `trailing-stop-rollout-review` and
  `strategy485-risk-reduction-review`. These proposals reference
  `docs/exit-side-operator-review-plan.md` and are not executable commands.
...
- For the fastest first check of whether the latest saved matrix can still be
  used, run:

  ```powershell
  .\scripts\prepare_profit_operator_quick_status.ps1
  ```
...
- The retired custom-grid aggressive-activation lane is no longer supported.
  Use current profit review tooling and keep Native Grid provider observation
  separate from strategy activation.
...
- Before treating a positive EntryDedup shadow lane as a future mutation
  candidate, run
...
- After the consistency, forward-return, and feasibility evidence has been
  ...
  `order_allowed=false`.
- For a fresh production rerun before operator review, run
  `.\scripts\prepare_entry_dedup_semantics_shadow_experiment_packet_ssh.ps1 -RequireReady`.
  This invokes the exposure consistency, semantics shadow review, and TP/SL
  feasibility smokes directly, then emits
  ...
  `entry_dedup_policy_change_allowed=false`. It is still review-only and does
...
- The wrapper runs the live-readiness audit, background automation smoke,
  runtime-evidence RCA, tiny-live loss RCA, signal-correctness smoke,
  LOCAL_TRADINGVIEW candidate smoke, and MCP parity smoke.
...
- Output includes deployment metadata status, `origin_delta_status`,
  `origin_runtime_delta_files`, `local_tradingview_current_candidate_status`,
  `local_tradingview_dry_run_receipt_armed`, `runtime_order_sent_evidence`,
  `runtime_order_sent_blocker_count`,
  `bundle_blockers`, `bundle_blocker_summary`,
...
| `KlineGapDetector` reports missing bars | Accepted only when every warning in the same active run has a later positive OKX backfill for that warning's symbol/interval and the count stays at or below `MAX_KLINE_GAP_WARN` (default `3`). Any unrecovered gap or threshold breach fails strict smoke; current 1h/4h quality evidence remains a separate tiny-live gate. |
| `MarketIndicatorHistoryCollector` parallel fetch timeout | Bounded provider latency warning while at or below `MAX_INDICATOR_FETCH_TIMEOUT_WARN` (default `5`). Exceeding the threshold fails strict smoke. Even below threshold, tiny-live review must separately prove current collection freshness; the warning alone never authorizes stale indicators. |
| `PositionAgingMonitor` aging-position warning | Bounded operational observation while at or below `MAX_AGING_POSITION_WARN` (default `10`). It does not block a code-only blue-green deploy by itself, but OCO/BTC-base management health and same-thesis exposure remain separate hard gates before any live start. |
| `OkxPrivateWsService` `Connection failure: Broken pipe` | Classified with the existing private-WS transient family, but every nonzero run requires a later `OkxPrivateWs Subscription confirmed` row after the latest warning and the count must remain within `MAX_OKX_PRIVATE_WS_TRANSIENT_WARN`. Missing recovery or a threshold breach fails strict smoke; REST OCO health remains separate fallback evidence during the gap. |
| `McpStreamableHttpController` exact `Unknown tool: getDbRuntimeStatus` | Bounded wrong-service probe after the Trading split, at or below `MAX_MCP_UNKNOWN_TOOL_WARN` (default `5`). Callers must route DB-runtime diagnostics to the owning service; other unknown tool names remain unclassified and fail strict smoke. |
...
For Native Grid verification, use the provider status, exact acceptance, and
functional-safety MCP tools documented earlier in this runbook. The removed
custom Grid post-env and post-open scripts must not be recreated.
