...
- Market-data MCP live sentiment dashboard reads default off in code and the tracked template; enable `TRADING_MARKET_DATA_MCP_LIVE_SENTIMENT_ENABLED=true` only when manual sentiment dashboards should read Fear&Greed, whale flow, OKX, Polymarket, and orderbook endpoints directly.
