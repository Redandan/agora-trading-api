...
| `ORDER_CAPABLE_FLAGS_REVIEW` | `.\scripts\audit_live_readiness_ssh.ps1`, then `.\scripts\prepare_live_order_capable_scope_review_packet.ps1` when any flag is true | Audit explicitly prints `order_capable_flags_true=[]`; missing order-capable evidence stays blocked. If any order/OCO/grid/fund/Earn/guardian live-action flag is true, the local scope packet must show per-flag coverage, current risk items, rollback env diff, and `order_allowed=false` before an operator can decide whether to accept or roll back that exact scope. | Stop live review; assemble the order-capable scope review packet and either request a separate reconcile/rollback decision or roll the scope back before any new proposal. |
