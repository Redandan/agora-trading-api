...
| `SIGNAL_POLICY_REVIEW_GAPS` | `.\scripts\smoke_signal_correctness_ssh.ps1` | No `REVIEW_POLICY_GAPS`, explicit 7d `governanceMode` is present and not `governanceMode=TOO_STRICT`, `governanceMode=TOO_LOOSE`, or `governanceMode=INSUFFICIENT_DATA`, and missed-opportunity `overallStatus=PASS`; missing or `N/A` governance/missed-opportunity evidence stays blocked. | Keep hard safety gates; review relaxation candidates in shadow/tiny-live caps only. |
...
`8b8437c8ad1bae6767393d625ab4454dd08686c5`) may legitimately report:
