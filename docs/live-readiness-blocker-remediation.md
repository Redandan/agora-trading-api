...
| `ORDER_CAPABLE_FLAGS_REVIEW` | `.\scripts\audit_live_readiness_ssh.ps1` | Audit explicitly prints `order_capable_flags_true`, `order_capable_flags_accepted`, and `order_capable_flags_unexpected=[]`; missing order-capable evidence stays blocked, including missing unexpected-flag evidence. Accepted true flags are limited to the exact active scope, such as LOCAL_TRADINGVIEW LIVE_MICRO effective OKX/OCO/execution flags or BTC_BASE_LIVE_MICRO effective OKX/execution flags plus OCO poller for existing OCO lifecycle cleanup. Any unexpected order/OCO/grid/fund/Earn/guardian live-action flag requires a new narrowly scoped review with per-flag coverage, current risk items, rollback env diff, and `order_allowed=false`. | Stop live review for unexpected flags and request a separate reconcile/rollback decision before any new proposal. |
