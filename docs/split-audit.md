...
- Trading signal source policy defaults to `TRADING_SIGNAL_SOURCE_PRIMARY=TRADINGVIEW` and `TRADING_LEGACY_LIVE_EVALUATOR_ENABLED=false`, so K-line close events do not run the legacy `LiveSignalEvaluator` unless an explicit rollback opts into `LEGACY` primary plus legacy evaluator enablement.
