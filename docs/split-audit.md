...
- Gemini advisor hint flip/staleness detector schedulers are bean-level explicit opt-in and disabled by default in code and in the tracked template, including `TRADING_GEMINI_ADVISOR_FLIP_DETECTOR_ENABLED=false` and `TRADING_GEMINI_ADVISOR_STALENESS_DETECTOR_ENABLED=false`, so hint-table scans and related Telegram alerts are explicit production opt-in behavior.
