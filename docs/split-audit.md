...
- The same code/template contract keeps attention weekly digest (`META_CONTROL_ATTENTION_WEEKLY_DIGEST_ENABLED=false`), scorecard digest (`META_CONTROL_SCORECARD_DIGEST_ENABLED=false`), autonomous digest, and ScoreBuy forming-day notification disabled by default until production intentionally opts in.
