...
- ML protection scans, edge staleness alerts, and auto-retrain are disabled by default in code and in the tracked template, so ML circuit writes, model registry writes, and related Telegram alerts are explicit production opt-in behavior. ML protection auto-kill is also disabled by default and requires `META_CONTROL_ML_PROTECTION_AUTO_KILL_SECONDARY_LOAD=true`.
...
- Grid runtime, auto-rebalance scheduling, and grid orphan recovery are disabled by default in code and in the tracked template; grid orphan recovery is bean-level explicit opt-in, so grid order placement, automatic range rebuilds, OKX fill lookups, recovery writes, and recovery Telegram alerts are explicit production opt-in behavior.
