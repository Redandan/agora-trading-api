...
- `smoke_local_health.ps1` clears host external keys for AgoraMarket, exchanges, Telegram, AI providers, and market-data providers; smoke logs must prove `AiTaskRouter` initialized with 0 providers, Jina embeddings are disabled, public market WebSocket auto-subscribe is disabled, OKX Earn top-up is disabled, Polymarket external monitoring is disabled, and trailing-stop OCO updates are disabled.
