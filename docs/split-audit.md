...
- `smoke_local_health.ps1` clears host external keys for AgoraMarket, exchanges, Telegram, AI providers, and market-data providers; smoke logs must prove `AiTaskRouter` initialized with 0 providers, Jina embeddings are disabled, public market WebSocket auto-subscribe is disabled, startup market-data backfills are disabled, OKX Earn top-up is disabled, Polymarket external monitoring is disabled, trailing-stop OCO updates are disabled, and short-squeeze alerting plus taker-buy collection are disabled.
