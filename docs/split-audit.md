...
- `smoke_local_health.ps1` clears host external keys for AgoraMarket, exchanges, Telegram, AI providers, and market-data providers, plus high-risk trading/deletion/AI automation toggles; smoke logs must prove `AiTaskRouter` initialized with 0 providers, Jina embeddings are disabled, public market WebSocket auto-subscribe is disabled, OKX liquidation WebSocket is disabled, startup market-data backfills are disabled, attribution/ML startup work is not scheduled, and short-squeeze alerting plus taker-buy collection are disabled. OKX Earn top-up, Polymarket monitoring, position-exit management, and trailing-stop OCO management are bean-level opt-in, forced off by local smoke, and smoke logs must not show Earn redemption/transfer, Polymarket snapshot/backfill/digest, OCO modification, or trailing-state side effects.
...
- Deploy/server verification require `AGORA_MARKET_BASE_URL` to point at stable AgoraMarketAPI nginx vhost dependency `https://agoramarketapi.purrtechllc.com` and fail on stale values.
...
- Env-template validation pins `AGORA_MARKET_INTERNAL_TIMEOUT_MS=3000` and local-only default CORS through `TRADING_CORS_ALLOWED_ORIGINS=http://localhost:*,http://127.0.0.1:*`, so split deploy prep cannot silently widen browser access or slow internal dependency failure.
- Deploy, server preflight, and server verification also require `AGORA_MARKET_INTERNAL_TIMEOUT_MS=3000`, so the runtime env cannot silently turn AgoraMarket exchange-rate dependency failure into a long deploy stall.
- Hourly orchestrator, market indicator collection, BTC price-move indicator writes, ETF pressure refresh/calculation fetches, and meta-control attribution are disabled by default in code and in the tracked template; the hourly orchestrator and BTC price-move indicator schedulers are bean-level explicit opt-in, so external indicator API collection, ETF Yahoo Finance reads, `market_indicator_history` writes, K-line gap backfills, attribution startup/hourly writes, and wide-TP Telegram scans are explicit production opt-in behavior.
- Composite indicator scheduled evaluation and market-indicator attention evaluation are bean-level explicit opt-in and disabled by default in code and in the tracked server env template, so CMI DB writes, MIH attention-rule evaluation, and related alerts are explicit production opt-in behavior.
- ML protection scans, edge staleness alerts, and auto-retrain are disabled by default in code and in the tracked template; ML protection scanning and edge staleness watching are bean-level explicit opt-in, so ML circuit writes, model registry writes, and related Telegram alerts are explicit production opt-in behavior. ML protection auto-kill is also disabled by default and requires `META_CONTROL_ML_PROTECTION_AUTO_KILL_SECONDARY_LOAD=true`.
- ML shadow inference logging is disabled by default in code and in the tracked template, including `META_CONTROL_ML_SHADOW_ENABLED=false`; production can opt in explicitly when live-signal HeatWave prediction lookups and `ml_inference_log` writes belong to this service.
- ML materialized training-table startup refresh is disabled by default in code and in the tracked template, so startup `bt_signal_training_v8_mat` writes are explicit production opt-in behavior; MCP-triggered manual refresh remains an explicit operator action.
...
- Market flip detection, analysis, and auto-escalation are disabled by default in code and in the tracked template; analysis and auto-escalation schedulers are bean-level explicit opt-in, so flip event writes, immediate flip notifications, pending flip AI analysis, event status updates, audit writes, and Telegram escalation are explicit production opt-in behavior.
- Wick-capture shadow observation is bean-level explicit opt-in, and shadow observation plus historical bootstrap are disabled by default in code and in the tracked template, including `WICK_CAPTURE_SHADOW_ENABLED=false` and `WICK_CAPTURE_SHADOW_BOOTSTRAP_ENABLED=false`, so shadow table writes, attention audit rows, outcome backfill writes, and Telegram context alerts are explicit production opt-in behavior.
- Shadow signal cleanup is bean-level explicit opt-in and disabled by default in code and in the tracked template, including `SHADOW_CLEANUP_ENABLED=false`, so automatic `bt_live_signal` timeout updates for stale shadow signals are explicit production opt-in behavior.
- Daily TG report orchestration is opt-in at the scheduler bean and disabled by default in the tracked template, including `TRADING_DAILY_TG_REPORT_ENABLED=false`, so daily Telegram reporting does not start automatically after repo split deploys.
- The same code/template contract keeps attention weekly digest (`META_CONTROL_ATTENTION_WEEKLY_DIGEST_ENABLED=false`), scorecard digest (`META_CONTROL_SCORECARD_DIGEST_ENABLED=false`), autonomous digest (`TRADING_AUTONOMOUS_DIGEST_ENABLED=false`, `TRADING_AUTONOMOUS_DIGEST_TELEGRAM_ENABLED=false`), and ScoreBuy forming-day notification disabled by default until production intentionally opts in. Scorecard, autonomous digest, and ScoreBuy forming-day notification schedulers are bean-level explicit opt-in; attention weekly digest keeps a method-level guard so MCP manual digest access remains available.
...
- Event-scan scheduled outbound notifications are bean-level explicit opt-in, disabled and dry-run by default in the tracked template, including `EVENT_SCAN_NOTIFICATION_ENABLED=false` and `EVENT_SCAN_NOTIFICATION_DRY_RUN=true`.
- Execution-event scheduled scanning is bean-level explicit opt-in and disabled by default in the tracked template, including `EXECUTION_EVENT_ENABLED=false`; its notification path stays dry-run with `EXECUTION_EVENT_NOTIFICATION_DRY_RUN=true` unless explicitly enabled.
- BTC price-move Telegram alerts are opt-in at the scheduler bean and disabled by default in the tracked template, including `TRADING_BTC_PRICE_MOVE_ALERT_ENABLED=false`.
...
- LongAiFilter is disabled by default in code and in the tracked template, including `TRADING_LONG_AI_FILTER_ENABLED=false`; production can opt in explicitly when LONG entry guard reads to Fear&Greed/OKX public market endpoints belong to this service.
- ShortAiFilter is disabled by default in code and in the tracked template, including `TRADING_SHORT_AI_FILTER_ENABLED=false`; production can opt in explicitly when short-signal external AI/MCP shadow checks belong to this service.
- Ensemble MCP preview live market reads are disabled by default in code and in the tracked template, including `TRADING_ENSEMBLE_PREVIEW_LIVE_MARKET_READS_ENABLED=false`; production can opt in explicitly when manual ensemble previews should read Fear&Greed, OKX, whale flow, and Polymarket directly.
- Market-data MCP live sentiment reads are disabled by default in code and in the tracked template, including `TRADING_MARKET_DATA_MCP_LIVE_SENTIMENT_ENABLED=false`; production can opt in explicitly when manual market-data tools such as sentiment dashboards, Polymarket risk checks, Fear&Greed history/backfill, or F&G trade-analysis should read Fear&Greed, whale flow, OKX, Polymarket, and orderbook endpoints directly.
- Market-data MCP external health probes are disabled by default in code and in the tracked template, including `TRADING_MARKET_DATA_MCP_EXTERNAL_HEALTH_PROBES_ENABLED=false`; production can opt in explicitly when `getSystemHealth` should actively ping OKX, Fear&Greed, Polymarket, whale flow, and orderbook endpoints.
- Market-data MCP external backfill/import tools are disabled by default in code and in the tracked template, including `TRADING_MARKET_DATA_MCP_EXTERNAL_BACKFILLS_ENABLED=false`; production can opt in explicitly when manual backfills should read OKX, The Graph, FRED, Hyperliquid, Polymarket, or Coinalyze and write `md_kline`, `market_indicator_history`, or historical odds rows.
- EventRiskControl remains a protective new-entry gate by default, but state-change Telegram notifications are disabled by default in code and in the tracked template, including `EVENT_RISK_CONTROL_STATUS_NOTIFY_ENABLED=false`.
...
- Autonomous exploration monitoring is bean-level explicit opt-in and disabled by default in code and in the tracked template, including `TRADING_EXPLORATION_MONITOR_ENABLED=false` and its Telegram notification flag.
- Autonomous exploration loop and auto-rollout promotion stay disabled in the tracked template, including `TRADING_EXPLORATION_LOOP_ENABLED=false`, `TRADING_EXPLORATION_LOOP_PRODUCTION_ENABLED=false`, `TRADING_EXPLORATION_ROLLOUT_AUTO_ENABLED=false`, `TRADING_EXPLORATION_ROLLOUT_ALLOW_PRODUCTION_PROMOTION=false`, and `TRADING_EXPLORATION_ROLLOUT_ALLOW_CAP_INCREASE=false`; auto-rollout scheduling is bean-level explicit opt-in.
- Market-signal risk-card scheduling is bean-level explicit opt-in, disabled, and dry-run by default in the tracked template, including `MARKET_SIGNAL_RISK_CARD_ENABLED=false` and `MARKET_SIGNAL_RISK_CARD_DRY_RUN=true`.
- Gemini advisor scheduling plus hint flip/staleness detector schedulers are bean-level explicit opt-in and disabled by default in code and in the tracked template, including `TRADING_GEMINI_ADVISOR_ENABLED=false`, `TRADING_GEMINI_ADVISOR_FLIP_DETECTOR_ENABLED=false`, and `TRADING_GEMINI_ADVISOR_STALENESS_DETECTOR_ENABLED=false`, so hint generation, hint-table scans, and related Telegram alerts are explicit production opt-in behavior.
- Signal outcome verification and alpha promotion scheduled tracking are disabled by default in code and in the tracked template; alpha promotion scheduling is bean-level explicit opt-in. Enable `SIGNAL_VERIFICATION_SCHEDULER_ENABLED=true` or `AGORA_ALPHA_TRACKER_ENABLED=true` only when this service should own outcome verification writes, snapshot-file writes, accuracy reports, and related Telegram alerts.
- Polymarket monitoring and WAI scheduled calculation are disabled by default in code and in the tracked template; WAI scheduling is bean-level explicit opt-in, so Polymarket API calls, odds/alert persistence, Telegram digests, and WAI `market_indicator_history` writes are explicit production opt-in behavior; WAI remains off with `TRADING_WAI_ENABLED=false`.
- Code and the tracked server env template keep market WebSocket side effects off by default, including `MARKET_LIQUIDATION_WS_ENABLED=false`; production can opt in explicitly when OKX public liquidation streams are intended.
- K-line divergence alerting is disabled by default in code and in the tracked template, including `TRADING_KLINE_DIVERGENCE_ENABLED=false`; both the manual scan and alert path require this opt-in before Telegram alerts can be emitted.
- Short-squeeze alerting and Binance taker-buy collection are bean-level explicit opt-in and disabled by default in code and in the tracked template, so Binance taker-buy polling, `market_indicator_history` writes, and related Telegram alerts are explicit production opt-in behavior.
- Grid runtime scheduling, auto-rebalance scheduling, and grid orphan recovery are disabled by default in code and in the tracked template; grid runtime and orphan recovery are bean-level explicit opt-in, so grid order placement, automatic range rebuilds, OKX fill lookups, recovery writes, and recovery Telegram alerts are explicit production opt-in behavior.
...
- TradingView webhook ingress is disabled and dry-run by default in code and in the tracked template; accepted alerts write decision-audit evidence only and do not place orders until a later tracked `bt_live_signal`/OCO execution path is separately implemented and authorized.
- Trading signal source policy defaults to `TRADING_SIGNAL_SOURCE_PRIMARY=TRADINGVIEW` and `TRADING_LEGACY_LIVE_EVALUATOR_ENABLED=false`, so K-line close events do not run the legacy `LiveSignalEvaluator` unless an explicit rollback opts into `LEGACY` primary plus legacy evaluator enablement. A separate free-account replacement lane is available through `TRADING_SIGNAL_SOURCE_PRIMARY=LOCAL_TRADINGVIEW` plus `TRADINGVIEW_LOCAL_ENABLED=true`; that lane only writes `LOCAL_TRADINGVIEW_PARITY` dry-run audit rows by default. `TRADINGVIEW_LOCAL_EXECUTION_MODE=DRY_RUN` records a dedicated LOCAL_TRADINGVIEW execution receipt for each parity order intent without using the ScoreBuy pre-position/confirmed/post-scout schedulers. `TRADINGVIEW_LOCAL_EXECUTION_MODE=BTC_BASE_DRY_RUN` records read-only BTC-base accumulation/shadow receipts from the same TradingView parity buy points and does not place orders, attach OCO, or require OCO lifecycle tracking. `TRADINGVIEW_LOCAL_EXECUTION_MODE=BTC_BASE_LIVE_MICRO` is the BTC-base live lane: when local TradingView parity BUY conditions pass, the service may place the configured small BTC spot buy, write trading audit/evidence rows, skip OCO by design, and enforce `TRADINGVIEW_LOCAL_BTC_BASE_MAX_EXPOSURE_USDT`. `TRADINGVIEW_LOCAL_EXECUTION_MODE=LIVE_MICRO` remains the OCO live lane: when local TradingView parity BUY conditions pass, the service may buy and immediately attach OCO without also setting the legacy three execution flags. A named legacy strategy can run beside `LOCAL_TRADINGVIEW` only when `TRADING_LEGACY_SECONDARY_EVALUATOR_ENABLED=true` and its id is listed in `TRADING_LEGACY_SECONDARY_ALLOWED_STRATEGY_IDS`; `TRADING_LEGACY_SECONDARY_MAX_NOTIONAL_USDT` caps that secondary live lane. Real LOCAL_TRADINGVIEW or secondary legacy orders still require OKX auto-trade enabled with private credentials, daily caps, duplicate-bar guards, and their lane-specific exposure/OCO gates; successful execution writes trading audit rows instead of changing marketplace state.
...
- Remaining `@Value` `:true` fallbacks are deliberately limited to protective/internal checks and dry-run flags. `scripts/verify_local.ps1` enforces the allowlist so new default-on `@Value` properties cannot bypass the split opt-in guardrails.
- Remaining `Environment.getProperty` default-`true` fallbacks are deliberately limited to MCP master-approval protection, ScoreBuy/TinyLive dry-run flags, and post-scout add sub-options behind disabled execution. `scripts/verify_local.ps1` enforces the allowlist so new default-on environment property fallbacks cannot bypass the split opt-in guardrails.
