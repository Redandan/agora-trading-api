...
- `SecurityPaths` exposes only the real retained service HTTP surfaces: OpenAPI docs, actuator probes/metrics, rate-limit JSON redirect, favicon, and authenticated dedicated-host Trading MCP. Trading MCP is available through server-local `/api/mcp` and public dedicated-host `/api/mcp` with bearer auth; shared-host `/api/trading/mcp` must be blocked by nginx.
