...
- Market-data MCP live sentiment dashboard reads are disabled by default in code and in the tracked template, including `TRADING_MARKET_DATA_MCP_LIVE_SENTIMENT_ENABLED=false`; production can opt in explicitly when manual sentiment dashboards should read Fear&Greed, whale flow, OKX, Polymarket, and orderbook endpoints directly.
