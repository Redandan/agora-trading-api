...
- Autonomous exploration monitoring is disabled by default in code and in the tracked template, including `TRADING_EXPLORATION_MONITOR_ENABLED=false` and its Telegram notification flag.
- Autonomous exploration loop and auto-rollout promotion stay disabled in the tracked template, including `TRADING_EXPLORATION_LOOP_ENABLED=false`, `TRADING_EXPLORATION_LOOP_PRODUCTION_ENABLED=false`, `TRADING_EXPLORATION_ROLLOUT_AUTO_ENABLED=false`, `TRADING_EXPLORATION_ROLLOUT_ALLOW_PRODUCTION_PROMOTION=false`, and `TRADING_EXPLORATION_ROLLOUT_ALLOW_CAP_INCREASE=false`.
