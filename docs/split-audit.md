...
- `smoke_local_health.ps1` clears host external keys for AgoraMarket, exchanges, Telegram, AI providers, and market-data providers, plus high-risk trading/deletion/AI automation toggles; smoke logs must prove `AiTaskRouter` initialized with 0 providers, Jina embeddings are disabled, public market WebSocket auto-subscribe is disabled, OKX liquidation WebSocket is disabled, startup market-data backfills are disabled, attribution/ML startup work is not scheduled, position-exit manager is disabled, trailing-stop OCO updates are disabled, and short-squeeze alerting plus taker-buy collection are disabled. OKX Earn top-up and Polymarket monitoring are bean-level opt-in, forced off by local smoke, and smoke logs must not show Earn redemption/transfer, Polymarket snapshot, Polymarket backfill, or Polymarket digest side effects.
...
- Event-scan scheduled outbound notifications also stay disabled and dry-run by default in the tracked template, including `EVENT_SCAN_NOTIFICATION_ENABLED=false` and `EVENT_SCAN_NOTIFICATION_DRY_RUN=true`.
- Execution-event scheduled scanning is disabled by default in the tracked template, including `EXECUTION_EVENT_ENABLED=false`; its notification path stays dry-run with `EXECUTION_EVENT_NOTIFICATION_DRY_RUN=true` unless explicitly enabled.
