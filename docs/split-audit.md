...
- Wick-capture shadow observation and historical bootstrap are disabled by default in code and in the tracked template, including `WICK_CAPTURE_SHADOW_ENABLED=false` and `WICK_CAPTURE_SHADOW_BOOTSTRAP_ENABLED=false`, so shadow table writes, attention audit rows, outcome backfill writes, and Telegram context alerts are explicit production opt-in behavior.
