...
- Short-squeeze alerting and Binance taker-buy collection are disabled by default in code and in the tracked template, so Binance taker-buy polling, `market_indicator_history` writes, and related Telegram alerts are explicit production opt-in behavior.
