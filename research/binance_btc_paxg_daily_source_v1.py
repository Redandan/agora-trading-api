#!/usr/bin/env python3
"""Acquire one deterministic checksum-bound BTCUSDT/PAXGUSDT daily corpus."""
...
FIRST_MONTH = "2020-09"
LAST_MONTH = "2024-12"
FIRST_DAY = date(2020, 9, 1)
LAST_DAY = date(2024, 12, 31)
EXPECTED_MANIFEST_TYPE = "BTC_PAXG_MONTHLY_RELATIVE_MOMENTUM_SOURCE_MANIFEST_V1"
...
def verify_source_manifest(path: Path) -> dict[str, Any]:
    try:
        ...
        raise common.CorpusReject("MANIFEST_REJECT:JSON") from error
    if manifest.get("document_type") != EXPECTED_MANIFEST_TYPE:
        raise common.CorpusReject("MANIFEST_REJECT:DOCUMENT_TYPE")
    if manifest.get("family_id") != EXPECTED_FAMILY_ID:
        raise common.CorpusReject("MANIFEST_REJECT:FAMILY_ID")
...
def acquire_archives() -> tuple[list[common.DailyBar], list[dict[str, Any]]]:
    ...
    with concurrent.futures.ThreadPoolExecutor(
        ...
        downloaded.extend(executor.map(common.download_archive, items))
...
def main() -> int:
    ...
    bundle = {
        "schema_version": "1",
        "document_type": "BINANCE_BTC_PAXG_SPOT_DAILY_SOURCE_CORPUS_V1",
        "authorization": "RESEARCH_ONLY_NOT_SHADOW_PAPER_OR_LIVE",
        "status": "SEALED_CHECKSUM_VERIFIED_COMPLETE_BTC_PAXG_2020_09_2024_SOURCE_NO_OUTCOME",
