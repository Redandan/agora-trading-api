#!/usr/bin/env python3
"""Preregistered Coin Metrics BTC hash-rate growth admission screen for DRA V1."""
...
DOCUMENT_TYPE = "BTC_DRA_BITCOIN_HASHRATE_GROWTH_ENTRY_ADMISSION_MANIFEST_V1"
RESULT_TYPE = "BTC_DRA_BITCOIN_HASHRATE_GROWTH_ENTRY_ADMISSION_SCREEN_V1"
RUNNER_IDENTITY = "BTC_DRA_BITCOIN_HASHRATE_GROWTH_ENTRY_ADMISSION_RUNNER_V1"
FACTOR_IDENTITY = "COIN_METRICS_BTC_HASHRATE_STRICT_POSITIVE_ADJACENT_28D_GROWTH_168H_V1"
EXPERIMENT_ID = "dra-bitcoin-hashrate-growth-entry-admission-historical-v1"
...
def validate_manifest(value: Any) -> dict[str, Any]:
    ...
    if manifest["factor"] != {
        "admission_rule": "ADMIT_PARENT_SIGNAL_ONLY_WHEN_CURRENT_28D_MEAN_HASHRATE_GT_IMMEDIATELY_PRECEDING_28D_MEAN",
        "factor_identity": FACTOR_IDENTITY,
        "formula": "current_28d_mean_hashrate-prior_nonoverlapping_28d_mean_hashrate",
    ...
    expected_paths = {
        ...
        "hypothesis": "research_pipeline/examples/dra-bitcoin-hashrate-growth-entry-admission-v1.hypothesis.json",
        "manifest_schema": "research_pipeline/btc-dra-bitcoin-hashrate-growth-entry-admission-manifest.v1.schema.json",
        "normalized_source": ".research-state/experiments/dra-bitcoin-hashrate-growth-entry-admission-historical-v1/inputs/coinmetrics-btc-hashrate-2018-2024.csv",
        "primary_prior": "research_pipeline/examples/dra-bitcoin-hashrate-growth-primary-prior.v1.json",
        "raw_source_response": ".research-state/experiments/dra-bitcoin-hashrate-growth-entry-admission-historical-v1/inputs/coinmetrics-btc-hashrate-2018-2024-raw.json",
        "reused_economic_runner": "research/btc_dra_cftc_tff_entry_admission_historical_v1.py",
        "runner": "research/btc_dra_bitcoin_hashrate_growth_entry_admission_historical_v1.py",
        "source_bundle": ".research-state/experiments/dra-bitcoin-hashrate-growth-entry-admission-historical-v1/inputs/coinmetrics-source-bundle.json",
        "source_metadata": "research_pipeline/examples/coinmetrics-btc-hashrate-daily-2018-2024.v1.source.json",
        "source_probe": "research/coinmetrics_btc_hashrate_source_probe.cjs",
    }
...
def load_hashrate(bindings: dict[str, Any]) -> tuple[dict[date, D], dict[str, Any]]:
    ...
    try:
        ...
        raise reused.ScreenReject("DATA_REJECT", "normalized HashRate source is not UTF-8") from error
    if not csv_rows or csv_rows[0] != ["date", "hash_rate_th_per_s"]:
        raise reused.ScreenReject("DATA_REJECT", "normalized HashRate header drift")
    rows: dict[date, D] = {}
    for index, row in enumerate(csv_rows[1:]):
        if len(row) != 2:
            raise reused.ScreenReject("DATA_REJECT", f"normalized HashRate row malformed: {index}")
        try:
            day = date.fromisoformat(row[0])
            value = D(row[1])
        except (ValueError, InvalidOperation) as error:
            raise reused.ScreenReject("DATA_REJECT", f"normalized HashRate value malformed: {index}") from error
        if day in rows or not value.is_finite() or value <= 0:
            raise reused.ScreenReject("DATA_REJECT", f"normalized HashRate identity drift: {index}")
        rows[day] = value
    ordered = sorted(rows)
    if len(ordered) != EXPECTED_ROWS or ordered[0] != EXPECTED_FIRST or ordered[-1] != EXPECTED_LAST:
        raise reused.ScreenReject("DATA_REJECT", "normalized HashRate sample boundary drift")
    if any(current - prior != timedelta(days=1) for prior, current in zip(ordered, ordered[1:], strict=False)):
        raise reused.ScreenReject("DATA_REJECT", "normalized HashRate daily continuity drift")
    ...
    if (
        ...
        or metadata.get("metric") != "HashRate"
...
def build_factor_points(rows: dict[date, D]) -> tuple[list[dict[str, Any]], dict[str, int]]:
    ...
    if full_days[0].weekday() != 0 or full_days[-1].weekday() != 6:
        raise reused.ScreenReject("DATA_REJECT", "HashRate complete-week boundary drift")
...
def run_screen(manifest_path: Path, input_path: Path, output_path: Path) -> dict[str, Any]:
    ...
    hashrate_rows, source_evidence = load_hashrate(bindings)
    factor_points, exclusions = build_factor_points(hashrate_rows)
    ...
    result = {
        ...
        "recommended_next_action": "REGISTER_ONE_FORMAL_CANDIDATE_FOR_INDEPENDENT_OOS" if passed else "PERMANENTLY_CLOSE_EXACT_BITCOIN_HASHRATE_GROWTH_FAMILY_WITHOUT_TUNING",
        "runner_identity": RUNNER_IDENTITY,
        "runner_sha256": reused.sha256_path(Path(__file__)),
        "schema_version": "1",
        "source_evidence": source_evidence,
        "status": "DESIGN_VALIDATION_PASS_READY_FOR_ONE_CANDIDATE" if passed else "NO_CANDIDATE_CLOSE_BITCOIN_HASHRATE_GROWTH_FAMILY",
    }
