#!/usr/bin/env python3
"""Preregistered FRED WALCL balance-sheet growth admission screen for BTC DRA V1."""
...
DOCUMENT_TYPE = "BTC_DRA_FED_BALANCE_SHEET_GROWTH_ENTRY_ADMISSION_MANIFEST_V1"
RESULT_TYPE = "BTC_DRA_FED_BALANCE_SHEET_GROWTH_ENTRY_ADMISSION_SCREEN_V1"
RUNNER_IDENTITY = "BTC_DRA_FED_BALANCE_SHEET_GROWTH_ENTRY_ADMISSION_RUNNER_V1"
FACTOR_IDENTITY = "FRED_WALCL_STRICT_POSITIVE_28D_GROWTH_CONTINUATION_168H_V1"
EXPERIMENT_ID = "dra-fed-balance-sheet-growth-entry-admission-historical-v1"
...
def validate_manifest(value: Any) -> dict[str, Any]:
    ...
    if manifest["availability"] != {
        ...
        "growth_lag_calendar_days": GROWTH_LAG_DAYS,
        "non_wednesday_action": "REJECT_SOURCE",
        "publication_lag_calendar_days": AVAILABILITY_LAG_DAYS,
    }:
    ...
    bindings = reused._exact_keys(
        manifest["bindings"],
        {
            ...
            "normalized_source", "primary_prior", "raw_source_archive",
            "reused_economic_runner", "runner", "source_bundle",
            "source_fetch_amendment", "source_probe",
        },
        "bindings",
    )
    expected_paths = {
        ...
        "hypothesis": "research_pipeline/examples/dra-fed-balance-sheet-growth-entry-admission-v1.hypothesis.json",
        "manifest_schema": "research_pipeline/btc-dra-fed-balance-sheet-growth-entry-admission-manifest.v1.schema.json",
        "normalized_source": ".research-state/experiments/dra-fed-balance-sheet-growth-entry-admission-historical-v1/inputs/fred-walcl-2018-2024.csv",
        "primary_prior": "research_pipeline/examples/dra-fed-balance-sheet-growth-primary-prior.v1.json",
        "raw_source_archive": ".research-state/experiments/dra-fed-balance-sheet-growth-entry-admission-historical-v1/inputs/fred-walcl-2018-2024-source-parts.zip",
        "reused_economic_runner": "research/btc_dra_cftc_tff_entry_admission_historical_v1.py",
        "runner": "research/btc_dra_fed_balance_sheet_growth_entry_admission_historical_v1.py",
        "source_bundle": ".research-state/experiments/dra-fed-balance-sheet-growth-entry-admission-historical-v1/inputs/fred-source-bundle.json",
        "source_fetch_amendment": "research_pipeline/examples/dra-fed-balance-sheet-growth-source-fetch-amendment.v1.json",
        "source_probe": "research/fred_walcl_source_probe.py",
    }
...
def load_walcl(manifest: dict[str, Any], bindings: dict[str, Any]) -> tuple[dict[date, int], dict[str, Any]]:
    ...
    try:
        ...
        raise reused.ScreenReject("DATA_REJECT", "normalized WALCL source is not UTF-8") from error
    if not csv_rows or csv_rows[0] != ["observation_date", "walcl_millions_usd"]:
        raise reused.ScreenReject("DATA_REJECT", "normalized WALCL header drift")
    rows: dict[date, int] = {}
    for index, row in enumerate(csv_rows[1:]):
        if len(row) != 2 or not row[1].isdigit():
            raise reused.ScreenReject("DATA_REJECT", f"normalized WALCL row malformed: {index}")
        try:
            day = date.fromisoformat(row[0])
        except ValueError as error:
            raise reused.ScreenReject("DATA_REJECT", f"normalized WALCL date malformed: {index}") from error
        if day in rows or day.weekday() != 2 or int(row[1]) <= 0:
            raise reused.ScreenReject("DATA_REJECT", f"normalized WALCL identity drift: {index}")
        rows[day] = int(row[1])
    ordered = sorted(rows)
    if len(ordered) != EXPECTED_ROWS or ordered[0] != EXPECTED_FIRST or ordered[-1] != EXPECTED_LAST:
        raise reused.ScreenReject("DATA_REJECT", "normalized WALCL sample boundary drift")
    if any(current - prior != timedelta(days=7) for prior, current in zip(ordered, ordered[1:], strict=False)):
        raise reused.ScreenReject("DATA_REJECT", "normalized WALCL weekly continuity drift")
    ...
    if (
        ...
        or archive.get("rows") != EXPECTED_ROWS
    ):
    ...
    return rows, {
        ...
        "rows": len(rows),
        "present_vintage_revision_boundary": "ORIGINAL_RELEASE_VALUES_AND_REVISION_VINTAGES_MISSING_PROOF",
    }


def build_factor_points(rows: dict[date, int]) -> tuple[list[dict[str, Any]], dict[str, int]]:
    points: list[dict[str, Any]] = []
    exclusions = {"MISSING_EXACT_28D_PREDECESSOR": 0, "DECISION_AT_OR_AFTER_CUTOFF": 0}
    for day in sorted(rows):
        prior_day = day - timedelta(days=GROWTH_LAG_DAYS)
        if prior_day not in rows:
        ...
        if eligible_at >= SELECTION_CUTOFF:
            ...
            continue
        delta = D(rows[day]) - D(rows[prior_day])
        points.append({
...
def run_screen(manifest_path: Path, input_path: Path, output_path: Path) -> dict[str, Any]:
    ...
    walcl_rows, source_evidence = load_walcl(manifest, bindings)
    factor_points, exclusions = build_factor_points(walcl_rows)
    ...
    result = {
        ...
        "recommended_next_action": "REGISTER_ONE_FORMAL_CANDIDATE_FOR_INDEPENDENT_OOS" if passed else "PERMANENTLY_CLOSE_EXACT_FED_BALANCE_SHEET_GROWTH_FAMILY_WITHOUT_TUNING",
        "runner_identity": RUNNER_IDENTITY,
        "runner_sha256": reused.sha256_path(Path(__file__)),
        "schema_version": "1",
        "source_evidence": source_evidence,
        "status": "DESIGN_VALIDATION_PASS_READY_FOR_ONE_CANDIDATE" if passed else "NO_CANDIDATE_CLOSE_FED_BALANCE_SHEET_GROWTH_FAMILY",
    }
