#!/usr/bin/env python3
"""Deterministic matched-capital DRA signal-confirmation-latency screen."""
...
MANIFEST_TYPE = "DRA_SIGNAL_CONFIRMATION_LATENCY_ENTRY_ADMISSION_MANIFEST_V1"
RESULT_TYPE = "DRA_SIGNAL_CONFIRMATION_LATENCY_ENTRY_ADMISSION_ECONOMIC_SCREEN_V1"
RUNNER_IDENTITY = "BTC_DRA_SIGNAL_CONFIRMATION_LATENCY_ENTRY_ADMISSION_RUNNER_V1"
PARENT_STRATEGY = "BTC_DRA_V1_BASELINE_250_USDT_FIXED_30_USDT_LOTS"
GATE_SET = "DRA_SIGNAL_CONFIRMATION_LATENCY_ENTRY_ADMISSION_ECONOMIC_GATES_V1"
...
FORBIDDEN_RESCUE_ACTIONS = [
    ...
    "ARMED_OR_SIGNAL_CLOCK_TUNING_AFTER_OUTCOME",
    "ARM_WINDOW_EXPIRY_OR_REARM_TUNING_AFTER_OUTCOME",
    "COOLDOWN_OR_CAPACITY_ROUTE_TUNING_AFTER_OUTCOME",
    "PRICE_INVENTORY_OR_PNL_AWARE_DECISION_FILTER_AFTER_OUTCOME",
...
def load_manifest(path: Path) -> tuple[dict[str, Any], bytes]:
    ...
    if value.get("mechanism") != {
        ...
        "price_or_inventory_state_read": "DENY",
    ...
    if value.get("gate_contract") != {
        ...
        "failure_disposition": "NO_CANDIDATE_PERMANENTLY_CLOSE_DRA_SIGNAL_CONFIRMATION_LATENCY_FAMILY",
...
class SignalConfirmationLatencyAdmissionEngine(capacity.EqualCapitalCapacityEngine):
    """Change only admission; decision state reads parent lifecycle clocks."""
    ...
    def __init__(self, *, threshold_hours: D) -> None:
        ...
        self.maximum_observed_confirmation_latency_hours = D("0")
    ...
    def _entry_lifecycle(self, bar: base.Bar) -> None:
        ...
        if self.armed_at is not None and bar.open_time > self.armed_at and self._signal(bar):
            ...
            if open_cost + LOT_COST_USDT > self.cap:
                ...
                latency_hours = D(state["confirmation_latency_hours"])
                self.maximum_observed_confirmation_latency_hours = max(
                    self.maximum_observed_confirmation_latency_hours, latency_hours
                )
        ...
        result.update(
            {
                ...
                "decision_state_fields_read": ["parent_armed_at", "signal_time"],
                "price_or_inventory_state_read_for_decision": False,
                ...
                "maximum_observed_confirmation_latency_hours": str(
                    self.maximum_observed_confirmation_latency_hours
                ),
...
def simulate_candidate(
    ...
    *,
    ...
    engine = SignalConfirmationLatencyAdmissionEngine(threshold_hours=threshold_hours)
    for bar in selected:
...
def run_screen(manifest_path: Path, input_path: Path, output_path: Path) -> dict[str, Any]:
    ...
    result = {
        ...
        "status": (
            ...
            if passed
            else "NO_CANDIDATE_PERMANENTLY_CLOSE_DRA_SIGNAL_CONFIRMATION_LATENCY_FAMILY"
        ),
        "recommended_next_action": (
            "FREEZE_SEPARATE_INDEPENDENT_OOS_CONTRACT_WITHOUT_ACTIVATION"
            if passed
            else "CLOSE_FAMILY_WITHOUT_THRESHOLD_CLOCK_ARM_WINDOW_COOLDOWN_ROUTE_OR_GATE_TUNING"
        ),
