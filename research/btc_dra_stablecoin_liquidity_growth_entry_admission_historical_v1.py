#!/usr/bin/env python3
"""Preregistered stablecoin-liquidity growth admission screen for BTC DRA V1."""
...
DOCUMENT_TYPE = "BTC_DRA_STABLECOIN_LIQUIDITY_GROWTH_ENTRY_ADMISSION_MANIFEST_V1"
RESULT_TYPE = "BTC_DRA_STABLECOIN_LIQUIDITY_GROWTH_ENTRY_ADMISSION_SCREEN_V1"
RUNNER_IDENTITY = "BTC_DRA_STABLECOIN_LIQUIDITY_GROWTH_ENTRY_ADMISSION_RUNNER_V1"
FACTOR_IDENTITIES = {
    "PRIMARY_COMBINED": "COIN_METRICS_USDT_USDC_CAPMRKTCURUSD_POSITIVE_28D_GROWTH_D_PLUS_2_DRA_ENTRY_ADMISSION_V1",
    "NEIGHBOR_USDT_ONLY": "COIN_METRICS_USDT_CAPMRKTCURUSD_POSITIVE_28D_GROWTH_D_PLUS_2_DRA_ENTRY_ADMISSION_V1",
    "NEIGHBOR_USDC_ONLY": "COIN_METRICS_USDC_CAPMRKTCURUSD_POSITIVE_28D_GROWTH_D_PLUS_2_DRA_ENTRY_ADMISSION_V1",
}
EXPERIMENT_ID = "dra-stablecoin-liquidity-growth-entry-admission-historical-v1"
...
EXPECTED_ROWS = 2287
EXPECTED_FIRST = date(2018, 9, 28)
EXPECTED_LAST = date(2024, 12, 31)
REFERENCE_DAYS = 28
PUBLICATION_LAG_DAYS = 2
FACTOR_VALID_HOURS = 24
VARIANT_ORDER = ("PRIMARY_COMBINED", "NEIGHBOR_USDT_ONLY", "NEIGHBOR_USDC_ONLY")
...
def validate_manifest(value: Any) -> dict[str, Any]:
    ...
    if manifest["factor"] != {
        ...
        "factor_identity": FACTOR_IDENTITIES["PRIMARY_COMBINED"],
        "growth_lag_calendar_days": REFERENCE_DAYS,
    ...
    expected_paths = {
        ...
        "manifest_schema": "research_pipeline/btc-dra-stablecoin-liquidity-growth-entry-admission-manifest.v1.schema.json",
        "negative_long_cash_decision": "research_pipeline/examples/btc-stablecoin-liquidity-growth-long-cash-historical.v1.decision.json",
        "normalized_source": ".research-state/experiments/btc-stablecoin-liquidity-growth-long-cash-historical-v1/inputs/coinmetrics-usdt-usdc-stablecoin-liquidity-2018-2024.csv",
        "primary_prior": "research_pipeline/examples/dra-stablecoin-liquidity-growth-entry-admission-primary-prior.v1.json",
        "raw_source_response": ".research-state/experiments/btc-stablecoin-liquidity-growth-long-cash-historical-v1/inputs/coinmetrics-usdt-usdc-stablecoin-liquidity-2018-2024-raw.json",
        "reused_economic_runner": "research/btc_dra_cftc_tff_entry_admission_historical_v1.py",
        "runner": "research/btc_dra_stablecoin_liquidity_growth_entry_admission_historical_v1.py",
        "source_bundle": ".research-state/experiments/btc-stablecoin-liquidity-growth-long-cash-historical-v1/inputs/coinmetrics-source-bundle.json",
        "source_metadata": "research_pipeline/examples/coinmetrics-usdt-usdc-stablecoin-liquidity-daily-2018-2024.v1.source.json",
        "source_probe": "research/coinmetrics_stablecoin_liquidity_source_probe.cjs",
    }
...
def load_stablecoin_rows(
    ...
    try:
        ...
        raise reused.ScreenReject("DATA_REJECT", "normalized stablecoin source is not UTF-8") from error
    if not csv_rows or csv_rows[0] != [
        "date", "usdt_cap_mrkt_cur_usd", "usdc_cap_mrkt_cur_usd"
    ]:
        raise reused.ScreenReject("DATA_REJECT", "normalized stablecoin header drift")
    rows: dict[date, tuple[D, D]] = {}
    for index, row in enumerate(csv_rows[1:]):
        if len(row) != 3:
            raise reused.ScreenReject("DATA_REJECT", f"normalized stablecoin row malformed: {index}")
        try:
            day = date.fromisoformat(row[0])
            usdt, usdc = D(row[1]), D(row[2])
        except (ValueError, InvalidOperation) as error:
            raise reused.ScreenReject("DATA_REJECT", f"normalized stablecoin value malformed: {index}") from error
        if day in rows or any(not value.is_finite() or value <= 0 for value in (usdt, usdc)):
            raise reused.ScreenReject("DATA_REJECT", f"normalized stablecoin identity drift: {index}")
        rows[day] = (usdt, usdc)
    ordered = sorted(rows)
    if len(ordered) != EXPECTED_ROWS or ordered[0] != EXPECTED_FIRST or ordered[-1] != EXPECTED_LAST:
        raise reused.ScreenReject("DATA_REJECT", "normalized stablecoin sample boundary drift")
    if len(raw) != 172992 or any(
        current - prior != timedelta(days=1)
        for prior, current in zip(ordered, ordered[1:], strict=False)
    ):
        raise reused.ScreenReject("DATA_REJECT", "normalized stablecoin continuity drift")
    ...
    if (
        ...
        or source_raw.get("rows") != EXPECTED_ROWS * 2
    ):
    ...
    if (
        metadata.get("status") != "SEALED_PRESENT_VINTAGE_SOURCE_READY_FOR_PREREGISTERED_HISTORICAL_AUDIT"
        or metadata.get("normalized_subset", {}).get("sha256") != bindings["normalized_source"]["sha256"]
...
def _level(values: tuple[D, D], mode: str) -> D:
    if mode == "PRIMARY_COMBINED":
        return values[0] + values[1]
    if mode == "NEIGHBOR_USDT_ONLY":
        return values[0]
    if mode == "NEIGHBOR_USDC_ONLY":
        return values[1]
...
def build_factor_points(
    ...
    exclusions = {"MISSING_PRIOR_28D_OBSERVATION": REFERENCE_DAYS, "DECISION_AT_OR_AFTER_CUTOFF": 0}
    for index in range(REFERENCE_DAYS, len(ordered)):
        ...
        if eligible_at >= SELECTION_CUTOFF:
            ...
            continue
        prior_day = ordered[index - REFERENCE_DAYS]
        delta = _level(rows[report_day], mode) - _level(rows[prior_day], mode)
        points.append({
...
def run_screen(manifest_path: Path, input_path: Path, output_path: Path) -> dict[str, Any]:
    ...
    rows, source_evidence = load_stablecoin_rows(bindings)
    ...
    primary = variants["PRIMARY_COMBINED"]
    neighbor_stability = {
    ...
    result = {
        ...
        "recommended_next_action": "REGISTER_ONE_FORMAL_CANDIDATE_FOR_INDEPENDENT_OOS" if passed else "PERMANENTLY_CLOSE_EXACT_STABLECOIN_LIQUIDITY_DRA_ADMISSION_FAMILY_WITHOUT_TUNING",
        "runner_identity": RUNNER_IDENTITY,
        "runner_sha256": reused.sha256_path(Path(__file__)),
        "schema_version": "1",
        "source_evidence": source_evidence,
        "status": "DESIGN_VALIDATION_PASS_READY_FOR_ONE_CANDIDATE" if passed else "NO_CANDIDATE_CLOSE_STABLECOIN_LIQUIDITY_DRA_ADMISSION_FAMILY",
        "variants": variants,
    }
