#!/usr/bin/env python3
"""Preregistered lagged weekly US EPU admission screen for BTC DRA V1."""
...
DOCUMENT_TYPE = "BTC_DRA_US_EPU_HEDGE_ENTRY_ADMISSION_MANIFEST_V1"
RESULT_TYPE = "BTC_DRA_US_EPU_HEDGE_ENTRY_ADMISSION_SCREEN_V1"
RUNNER_IDENTITY = "BTC_DRA_US_EPU_HEDGE_ENTRY_ADMISSION_RUNNER_V1"
FACTOR_IDENTITY = "FRED_USEPUINDXD_WEEKLY_MEAN_TO_PRIOR_52W_MEDIAN_AT_OR_ABOVE_168H_V1"
EXPERIMENT_ID = "dra-us-epu-hedge-entry-admission-historical-v1"
PARENT_STRATEGY = reused.PARENT_STRATEGY
GATE_SET = "BTC_DRA_US_EPU_HEDGE_ENTRY_ADMISSION_GATES_V1"
...
PRIMARY_THRESHOLD = D("1.00")
NEIGHBOR_THRESHOLDS = (D("0.80"), D("1.20"))
...
def validate_manifest(value: Any) -> dict[str, Any]:
    ...
    if manifest["factor"] != {
        "admission_rule": "ADMIT_PARENT_SIGNAL_ONLY_WHEN_WEEKLY_USEPUINDXD_MEAN_TO_PRIOR_52W_MEDIAN_AT_OR_ABOVE_THRESHOLD",
        "factor_identity": FACTOR_IDENTITY,
        "formula": "current_complete_week_mean_usepuindxd/median_prior_52_nonoverlapping_complete_week_means",
        "primary_threshold": "1.00",
        "relation": "AT_OR_ABOVE",
        "stability_neighbors": ["0.80", "1.20"],
        "veto_action": "HOLD_CASH",
    }:
        raise reused.ScreenReject("CONTRACT_REJECT", "factor semantics drift")
    if manifest["availability"] != {
        "eligible_time": "WEEK_ENDING_SUNDAY_PLUS_7_CALENDAR_DAYS_AT_00_00_UTC",
        "factor_valid_hours": FACTOR_VALID_HOURS,
        "publication_lag_calendar_days": PUBLICATION_LAG_DAYS,
        "week_definition": "EXACTLY_SEVEN_DAILY_OBSERVATIONS_MONDAY_THROUGH_SUNDAY",
    }:
    ...
    expected_paths = {
        ...
        "hypothesis": "research_pipeline/examples/dra-us-epu-hedge-entry-admission-v1.hypothesis.json",
        "manifest_schema": "research_pipeline/btc-dra-us-epu-hedge-entry-admission-manifest.v1.schema.json",
        "normalized_source": ".research-state/experiments/dra-us-epu-hedge-entry-admission-historical-v1/inputs/fred-usepuindxd-daily-2018-2024.csv",
        "primary_prior": "research_pipeline/examples/dra-us-epu-hedge-entry-admission-primary-prior.v1.json",
        "raw_source_response": ".research-state/experiments/dra-us-epu-hedge-entry-admission-historical-v1/inputs/fred-usepuindxd-2018-2024-raw.csv",
        "reused_economic_runner": "research/btc_dra_cftc_tff_entry_admission_historical_v1.py",
        "runner": "research/btc_dra_us_epu_hedge_entry_admission_historical_v1.py",
        "source_bundle": ".research-state/experiments/dra-us-epu-hedge-entry-admission-historical-v1/inputs/fred-source-bundle.json",
        "source_metadata": "research_pipeline/examples/fred-usepuindxd-daily-2018-2024.v1.source.json",
        "source_probe": "research/fred_usepuindxd_source_probe.py",
    }
...
def load_epu(bindings: dict[str, Any]) -> tuple[dict[date, D], dict[str, Any]]:
    ...
    try:
        ...
        raise reused.ScreenReject("DATA_REJECT", "normalized USEPUINDXD source is not UTF-8") from error
    if not csv_rows or csv_rows[0] != ["observation_date", "usepuindxd"]:
        raise reused.ScreenReject("DATA_REJECT", "normalized USEPUINDXD header drift")
    rows: dict[date, D] = {}
    for index, row in enumerate(csv_rows[1:]):
        if len(row) != 2:
            raise reused.ScreenReject("DATA_REJECT", f"normalized USEPUINDXD row malformed: {index}")
        try:
            day = date.fromisoformat(row[0])
            value = D(row[1])
        except (ValueError, InvalidOperation) as error:
            raise reused.ScreenReject("DATA_REJECT", f"normalized USEPUINDXD value malformed: {index}") from error
        if day in rows or not value.is_finite() or value <= 0:
            raise reused.ScreenReject("DATA_REJECT", f"normalized USEPUINDXD identity drift: {index}")
        rows[day] = value
    ordered = sorted(rows)
    if len(ordered) != EXPECTED_ROWS or ordered[0] != EXPECTED_FIRST or ordered[-1] != EXPECTED_LAST:
        raise reused.ScreenReject("DATA_REJECT", "normalized USEPUINDXD sample boundary drift")
    if any(current - prior != timedelta(days=1) for prior, current in zip(ordered, ordered[1:], strict=False)):
        raise reused.ScreenReject("DATA_REJECT", "normalized USEPUINDXD daily continuity drift")
    ...
    normalized = bundle.get("normalized_daily_subset", {})
    ...
    try:
        ...
        raise reused.ScreenReject("DATA_REJECT", "source metadata is not strict JSON") from error
    if (
        metadata.get("status") != "SEALED_HISTORICAL_SOURCE"
        or metadata.get("series") != "USEPUINDXD"
        or metadata.get("source_bundle", {}).get("sha256") != bindings["source_bundle"]["sha256"]
    ...
    return rows, {
        ...
        "present_vintage_revision_boundary": "ORIGINAL_NEWS_INDEX_RELEASE_VINTAGES_AND_REVISIONS_MISSING_PROOF",
    }
...
def build_factor_points(rows: dict[date, D], threshold: D) -> tuple[list[dict[str, Any]], dict[str, int]]:
    if threshold not in THRESHOLDS:
        raise reused.ScreenReject("CONTRACT_REJECT", "unregistered EPU threshold")
    ordered = sorted(rows)
    full_days = ordered[:FULL_WEEK_DAYS]
    if full_days[0].weekday() != 0 or full_days[-1].weekday() != 6:
        raise reused.ScreenReject("DATA_REJECT", "USEPUINDXD complete-week boundary drift")
    weekly: list[tuple[date, D]] = []
    for start_index in range(0, len(full_days), 7):
        days = full_days[start_index:start_index + 7]
        if len(days) != 7 or any(current - prior != timedelta(days=1) for prior, current in zip(days, days[1:], strict=False)):
            raise reused.ScreenReject("DATA_REJECT", "USEPUINDXD incomplete week")
        weekly.append((days[-1], sum((rows[day] for day in days), D("0")) / D("7")))
    if len(weekly) != EXPECTED_COMPLETE_WEEKS:
        raise reused.ScreenReject("DATA_REJECT", "USEPUINDXD complete-week count drift")
    points: list[dict[str, Any]] = []
    exclusions = {
        "MISSING_PRIOR_52_COMPLETE_WEEKS": PRIOR_WEEKS,
        "INCOMPLETE_TAIL_DAYS": len(ordered) - len(full_days),
        "DECISION_AT_OR_AFTER_CUTOFF": 0,
    }
    for index in range(PRIOR_WEEKS, len(weekly)):
        week_end, current_mean = weekly[index]
        ...
        if eligible_at >= SELECTION_CUTOFF:
            ...
            continue
        prior_median = _median([value for _, value in weekly[index - PRIOR_WEEKS:index]])
        if prior_median <= 0:
            raise reused.ScreenReject("DATA_REJECT", "USEPUINDXD prior median is not positive")
        ratio = current_mean / prior_median
        points.append({
            "report_date": week_end.isoformat(),
            "prior_report_date": weekly[index - 1][0].isoformat(),
            "eligible_at": eligible_at.isoformat(),
            "factor_delta": str(ratio - threshold),
            "factor_ratio": str(ratio),
            "factor_sign": 1 if ratio >= threshold else -1,
...
def _run_variant(
    bars: list[reused.base.Bar], baseline: dict[str, Any], rows: dict[date, D], threshold: D
) -> tuple[dict[str, Any], list[dict[str, Any]], dict[str, int]]:
    factor_points, exclusions = build_factor_points(rows, threshold)
...
def _neighbor_stability_gates(
    variants: dict[str, dict[str, Any]], baseline: dict[str, Any]
) -> dict[str, bool]:
    gates: dict[str, bool] = {}
    for threshold in NEIGHBOR_THRESHOLDS:
        label = format(threshold, ".2f").replace(".", "_")
        evidence = variants[format(threshold, ".2f")]["economic_evidence"]
        design = evidence["design"]
        validation = evidence["validation"]
        gates[f"neighbor_{label}_design_total_pnl_improves"] = (
            reused._value(design, "total_pnl_usdt") > reused._value(baseline["design"], "total_pnl_usdt")
        )
        gates[f"neighbor_{label}_validation_total_pnl_improves"] = (
            reused._value(validation, "total_pnl_usdt") > reused._value(baseline["validation"], "total_pnl_usdt")
        )
        gates[f"neighbor_{label}_validation_realized_pnl_improves"] = (
            reused._value(validation, "realized_usdt") > reused._value(baseline["validation"], "realized_usdt")
        )
        gates[f"neighbor_{label}_validation_drawdown_within_0_25pp"] = (
            reused._value(validation, "max_drawdown_pct")
            <= reused._value(baseline["validation"], "max_drawdown_pct") + reused.DD_TOLERANCE_PP
        )
...
def run_screen(manifest_path: Path, input_path: Path, output_path: Path) -> dict[str, Any]:
    ...
    passed = (
        all(primary_economic_gates.values())
        and all(all(window["gates"].values()) for window in primary_predictive.values())
        and all(neighbor_stability_gates.values())
    )
    result = {
        "authorization": AUTHORIZATION,
        "baseline": baseline,
        ...
        "recommended_next_action": "REGISTER_ONE_FORMAL_CANDIDATE_FOR_INDEPENDENT_OOS" if passed else "PERMANENTLY_CLOSE_EXACT_US_EPU_HEDGE_ENTRY_ADMISSION_FAMILY_WITHOUT_TUNING",
        "runner_identity": RUNNER_IDENTITY,
        "runner_sha256": reused.sha256_path(Path(__file__)),
        "schema_version": "1",
        "source_evidence": source_evidence,
        "status": "DESIGN_VALIDATION_PASS_READY_FOR_ONE_CANDIDATE" if passed else "NO_CANDIDATE_CLOSE_US_EPU_HEDGE_ENTRY_ADMISSION_FAMILY",
