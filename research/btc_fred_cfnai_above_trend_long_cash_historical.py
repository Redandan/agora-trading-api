#!/usr/bin/env python3
"""Deterministic matched-capital audit for lagged positive CFNAI."""
...
EXPERIMENT_ID = "btc-fred-cfnai-above-trend-long-cash-historical-v1"
EXPECTED_MANIFEST_TYPE = "BTC_FRED_CFNAI_ABOVE_TREND_LONG_CASH_HISTORICAL_MANIFEST_V1"
EXPECTED_DATA_SHA256 = "e436a5a2b093365886464dd3e471cc5cd55c54bd2c06ceaa898ac218c45436dd"
EXPECTED_DATA_ROWS = 52_608
CFNAI_SOURCE = REPO_ROOT / (
    ".research-state/experiments/"
    "btc-fred-cfnai-above-trend-long-cash-historical-v1/inputs/"
    "cfnai-monthly-2018-2024.csv"
)
CFNAI_BUNDLE = REPO_ROOT / (
    ".research-state/experiments/"
    "btc-fred-cfnai-above-trend-long-cash-historical-v1/inputs/"
    "cfnai-source-bundle.json"
)
...
PRIOR_SOURCE = REPO_ROOT / "research_pipeline/examples/btc-fred-cfnai-above-trend-long-cash-primary-prior.v1.json"
SPEC_SOURCE = REPO_ROOT / "research_pipeline/examples/btc-fred-cfnai-above-trend-source-feasibility.v1.spec.json"
PROBE_SOURCE = REPO_ROOT / "research/fred_cfnai_source_probe.py"
HYPOTHESIS_SOURCE = REPO_ROOT / "research_pipeline/examples/btc-fred-cfnai-above-trend-long-cash-v1.hypothesis.json"
EXPECTED_BINDINGS = {
    "normalized_cfnai": (
        CFNAI_SOURCE,
        "d547cfe99747168ce7b4ed08098fdde8b23c7da314de75452a89ebc404a8127f",
    ),
    "source_bundle": (
        CFNAI_BUNDLE,
        "a07e750784f1f0cad86e0bbed7e5b124cc9dcdd5a00a24a03d64088d5aba31ef",
    ),
    "economic_kernel": (
        KERNEL_SOURCE,
    ...
    "primary_prior": (
        PRIOR_SOURCE,
        "05047b9163a84dbbb82a9c1e9df87e6a5688f307c81e270181fe70b5cb2d89ca",
    ),
    "source_spec": (
        SPEC_SOURCE,
        "5eb5710709f2296e83804d418a2883129a0a34f66abf6885f876ae51b801b6c2",
    ),
    "source_probe": (
        PROBE_SOURCE,
        "508d575b5dbabd5aa6281118fc78f39be6472fecca04839dd506d269c5d0bd5b",
    ),
    "hypothesis": (
        HYPOTHESIS_SOURCE,
        "b4d65c603765bdb7ca298eae326e2a1456c9581c1232a051eed136b777b5a126",
    ),
}
...
GATES = (
    ...
    "cfnai_sha256_bundle_and_84_months_match",
    "hourly_lattice_monthly_continuity_and_60d_availability_pass",
    "frozen_runner_kernel_ledger_reference_parser_prior_hypothesis_probe_and_spec_sha256_match",
    "single_variant_strict_positive_cfnai_no_rescue_contract_pass",
    "design_signals_exactly_48_with_27_above_trend_and_21_other",
    "validation_signals_exactly_24_with_6_above_trend_and_18_other",
    "design_position_changes_between_6_and_30",
    "validation_position_changes_between_3_and_18",
...
def load_cfnai(path: Path) -> list[tuple[date, D]]:
    rows: list[tuple[date, D]] = []
    with path.open("r", encoding="utf-8", newline="") as stream:
        reader = csv.DictReader(stream)
        if reader.fieldnames != ["observation_date", "cfnai_present_vintage"]:
            raise ResearchReject(f"DATA_REJECT:CFNAI_COLUMNS:{reader.fieldnames}")
        for row in reader:
            try:
                day = date.fromisoformat(row["observation_date"])
                value = D(row["cfnai_present_vintage"])
            except (KeyError, ValueError, InvalidOperation) as error:
                raise ResearchReject("DATA_REJECT:CFNAI_PARSE") from error
            if day.day != 1 or not value.is_finite() or value < D("-20") or value > D("20"):
                raise ResearchReject(f"DATA_REJECT:CFNAI_VALUE:{day}")
            if rows and add_months(rows[-1][0], 1) != day:
                raise ResearchReject(f"DATA_REJECT:CFNAI_CONTINUITY:{rows[-1][0]}:{day}")
            rows.append((day, value))
    if len(rows) != 84 or rows[0][0] != date(2018, 1, 1) or rows[-1][0] != date(2024, 12, 1):
        raise ResearchReject("DATA_REJECT:CFNAI_INVENTORY")
    return rows
...
def targets_by_execution_time(
    ...
    targets = {
        datetime.combine(day, datetime.min.time()) + timedelta(days=60): value > ZERO
    ...
    return targets, {
        "formula": "CFNAI_MONTH_t",
        "threshold": "STRICTLY_GREATER_THAN_PUBLISHER_ZERO_TREND_THRESHOLD",
        "comparison": "EXACT_DECIMAL_WITHOUT_ROUNDING",
        "availability": "OBSERVATION_MONTH_START_PLUS_60_CALENDAR_DAYS_AT_0000_UTC",
        "evaluation_count": len(states),
        "above_trend_count": sum(states),
...
def evaluate(
    ...
    gates = {
        ...
        GATES[3]: True,
        GATES[4]: True,
        GATES[5]: dc["signal_evaluation_count"] == 48 and dc["long_target_count"] == 27 and dc["cash_target_count"] == 21,
        GATES[6]: vc["signal_evaluation_count"] == 24 and vc["long_target_count"] == 6 and vc["cash_target_count"] == 18,
        GATES[7]: D("6") <= dn["position_changes"] <= D("30"),
        GATES[8]: D("3") <= vn["position_changes"] <= D("18"),
...
def validate_manifest(manifest: dict[str, Any], runner: Path) -> None:
    if (
        ...
        or manifest.get("hypothesis_id") != "btc-fred-cfnai-above-trend-long-cash-v1"
    ):
    ...
    expected_policy = {
        "policy_id": "BTC_FRED_CFNAI_ABOVE_TREND_LONG_CASH_V1",
        "availability": "OBSERVATION_MONTH_START_PLUS_60_CALENDAR_DAYS_AT_0000_UTC",
        "formula": "CFNAI_MONTH_t",
        "long_condition": "CFNAI_STRICTLY_GREATER_THAN_ZERO",
...
def build_output(input_path: Path, manifest_path: Path) -> dict[str, Any]:
    ...
    kernel = load_module("cfnai_kernel", KERNEL_SOURCE)
    ledger = load_module("cfnai_ledger", LEDGER_SOURCE)
    reference = load_module("cfnai_reference", REFERENCE_SOURCE)
    parser = load_module("cfnai_parser", PARSER_SOURCE)
    ...
    cfnai = load_cfnai(CFNAI_SOURCE)
    targets, feature = targets_by_execution_time(cfnai)
    ...
    return {
        "schema_version": "1",
        "document_type": "BTC_FRED_CFNAI_ABOVE_TREND_LONG_CASH_HISTORICAL_RESULT_V1",
        "experiment_id": EXPERIMENT_ID,
        "authorization": "RESEARCH_ONLY_NOT_SHADOW_PAPER_OR_LIVE",
        "status": "CANDIDATE_FROZEN_REPORTED_NOT_ACTIVATED" if passed else "NO_CANDIDATE_CLOSE_BTC_FRED_CFNAI_ABOVE_TREND_LONG_CASH_FAMILY",
        "decision": "HISTORICAL_GATES_PASS_INDEPENDENT_OOS_REQUIRED" if passed else "PERMANENTLY_CLOSE_EXACT_LAGGED_CFNAI_ABOVE_TREND_LONG_CASH_FAMILY_WITHOUT_THRESHOLD_LAG_SMOOTHING_DIRECTION_OR_SIZING_TUNING",
        ...
        "oos_opened": False,
        "claim_boundary": "Historical present-vintage CFNAI replication only; a pass requires independent sealed OOS and never authorizes activation.",
