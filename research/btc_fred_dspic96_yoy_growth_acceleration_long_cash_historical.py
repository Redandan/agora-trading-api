#!/usr/bin/env python3
"""Deterministic matched-capital audit for lagged DSPIC96 YoY acceleration."""
...
EXPERIMENT_ID = "btc-fred-dspic96-yoy-growth-acceleration-long-cash-historical-v1"
EXPECTED_MANIFEST_TYPE = "BTC_FRED_DSPIC96_YOY_GROWTH_ACCELERATION_LONG_CASH_HISTORICAL_MANIFEST_V1"
EXPECTED_DATA_SHA256 = "e436a5a2b093365886464dd3e471cc5cd55c54bd2c06ceaa898ac218c45436dd"
EXPECTED_DATA_ROWS = 52_608
DSPIC96_SOURCE = REPO_ROOT / ".research-state/experiments/btc-fred-dspic96-yoy-growth-acceleration-long-cash-historical-v1/inputs/dspic96-monthly-2017-2024.csv"
DSPIC96_BUNDLE = REPO_ROOT / ".research-state/experiments/btc-fred-dspic96-yoy-growth-acceleration-long-cash-historical-v1/inputs/dspic96-source-bundle.json"
...
PRIOR_SOURCE = REPO_ROOT / "research_pipeline/examples/btc-fred-dspic96-yoy-growth-acceleration-long-cash-primary-prior.v1.json"
SPEC_SOURCE = REPO_ROOT / "research_pipeline/examples/btc-fred-dspic96-yoy-growth-acceleration-source-feasibility.v1.spec.json"
PROBE_SOURCE = REPO_ROOT / "research/fred_dspic96_source_probe.py"
HYPOTHESIS_SOURCE = REPO_ROOT / "research_pipeline/examples/btc-fred-dspic96-yoy-growth-acceleration-long-cash-v1.hypothesis.json"
EXPECTED_BINDINGS = {
    "normalized_dspic96": (DSPIC96_SOURCE, "ae6b4292ca56abe93dbc95ec96fbbe5e1a125ab903105b8ad11867150e778035"),
    "source_bundle": (DSPIC96_BUNDLE, "d6f4a9d9e9c47a63a639461f8d3989e9f029cfa022a87a39e7cf525ca5e35760"),
    ...
    "primary_prior": (PRIOR_SOURCE, "da92a3db1459056af553b4cbdd6b915901bc1294b65ca84fbd3f1c6a68f941cd"),
    "source_spec": (SPEC_SOURCE, "220e43518f125dc9389a93f8eeb0e983209b3797b1f77d62a28e6c9e066afd3a"),
    "source_probe": (PROBE_SOURCE, "ed8be72e5b8e6d61e0a4faa629f2366b6d5ab7d21e45e31809fb3a74ec350b43"),
    "hypothesis": (HYPOTHESIS_SOURCE, "24eb161a2ef834800e9298e10e43419bcd0e29f217f8a5d228ef4c6ab481fd94"),
}
...
GATES = (
    ...
    "dspic96_sha256_bundle_and_96_months_match",
    ...
    "validation_signals_exactly_24_with_13_accelerating_and_11_nonaccelerating",
...
def load_dspic96(path: Path) -> list[tuple[str, D]]:
    rows: list[tuple[str, D]] = []
    with path.open("r", encoding="utf-8", newline="") as stream:
        reader = csv.DictReader(stream)
        if reader.fieldnames != ["observation_date", "dspic96_billions_chained_2017_dollars_saar"]:
            raise ResearchReject(f"DATA_REJECT:DSPIC96_COLUMNS:{reader.fieldnames}")
        for row in reader:
            try:
                month = row["observation_date"][:7]
                value = D(row["dspic96_billions_chained_2017_dollars_saar"])
            except (KeyError, InvalidOperation) as error:
                raise ResearchReject("DATA_REJECT:DSPIC96_PARSE") from error
            if row["observation_date"] != f"{month}-01" or value <= ZERO or value > D("100000"):
                raise ResearchReject(f"DATA_REJECT:DSPIC96_VALUE:{month}")
            if rows and add_months(rows[-1][0], 1) != month:
                raise ResearchReject(f"DATA_REJECT:DSPIC96_CONTINUITY:{rows[-1][0]}:{month}")
            rows.append((month, value))
    if len(rows) != 96 or rows[0][0] != "2017-01" or rows[-1][0] != "2024-12":
        raise ResearchReject("DATA_REJECT:DSPIC96_INVENTORY")
    return rows
...
def targets_by_execution_time(rows: list[tuple[str, D]]) -> tuple[dict[datetime, bool], dict[str, Any]]:
    ...
    return targets, {
        "formula": "(DSPIC96_t/DSPIC96_t_minus_12-1)-(DSPIC96_t_minus_1/DSPIC96_t_minus_13-1)",
...
def evaluate(
    ...
    gates = {
        ...
        GATES[6]: vc["signal_evaluation_count"] == 24 and vc["long_target_count"] == 13 and vc["cash_target_count"] == 11,
...
def validate_manifest(manifest: dict[str, Any], runner: Path) -> None:
    ...
    expected_policy = {
        "policy_id": "BTC_FRED_DSPIC96_YOY_GROWTH_ACCELERATION_LONG_CASH_V1",
...
def build_output(input_path: Path, manifest_path: Path) -> dict[str, Any]:
    ...
    kernel = load_module("dspic96_kernel", KERNEL_SOURCE)
    ledger = load_module("dspic96_ledger", LEDGER_SOURCE)
    reference = load_module("dspic96_reference", REFERENCE_SOURCE)
    parser = load_module("dspic96_parser", PARSER_SOURCE)
    ...
    dspic96 = load_dspic96(DSPIC96_SOURCE)
    targets, feature = targets_by_execution_time(dspic96)
    ...
    return {
        "schema_version": "1",
        "document_type": "BTC_FRED_DSPIC96_YOY_GROWTH_ACCELERATION_LONG_CASH_HISTORICAL_RESULT_V1",
        "experiment_id": EXPERIMENT_ID,
        "authorization": "RESEARCH_ONLY_NOT_SHADOW_PAPER_OR_LIVE",
        "status": "CANDIDATE_FROZEN_REPORTED_NOT_ACTIVATED" if passed else "NO_CANDIDATE_CLOSE_BTC_FRED_DSPIC96_YOY_GROWTH_ACCELERATION_LONG_CASH_FAMILY",
        "decision": "HISTORICAL_GATES_PASS_INDEPENDENT_OOS_REQUIRED" if passed else "PERMANENTLY_CLOSE_EXACT_LAGGED_DSPIC96_YOY_GROWTH_ACCELERATION_LONG_CASH_FAMILY_WITHOUT_LEVEL_LAG_THRESHOLD_DIRECTION_OR_SIZING_TUNING",
        ...
        "oos_opened": False,
        "claim_boundary": "Historical present-vintage real disposable-income replication only; a pass requires independent sealed OOS and never authorizes activation.",
