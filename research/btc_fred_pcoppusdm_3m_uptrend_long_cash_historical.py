#!/usr/bin/env python3
"""Deterministic matched-capital audit for lagged PCOPPUSDM three-month direction."""
...
EXPERIMENT_ID = "btc-fred-pcoppusdm-3m-uptrend-long-cash-historical-v1"
EXPECTED_MANIFEST_TYPE = "BTC_FRED_PCOPPUSDM_3M_UPTREND_LONG_CASH_HISTORICAL_MANIFEST_V1"
EXPECTED_DATA_SHA256 = "e436a5a2b093365886464dd3e471cc5cd55c54bd2c06ceaa898ac218c45436dd"
EXPECTED_DATA_ROWS = 52_608
PCOPPUSDM_SOURCE = REPO_ROOT / (
    ".research-state/experiments/"
    "btc-fred-pcoppusdm-3m-uptrend-long-cash-historical-v1/inputs/"
    "pcoppusdm-monthly-2018-2024.csv"
)
PCOPPUSDM_BUNDLE = REPO_ROOT / (
    ".research-state/experiments/"
    "btc-fred-pcoppusdm-3m-uptrend-long-cash-historical-v1/inputs/"
    "pcoppusdm-source-bundle.json"
)
...
PRIOR_SOURCE = REPO_ROOT / "research_pipeline/examples/btc-fred-pcoppusdm-3m-uptrend-long-cash-primary-prior.v1.json"
SPEC_SOURCE = REPO_ROOT / "research_pipeline/examples/btc-fred-pcoppusdm-3m-uptrend-source-feasibility.v1.spec.json"
PROBE_SOURCE = REPO_ROOT / "research/fred_pcoppusdm_source_probe.py"
HYPOTHESIS_SOURCE = REPO_ROOT / "research_pipeline/examples/btc-fred-pcoppusdm-3m-uptrend-long-cash-v1.hypothesis.json"
EXPECTED_BINDINGS = {
    "normalized_pcoppusdm": (
        PCOPPUSDM_SOURCE,
        "b312cfa17bbfecb7a73c6adc29161609593f99f1c40fb14d44d2925d3265f9a1",
    ),
    "source_bundle": (
        PCOPPUSDM_BUNDLE,
        "a9914e5ae7be1a76c2a7805b509977732d07070297742599fad941c35e8d7f8f",
    ),
    "economic_kernel": (
        KERNEL_SOURCE,
    ...
    "primary_prior": (
        PRIOR_SOURCE,
        "8b8836768230460c119a1b58d134c9428e8e832bb8236e2c35de3deea80d87b9",
    ),
    "source_spec": (
        SPEC_SOURCE,
        "04e6575e163accf1ca5ec908bcc272929ddb028370dd944c01f5463b5b5649ef",
    ),
    "source_probe": (
        PROBE_SOURCE,
        "e5cf27546f07a89abe163960a7c5ee4772d3de4118a813470b746948cad7929c",
    ),
    "hypothesis": (
        HYPOTHESIS_SOURCE,
        "c43814541192073274a5c97b17b273d91fcf794215b102893dd593991ddc086f",
    ),
}
...
GATES = (
    ...
    "pcoppusdm_sha256_bundle_and_84_months_match",
    "hourly_lattice_monthly_continuity_and_75d_availability_pass",
    "frozen_runner_kernel_ledger_reference_parser_prior_hypothesis_probe_and_spec_sha256_match",
    "single_variant_strict_three_month_copper_uptrend_no_rescue_contract_pass",
    "design_signals_exactly_48_with_29_uptrend_and_19_other",
    "validation_signals_exactly_24_with_14_uptrend_and_10_other",
...
def load_pcoppusdm(path: Path) -> list[tuple[date, D]]:
    rows: list[tuple[date, D]] = []
    with path.open("r", encoding="utf-8", newline="") as stream:
        reader = csv.DictReader(stream)
        if reader.fieldnames != [
            "observation_date",
            "pcoppusdm_usd_per_metric_ton_period_average",
        ]:
            raise ResearchReject(f"DATA_REJECT:PCOPPUSDM_COLUMNS:{reader.fieldnames}")
        for row in reader:
            try:
                day = date.fromisoformat(row["observation_date"])
                value = D(row["pcoppusdm_usd_per_metric_ton_period_average"])
            except (KeyError, ValueError, InvalidOperation) as error:
                raise ResearchReject("DATA_REJECT:PCOPPUSDM_PARSE") from error
            if (
                day.day != 1
                ...
                raise ResearchReject(f"DATA_REJECT:PCOPPUSDM_VALUE:{day}")
            if rows and add_months(rows[-1][0], 1) != day:
                raise ResearchReject(
                    f"DATA_REJECT:PCOPPUSDM_CONTINUITY:{rows[-1][0]}:{day}"
                )
            rows.append((day, value))
    if (
        len(rows) != 84
        or rows[0][0] != date(2018, 1, 1)
        or rows[-1][0] != date(2024, 12, 1)
    ):
        raise ResearchReject("DATA_REJECT:PCOPPUSDM_INVENTORY")
    return rows
...
def targets_by_execution_time(
    ...
    for index in range(3, len(rows)):
        ...
        effective = datetime.combine(day, datetime.min.time()) + timedelta(days=75)
        targets[effective] = value > rows[index - 3][1]
    if len(targets) != 81:
        raise ResearchReject("FEATURE_REJECT:EVALUATION_COUNT")
    states = list(targets.values())
    return targets, {
        "formula": "PCOPPUSDM_MONTH_t-PCOPPUSDM_MONTH_t_minus_3",
        "threshold": "STRICTLY_GREATER_THAN_ZERO",
        "comparison": "EXACT_DECIMAL_WITHOUT_ROUNDING",
        "availability": "OBSERVATION_MONTH_START_PLUS_75_CALENDAR_DAYS_AT_0000_UTC",
...
def evaluate(
    ...
    upside_breadth = sum(
        item["NORMAL"]["upside_capture"] >= D("0.50") for item in raw.values()
    )
    ...
    gates = {
        ...
        GATES[3]: True,
        GATES[4]: True,
        GATES[5]: dc["signal_evaluation_count"] == 48
        and dc["long_target_count"] == 29
        and dc["cash_target_count"] == 19,
        GATES[6]: vc["signal_evaluation_count"] == 24
        and vc["long_target_count"] == 14
        and vc["cash_target_count"] == 10,
...
def validate_manifest(manifest: dict[str, Any], runner: Path) -> None:
    if (
        ...
        != "btc-fred-pcoppusdm-3m-uptrend-long-cash-v1"
    ):
    ...
    expected_policy = {
        "policy_id": "BTC_FRED_PCOPPUSDM_3M_UPTREND_LONG_CASH_V1",
        "availability": "OBSERVATION_MONTH_START_PLUS_75_CALENDAR_DAYS_AT_0000_UTC",
        "formula": "PCOPPUSDM_MONTH_t-PCOPPUSDM_MONTH_t_minus_3",
        "long_condition": "THREE_MONTH_CHANGE_STRICTLY_GREATER_THAN_ZERO",
...
def build_output(input_path: Path, manifest_path: Path) -> dict[str, Any]:
    ...
    kernel = load_module("pcoppusdm_kernel", KERNEL_SOURCE)
    ledger = load_module("pcoppusdm_ledger", LEDGER_SOURCE)
    reference = load_module("pcoppusdm_reference", REFERENCE_SOURCE)
    parser = load_module("pcoppusdm_parser", PARSER_SOURCE)
    ...
    pcoppusdm = load_pcoppusdm(PCOPPUSDM_SOURCE)
    targets, feature = targets_by_execution_time(pcoppusdm)
    ...
    return {
        "schema_version": "1",
        "document_type": "BTC_FRED_PCOPPUSDM_3M_UPTREND_LONG_CASH_HISTORICAL_RESULT_V1",
        "experiment_id": EXPERIMENT_ID,
        "authorization": "RESEARCH_ONLY_NOT_SHADOW_PAPER_OR_LIVE",
        "status": (
            "CANDIDATE_FROZEN_REPORTED_NOT_ACTIVATED"
            if passed
            else "NO_CANDIDATE_CLOSE_BTC_FRED_PCOPPUSDM_3M_UPTREND_LONG_CASH_FAMILY"
        ),
        "decision": (
            "HISTORICAL_GATES_PASS_INDEPENDENT_OOS_REQUIRED"
            if passed
            else "PERMANENTLY_CLOSE_EXACT_LAGGED_PCOPPUSDM_3M_UPTREND_LONG_CASH_FAMILY_WITHOUT_HORIZON_THRESHOLD_DIRECTION_OR_SIZING_TUNING"
        ),
        ...
        "oos_opened": False,
        "claim_boundary": "Historical present-vintage PCOPPUSDM replication only; a pass requires independent sealed OOS and never authorizes activation.",
