#!/usr/bin/env python3
"""Deterministic pre-economic screen for the frozen WTREGEN liquidity hypothesis."""
...
EXPECTED_TGA_SHA256 = "592d8c89716f62e7cc1b86558b3f5ee838ebc03f643648a8b72ade066f5666a4"
EXPECTED_TGA_ROWS = 365
EXPECTED_SOURCE_BUNDLE_SHA256 = "25afa9a3f660b6ee457bf5b751e13d4b6a986c77a0ea3101f1fee399e75c5e47"
EXPECTED_PRIOR_SHA256 = "053a6cc8b22f2053527c3b2b36adb1b30d8480168ca635acd88791a246737f56"
EXPECTED_HYPOTHESIS_SHA256 = "2468423d9b381d712084408c33a34162825560cced13810005400d843f15fb86"
EXPERIMENT_ID = "btc-fred-tga-drawdown-liquidity-support-historical-v1"
EXPECTED_MANIFEST_TYPE = "BTC_FRED_TGA_DRAWDOWN_LIQUIDITY_SUPPORT_HISTORICAL_MANIFEST_V1"

EXPECTED_SOURCE_PATHS = {
    "FROZEN_DIRECT_PRE_ECONOMIC_RUNNER": "research/btc_fred_tga_drawdown_liquidity_support_historical.py",
    "FROZEN_H1_PARSER_AND_DETERMINISTIC_STATISTICS": "research/btc_cftc_dealer_net_position_change_long_cash_historical.py",
    "FROZEN_FAIL_CLOSED_WTREGEN_SOURCE_PROBE": "research/fred_wtregen_source_probe.py",
    "SEALED_PRIMARY_AND_ADVERSARIAL_PRIOR": "research_pipeline/examples/btc-fred-tga-drawdown-liquidity-support-long-cash-primary-prior.v1.json",
    "FROZEN_SCHEMA_VALID_PRE_OUTCOME_HYPOTHESIS": "research_pipeline/examples/btc-fred-tga-drawdown-liquidity-support-long-cash-v1.hypothesis.json",
    "SEALED_OFFICIAL_WTREGEN_SOURCE_BUNDLE": ".research-state/experiments/btc-fred-tga-drawdown-liquidity-support-historical-v1/inputs/fred-wtregen-source-bundle.json",
    "SEALED_NORMALIZED_WTREGEN_CORPUS": ".research-state/experiments/btc-fred-tga-drawdown-liquidity-support-historical-v1/inputs/fred-wtregen-weekly-2018-2024.csv",
}

PREDICTIVE_GATE_NAMES = (
    "minimum_evaluable_episodes",
    "minimum_supportive_episodes",
    "minimum_drain_or_neutral_episodes",
...
EXPECTED_GATES = (
    ...
    "wtregen_sha256_rows_weekly_lattice_and_day2_availability_match",
...
def load_tga(path: Path) -> list[tuple[date, D]]:
    if not path.is_file() or sha256(path) != EXPECTED_TGA_SHA256:
        raise ResearchReject("DATA_REJECT:WTREGEN_SHA256")
    with path.open("r", encoding="utf-8", newline="") as stream:
        rows = list(csv.reader(stream))
    if not rows or rows[0] != ["observation_date", "wtregen_millions_usd"]:
        raise ResearchReject("DATA_REJECT:WTREGEN_HEADER")
    parsed: list[tuple[date, D]] = []
    for index, row in enumerate(rows[1:]):
        if len(row) != 2:
            raise ResearchReject(f"DATA_REJECT:WTREGEN_ROW:{index}")
        try:
            day = date.fromisoformat(row[0])
            value = D(row[1])
        except Exception as error:
            raise ResearchReject(f"DATA_REJECT:WTREGEN_PARSE:{index}") from error
        if value <= ZERO:
            raise ResearchReject(f"DATA_REJECT:WTREGEN_VALUE:{index}")
        parsed.append((day, value))
    if len(parsed) != EXPECTED_TGA_ROWS:
        raise ResearchReject(f"DATA_REJECT:WTREGEN_ROWS:{len(parsed)}")
    if parsed[0][0] != date(2018, 1, 3) or parsed[-1][0] != date(2024, 12, 25):
        raise ResearchReject("DATA_REJECT:WTREGEN_BOUNDARY")
    if len({day for day, _ in parsed}) != len(parsed):
        raise ResearchReject("DATA_REJECT:WTREGEN_DUPLICATE")
    for prior, current in zip(parsed, parsed[1:], strict=False):
        if current[0] - prior[0] != timedelta(days=7) or current[0].weekday() != 2:
            raise ResearchReject("DATA_REJECT:WTREGEN_WEEKLY_LATTICE")
    return parsed
...
def build_factor_points(rows: list[tuple[date, D]]) -> list[FactorPoint]:
    points = [
        FactorPoint(
            ...
            prior_four_week_value=rows[index - LOOKBACK_OBSERVATIONS][1],
        )
        for index in range(LOOKBACK_OBSERVATIONS, len(rows))
    ]
    if len(points) != EXPECTED_TGA_ROWS - LOOKBACK_OBSERVATIONS:
        raise ResearchReject("DATA_REJECT:WTREGEN_FACTOR_COUNT")
    return points
...
def predictive_evidence(
    ...
    *,
        ...
        episodes.append(
            {
                ...
                "state": "SUPPORTIVE" if point.supportive else "DRAIN_OR_NEUTRAL",
                "current_wtregen_millions_usd": shared.q(point.current_value),
                "prior_four_week_wtregen_millions_usd": shared.q(
    ...
    other = [item for item in episodes if item["state"] == "DRAIN_OR_NEUTRAL"]
    supportive_returns = _state_values(episodes, "SUPPORTIVE", "terminal_return_168h")
    other_returns = _state_values(episodes, "DRAIN_OR_NEUTRAL", "terminal_return_168h")
    supportive_dd = _state_values(
        episodes, "SUPPORTIVE", "peak_to_trough_max_drawdown_168h"
    )
    other_dd = _state_values(
        episodes, "DRAIN_OR_NEUTRAL", "peak_to_trough_max_drawdown_168h"
    )
    ...
    quartile_counts = {state: [0, 0, 0, 0] for state in ("SUPPORTIVE", "DRAIN_OR_NEUTRAL")}
    ...
    gates = {
        ...
        "minimum_drain_or_neutral_episodes": len(other) >= minimum_each,
    ...
    return {
        ...
        "statistics": {
            ...
            "drain_or_neutral_count": len(other),
            "supportive_quartile_counts": quartile_counts["SUPPORTIVE"],
            "drain_or_neutral_quartile_counts": quartile_counts["DRAIN_OR_NEUTRAL"],
            "supportive_anchor_years": sorted(year_counts["SUPPORTIVE"]),
            "drain_or_neutral_anchor_years": sorted(year_counts["DRAIN_OR_NEUTRAL"]),
            ...
            "drain_or_neutral_median_terminal_return_168h": shared.nullable(other_median_return),
            "supportive_negative_terminal_rate": shared.nullable(supportive_negative_rate),
            "drain_or_neutral_negative_terminal_rate": shared.nullable(other_negative_rate),
            ...
            "drain_or_neutral_median_path_drawdown_168h": shared.nullable(other_median_dd),
            "supportive_p75_path_drawdown_168h": shared.nullable(supportive_p75_dd),
            "drain_or_neutral_p75_path_drawdown_168h": shared.nullable(other_p75_dd),
...
def expected_policy() -> dict[str, Any]:
    return {
        "factor_identity": "FRED_WTREGEN_CURRENT_VALUE_STRICTLY_BELOW_VALUE_FOUR_ELIGIBLE_WEEKLY_OBSERVATIONS_EARLIER",
        "supportive_condition": "CURRENT_WTREGEN_STRICTLY_BELOW_VALUE_FOUR_WEEKLY_OBSERVATIONS_EARLIER",
...
def validate_manifest(manifest: dict[str, Any]) -> None:
    ...
    if manifest.get("datasets") != {
        ...
        "wtregen": {
            "path": ".research-state/experiments/btc-fred-tga-drawdown-liquidity-support-historical-v1/inputs/fred-wtregen-weekly-2018-2024.csv",
            "sha256": EXPECTED_TGA_SHA256,
            "rows": EXPECTED_TGA_ROWS,
            "first_date": "2018-01-03",
            "last_date": "2024-12-25",
            "present_vintage": True,
        },
        "source_bundle": {
            "path": ".research-state/experiments/btc-fred-tga-drawdown-liquidity-support-historical-v1/inputs/fred-wtregen-source-bundle.json",
...
def evaluate_gates(predictive: dict[str, dict[str, Any]]) -> tuple[dict[str, bool], list[str]]:
    gates: dict[str, bool] = {
        ...
        "wtregen_sha256_rows_weekly_lattice_and_day2_availability_match": True,
...
def build_output(btc_input: Path, tga_input: Path, manifest_path: Path) -> dict[str, Any]:
    ...
    tga_rows = load_tga(tga_input)
    points = build_factor_points(tga_rows)
    predictive = {
    ...
    return {
        "schema_version": "1",
        "document_type": "BTC_FRED_TGA_DRAWDOWN_LIQUIDITY_SUPPORT_HISTORICAL_RESULT_V1",
        "experiment_id": EXPERIMENT_ID,
        "authorization": "RESEARCH_ONLY_NOT_SHADOW_PAPER_OR_LIVE",
        "status": "PREDICTIVE_SCREEN_PASS_ECONOMIC_MANIFEST_REQUIRED_NO_CANDIDATE" if passed else "NO_CANDIDATE_CLOSE_BTC_FRED_TGA_DRAWDOWN_LIQUIDITY_SUPPORT_FAMILY_PRE_ECONOMIC",
        "decision": "FREEZE_SEPARATE_MATCHED_CAPITAL_ECONOMIC_EXPERIMENT_WITHOUT_OOS" if passed else "PERMANENTLY_CLOSE_EXACT_WTREGEN_FOUR_WEEK_DRAWDOWN_FAMILY_WITHOUT_ECONOMIC_ACCESS_OR_TUNING",
        ...
        "datasets": {
            ...
            "wtregen": {"path": tga_input.relative_to(REPO_ROOT).as_posix(), "sha256": sha256(tga_input), "rows": len(tga_rows)},
        ...
        "factor_inventory": {
            ...
            "drain_or_neutral_points": sum(not point.supportive for point in points),
        ...
        "claim_boundary": "Historical present-vintage WTREGEN and pre-2025 BTC evidence only. Federal Reserve balance-sheet mechanics do not establish independent BTC alpha, and this screen does not create a strategy candidate or authorize activation.",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--tga-input", type=Path, required=True)
...
def main() -> int:
    args = parse_args()
    inputs = [args.btc_input.resolve(), args.tga_input.resolve(), args.manifest.resolve()]
