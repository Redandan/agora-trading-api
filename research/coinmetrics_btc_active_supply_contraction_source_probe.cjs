...
const SOURCE_URL = "https://community-api.coinmetrics.io/v4/timeseries/asset-metrics?assets=btc&metrics=SplyActPct1yr&frequency=1d&start_time=2018-01-01&end_time=2024-12-31&page_size=10000&status=reviewed";
...
function parseRows(bytes) {
  ...
  const rows = payload.data.map((row, index) => {
    ...
    if (JSON.stringify(keys) !== JSON.stringify(["SplyActPct1yr", "asset", "time"])) {
    ...
    if (typeof row.SplyActPct1yr !== "string" || !decimal.test(row.SplyActPct1yr)) {
      throw new SourceReject(`SOURCE_REJECT:ACTIVE_SUPPLY_DECIMAL:${index}`);
    }
    const activeSupplyPct = Number(row.SplyActPct1yr);
    if (!Number.isFinite(activeSupplyPct) || activeSupplyPct <= 0 || activeSupplyPct > 100) {
      throw new SourceReject(`SOURCE_REJECT:ACTIVE_SUPPLY_RANGE:${index}`);
    }
    return {
      activeSupplyPct,
      date: row.time.slice(0, 10),
      rawActiveSupplyPct: row.SplyActPct1yr,
...
function featureFeasibility(rows) {
  ...
  const prefix = [0];
  for (const row of fullRows) prefix.push(prefix.at(-1) + row.activeSupplyPct);
  const states = [];
  let firstReportDate = null;
  for (let index = WINDOW_DAYS * 2 - 1; index < fullRows.length; index += 1) {
    if (new Date(fullRows[index].time).getUTCDay() !== 0) continue;
    const currentStart = index - WINDOW_DAYS + 1;
    const priorStart = currentStart - WINDOW_DAYS;
    const currentTotal = prefix[index + 1] - prefix[currentStart];
    const priorTotal = prefix[currentStart] - prefix[priorStart];
    states.push(currentTotal < priorTotal);
    if (firstReportDate === null) firstReportDate = fullRows[index].date;
  }
  if (!states.length || firstReportDate === null) throw new SourceReject("SOURCE_REJECT:NO_EVALUABLE_FEATURE");
  let transitions = 0;
  ...
  return {
    ...
    contraction_weeks: states.filter(Boolean).length,
    noncontraction_weeks: states.filter((value) => !value).length,
    transitions,
...
async function main() {
  ...
  try {
    response = await fetch(SOURCE_URL, {
      ...
      headers: {
        ...
        "User-Agent": "AgoraResearchCoinMetricsBtcActiveSupplyContractionSourceProbe/1.0",
      },
    });
  } finally {
  ...
  const normalizedBytes = Buffer.from([
    "date,active_supply_pct_1yr",
    ...rows.map((row) => `${row.date},${row.rawActiveSupplyPct}`),
  ].join("\n") + "\n", "utf8");
  const feasibility = featureFeasibility(rows);
  const bundle = {
    schema_version: "1",
    document_type: "COIN_METRICS_BTC_ACTIVE_SUPPLY_CONTRACTION_DAILY_SOURCE_BUNDLE_V1",
    ...
    normalized_subset: {
      ...
      columns: ["date", "active_supply_pct_1yr"],
    },
    pre_outcome_feature_feasibility: feasibility,
    publication_timing_boundary: "Use only a complete Sunday D after two adjacent 28-day windows. D becomes usable at D plus three calendar days 00:00 UTC and remains valid for at most 168 hours.",
