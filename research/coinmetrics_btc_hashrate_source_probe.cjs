...
const SOURCE_URL = "https://community-api.coinmetrics.io/v4/timeseries/asset-metrics?assets=btc&metrics=HashRate&frequency=1d&start_time=2018-01-01&end_time=2024-12-31&page_size=10000&status=reviewed";
...
function parseRows(bytes) {
  ...
  const rows = payload.data.map((row, index) => {
    ...
    if (JSON.stringify(keys) !== JSON.stringify(["HashRate", "asset", "time"])) {
    ...
    if (typeof row.HashRate !== "string" || !decimal.test(row.HashRate)) {
      throw new SourceReject(`SOURCE_REJECT:HASHRATE_DECIMAL:${index}`);
    }
    const hashRate = Number(row.HashRate);
    if (!Number.isFinite(hashRate) || hashRate <= 0 || hashRate > 1e15) {
      throw new SourceReject(`SOURCE_REJECT:HASHRATE_RANGE:${index}`);
    }
    return { date: row.time.slice(0, 10), time: Date.parse(row.time), hashRate, rawHashRate: row.HashRate };
  });
...
function featureFeasibility(rows) {
  ...
  const prefix = [0];
  for (const row of fullRows) prefix.push(prefix.at(-1) + row.hashRate);
  const states = [];
...
async function main() {
  ...
  try {
    response = await fetch(SOURCE_URL, {
      ...
      headers: { Accept: "application/json", "User-Agent": "AgoraResearchCoinMetricsHashRateSourceProbe/1.0" },
    });
  } finally {
  ...
  const normalizedBytes = Buffer.from([
    "date,hash_rate_th_per_s",
    ...rows.map((row) => `${row.date},${row.rawHashRate}`),
  ].join("\n") + "\n", "utf8");
  const feasibility = featureFeasibility(rows);
  const bundle = {
    schema_version: "1",
    document_type: "COIN_METRICS_BTC_HASHRATE_DAILY_SOURCE_BUNDLE_V1",
    ...
    normalized_subset: {
      ...
      columns: ["date", "hash_rate_th_per_s"],
    },
