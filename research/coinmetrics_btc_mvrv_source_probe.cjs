...
const SOURCE_URL = "https://community-api.coinmetrics.io/v4/timeseries/asset-metrics?assets=btc&metrics=CapMVRVCur&frequency=1d&start_time=2018-01-01&end_time=2024-12-31&page_size=10000&status=reviewed";
...
function parseRows(bytes) {
  ...
  const rows = payload.data.map((row, index) => {
    ...
    if (JSON.stringify(keys) !== JSON.stringify(["CapMVRVCur", "asset", "time"])) {
    ...
    if (typeof row.CapMVRVCur !== "string" || !decimal.test(row.CapMVRVCur)) {
      throw new SourceReject(`SOURCE_REJECT:MVRV_DECIMAL:${index}`);
    }
    const mvrv = Number(row.CapMVRVCur);
    if (!Number.isFinite(mvrv) || mvrv <= 0 || mvrv > 100) {
      throw new SourceReject(`SOURCE_REJECT:MVRV_RANGE:${index}`);
    }
    return { date: row.time.slice(0, 10), time: Date.parse(row.time), mvrv, rawMvrv: row.CapMVRVCur };
  });
  if (rows[0].date !== "2018-01-01" || rows.at(-1).date !== "2024-12-31") throw new SourceReject("SOURCE_REJECT:BOUNDARY");
...
function featureFeasibility(rows) {
  ...
  for (let index = LOOKBACK_DAYS; index < fullRows.length; index += 1) {
    ...
    const reference = median(fullRows.slice(index - LOOKBACK_DAYS, index).map((row) => row.mvrv));
    states.push(fullRows[index].mvrv < reference);
  ...
  return {
    ...
    below_prior_365d_median_weeks: states.filter(Boolean).length,
    at_or_above_prior_365d_median_weeks: states.filter((value) => !value).length,
    transitions,
...
function argumentsByName(argv) {
  ...
  for (const name of ["--bundle", "--raw", "--normalized"]) if (!values[name]) throw new SourceReject(`ARGUMENT_REJECT:${name}`);
  return values;
}
...
async function main() {
  ...
  try {
    response = await fetch(SOURCE_URL, {
      ...
      headers: { Accept: "application/json", "User-Agent": "AgoraResearchCoinMetricsMvrvSourceProbe/1.0" },
    });
  } finally {
  ...
  const normalizedBytes = Buffer.from(["date,mvrv", ...rows.map((row) => `${row.date},${row.rawMvrv}`)].join("\n") + "\n", "utf8");
  const feasibility = featureFeasibility(rows);
  const bundle = {
    schema_version: "1",
    document_type: "COIN_METRICS_BTC_MVRV_DAILY_SOURCE_BUNDLE_V1",
    ...
    request_contract: { method: "GET", url: SOURCE_URL, credentials: "DENY", redirect: "DENY", retry: "DENY", maximum_response_bytes: MAX_RESPONSE_BYTES, requested_status: "reviewed" },
    raw_response: { path: path.relative(REPO_ROOT, rawFile).replaceAll("\\", "/"), status: response.status, content_type: response.headers.get("content-type"), etag: response.headers.get("etag"), last_modified: response.headers.get("last-modified"), bytes: rawBytes.length, sha256: sha256(rawBytes), rows: rows.length },
    normalized_subset: { path: path.relative(REPO_ROOT, normalizedFile).replaceAll("\\", "/"), sha256: sha256(normalizedBytes), bytes: normalizedBytes.length, rows: rows.length, first_date: rows[0].date, last_date: rows.at(-1).date, columns: ["date", "mvrv"] },
  ...
  process.stdout.write(`${JSON.stringify({ status: bundle.status, bundle: path.relative(REPO_ROOT, bundleFile).replaceAll("\\", "/"), bundle_sha256: sha256(bundleBytes), raw: bundle.raw_response, normalized: bundle.normalized_subset, feasibility })}\n`);
}
