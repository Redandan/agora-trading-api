#!/usr/bin/env python3
"""Pre-economic duplicate and action support audit for DRA prior-day momentum."""
...
SPEC_TYPE = "DRA_NATIVE_PRIOR_DAY_MOMENTUM_PREOUTCOME_SPEC_V1"
RESULT_TYPE = "DRA_NATIVE_PRIOR_DAY_MOMENTUM_PREOUTCOME_RESULT_V1"
FAMILY_ID = "dra-native-prior-day-momentum-continuation-entry-admission"
RUNNER_IDENTITY = "DRA_NATIVE_PRIOR_DAY_MOMENTUM_SUPPORT_V1"
...
VARIANTS = (
    ("lower_threshold_neighbor", D("-0.25"), "prior-day-momentum-at-least-minus-0-25-v1"),
    ("primary", D("0.00"), "prior-day-momentum-at-least-0-00-v1"),
    ("upper_threshold_neighbor", D("0.25"), "prior-day-momentum-at-least-plus-0-25-v1"),
)
...
def load_spec(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(runner, dict) or runner.get("path") != "research/dra_native_prior_day_momentum_support_v1.py":
...
def prior_day_momentum_atr(close: D, day_open: D | None, atr14: D | None) -> D | None:
    if day_open is None or atr14 is None:
        return None
    if atr14 <= 0:
        raise SupportReject("DATA_REJECT:NONPOSITIVE_ATR14")
    return ((close - day_open) / atr14).quantize(QUANTUM, rounding=ROUND_HALF_UP)


def parent_momentum_atr(close: D, close_24h_ago: D, atr14: D | None) -> D | None:
    if atr14 is None:
        return None
    if atr14 <= 0:
        raise SupportReject("DATA_REJECT:NONPOSITIVE_ATR14")
    return ((close - close_24h_ago) / atr14).quantize(QUANTUM, rounding=ROUND_HALF_UP)


def actionable_veto(snapshot: dict[str, Any], threshold: D) -> bool:
    if not snapshot["capacity_admissible"]:
        return False
    value = snapshot["prior_day_momentum_atr"]
    return value is None or D(value) < threshold


class ParentSignalPriorDayMomentumObserver(capacity.EqualCapitalCapacityEngine):
    def __init__(self) -> None:
        super().__init__(
            slot_capacity_usdt=SLOT_CAPACITY_USDT,
            initial_equity_usdt=INITIAL_EQUITY_USDT,
        )
        self.current_day_open: D | None = None
    ...
    def _signal(self, bar: base.Bar) -> bool:
        ...
        candidate = prior_day_momentum_atr(close, self.current_day_open, self.atr14)
        parent = parent_momentum_atr(close, close_24h_ago, self.atr14)
        self.signal_snapshots.append(
            {
                "signal_time": bar.open_time.isoformat(),
                "signal_day_open_usdt": None if self.current_day_open is None else str(self.current_day_open),
                "signal_close_usdt": str(close),
                "close_24h_ago_usdt": str(close_24h_ago),
                "updated_atr14_usdt": None if self.atr14 is None else str(self.atr14),
                "prior_day_momentum_atr": None if candidate is None else str(candidate),
                "parent_momentum_atr": None if parent is None else str(parent),
...
def simulate_parent_signals(
    ...
    observer = ParentSignalPriorDayMomentumObserver()
    for bar in selected:
...
def summarize_window(
    ...
    available = [row for row in capacity_events if row["prior_day_momentum_atr"] is not None]
    values = [D(row["prior_day_momentum_atr"]) for row in available]
    ...
    sign_disagreements = sum(
        ...
        if D(row["parent_momentum_atr"]) > 0 and D(row["prior_day_momentum_atr"]) < 0
    )
    return {
        ...
        "parent_positive_momentum_count": sum(
            1 for row in available if D(row["parent_momentum_atr"]) > 0
        ),
        "candidate_parent_sign_disagreement_count": sign_disagreements,
        "prior_day_momentum_atr_summary": {
...
def build_result(spec: dict[str, Any], spec_path: Path, bars: list[base.Bar]) -> dict[str, Any]:
    ...
    primary_year_counts = {
        year: value["primary_actionable_veto_count"] for year, value in annual.items()
    }
    ...
    status = (
        ...
        if not failed
        else "NO_HYPOTHESIS_CLOSE_PRIOR_DAY_MOMENTUM_DUPLICATE_AT_SUPPORT_GATE"
    )
    return {
        "schema_version": "1",
        ...
        "scope_note": "Pre-economic causal duplicate and action support only; no PnL, OOS, canonical write, paid API, Production, Trading, database, order, fund, OCO, Grid, SHADOW, PAPER or LIVE action occurred.",
    }
