#!/usr/bin/env python3
"""Pre-economic support probe for signal-day occupancy above prior EMA20."""
...
SPEC_TYPE = "DRA_NATIVE_SIGNAL_DAY_EMA20_OCCUPANCY_PREOUTCOME_SPEC_V1"
RESULT_TYPE = "DRA_NATIVE_SIGNAL_DAY_EMA20_OCCUPANCY_PREOUTCOME_RESULT_V1"
FAMILY_ID = "dra-native-signal-day-ema20-occupancy-entry-admission"
RUNNER_IDENTITY = "DRA_NATIVE_SIGNAL_DAY_EMA20_OCCUPANCY_SUPPORT_V1"
...
SIGNAL_DAY_HOURS = 24
VARIANTS = (
    ("lower_threshold_neighbor", D("0.50"), "ema20-occupancy-at-least-12-of-24-v1"),
    ("primary", D("0.75"), "ema20-occupancy-at-least-18-of-24-v1"),
    ("upper_threshold_neighbor", D("1.00"), "ema20-occupancy-24-of-24-v1"),
)
...
def load_spec(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(runner, dict) or runner.get("path") != "research/dra_native_signal_day_ema20_occupancy_support_v1.py":
...
def ema20_occupancy(close_count: int, above_count: int, reference_ema20: D | None) -> D | None:
    if reference_ema20 is None or close_count != SIGNAL_DAY_HOURS:
        return None
    if above_count < 0 or above_count > close_count:
        raise SupportReject("DATA_REJECT:INVALID_OCCUPANCY_COUNT")
    return (D(above_count) / D(close_count)).quantize(QUANTUM, rounding=ROUND_HALF_UP)


def actionable_veto(snapshot: dict[str, Any], threshold: D) -> bool:
    if not snapshot["capacity_admissible"]:
        return False
    value = snapshot["ema20_occupancy_share"]
    return value is None or D(value) < threshold


def _expected_action_fingerprints() -> dict[str, str]:
    return {
        "new_family": "ACTION_DRA_ENTRY_ADMISSION|CLOCK_CAPACITY_ADMISSIBLE_DRA_SIGNAL|FEATURE_24_SIGNAL_DAY_H1_CLOSES_ABOVE_PRIOR_DAY_EMA20_SHARE_GTE_0_75",
        "prior_day_momentum": "ACTION_DRA_ENTRY_ADMISSION|CLOCK_CAPACITY_ADMISSIBLE_DRA_SIGNAL|FEATURE_SIGNAL_DAY_CLOSE_MINUS_SIGNAL_DAY_OPEN_DIV_UPDATED_ATR14_GTE_0",
        "intraday_sign_persistence": "ACTION_DRA_ENTRY_ADMISSION|CLOCK_COMPLETE_UTC_DAY|FEATURE_ADJACENT_H1_RETURN_SAME_SIGN_SHARE_TO_PRIOR_20D_MEDIAN_GTE_1",
        ...
        "late_day_activity": "ACTION_DRA_ENTRY_ADMISSION|CLOCK_COMPLETE_UTC_DAY|FEATURE_FINAL_6H_ABS_RETURN_SHARE_TO_PRIOR_20D_MEDIAN_GTE_1",
    }


class ParentSignalEma20OccupancyObserver(capacity.EqualCapitalCapacityEngine):
    def __init__(self) -> None:
        super().__init__(
            slot_capacity_usdt=SLOT_CAPACITY_USDT,
            initial_equity_usdt=INITIAL_EQUITY_USDT,
        )
        self.signal_day_reference_ema20: D | None = None
        self.signal_day_close_count = 0
        self.signal_day_above_count = 0
        self.signal_snapshots: list[dict[str, Any]] = []

    def _indicators(self, bar: base.Bar) -> None:
        if bar.open_time.hour == 0:
            self.signal_day_reference_ema20 = self.ema20
            self.signal_day_close_count = 0
            self.signal_day_above_count = 0
        self.signal_day_close_count += 1
        if self.signal_day_reference_ema20 is not None and bar.close > self.signal_day_reference_ema20:
            self.signal_day_above_count += 1
    ...
    def _signal(self, bar: base.Bar) -> bool:
        ...
        occupancy = ema20_occupancy(
            self.signal_day_close_count,
            self.signal_day_above_count,
            self.signal_day_reference_ema20,
        )
        self.signal_snapshots.append(
            {
                "signal_time": bar.open_time.isoformat(),
                "prior_complete_day_ema20_usdt": (
                    None if self.signal_day_reference_ema20 is None else str(self.signal_day_reference_ema20)
                ),
                "signal_day_complete_h1_close_count": self.signal_day_close_count,
                "signal_day_h1_closes_above_prior_ema20": self.signal_day_above_count,
                "ema20_occupancy_share": None if occupancy is None else str(occupancy),
...
def simulate_parent_signals(
    ...
    observer = ParentSignalEma20OccupancyObserver()
    for bar in selected:
...
def summarize_window(
    ...
    available = [row for row in capacity_events if row["ema20_occupancy_share"] is not None]
    values = [D(row["ema20_occupancy_share"]) for row in available]
    variants: dict[str, Any] = {}
    for role, threshold, variant_id in VARIANTS:
        veto_times = [row["signal_time"] for row in snapshots if actionable_veto(row, threshold)]
        variants[role] = {
...
def build_result(spec: dict[str, Any], spec_path: Path, bars: list[base.Bar]) -> dict[str, Any]:
    ...
    status = (
        ...
        if not failed
        else "NO_HYPOTHESIS_CLOSE_SIGNAL_DAY_EMA20_OCCUPANCY_AT_SUPPORT_GATE"
    )
    return {
        "schema_version": "1",
