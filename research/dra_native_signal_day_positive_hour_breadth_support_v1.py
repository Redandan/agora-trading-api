#!/usr/bin/env python3
"""Pre-economic support probe for signal-day positive-H1-return breadth."""
...
SPEC_TYPE = "DRA_NATIVE_SIGNAL_DAY_POSITIVE_HOUR_BREADTH_PREOUTCOME_SPEC_V1"
RESULT_TYPE = "DRA_NATIVE_SIGNAL_DAY_POSITIVE_HOUR_BREADTH_PREOUTCOME_RESULT_V1"
FAMILY_ID = "dra-native-signal-day-positive-hour-breadth-entry-admission"
RUNNER_IDENTITY = "DRA_NATIVE_SIGNAL_DAY_POSITIVE_HOUR_BREADTH_SUPPORT_V1"
...
ANNUAL = {str(year): (datetime(year, 1, 1), datetime(year + 1, 1, 1)) for year in range(2020, 2025)}
...
SIGNAL_DAY_HOURS = 24
VARIANTS = (
    ("lower_threshold_neighbor", 11, "positive-hour-breadth-at-least-11-of-24-v1"),
    ("primary", 13, "positive-hour-breadth-at-least-13-of-24-v1"),
    ("upper_threshold_neighbor", 15, "positive-hour-breadth-at-least-15-of-24-v1"),
)
...
def load_spec(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(runner, dict) or runner.get("path") != "research/dra_native_signal_day_positive_hour_breadth_support_v1.py":
...
def positive_hour_breadth(return_count: int, positive_count: int) -> tuple[int, D] | None:
    if return_count != SIGNAL_DAY_HOURS:
        return None
    if positive_count < 0 or positive_count > return_count:
        raise SupportReject("DATA_REJECT:INVALID_POSITIVE_HOUR_COUNT")
    share = (D(positive_count) / D(return_count)).quantize(QUANTUM, rounding=ROUND_HALF_UP)
    return positive_count, share


def actionable_veto(snapshot: dict[str, Any], minimum_positive_count: int) -> bool:
    if not snapshot["capacity_admissible"]:
        return False
    value = snapshot["positive_h1_return_count"]
    return value is None or value < minimum_positive_count


def _expected_action_fingerprints() -> dict[str, str]:
    return {
        "new_family": "ACTION_DRA_ENTRY_ADMISSION|CLOCK_CAPACITY_ADMISSIBLE_DRA_SIGNAL|FEATURE_24_SIGNAL_DAY_POSITIVE_H1_CLOSE_TO_PREVIOUS_CLOSE_RETURN_COUNT_GTE_13",
        "net_signal_day_momentum": "ACTION_DRA_ENTRY_ADMISSION|CLOCK_CAPACITY_ADMISSIBLE_DRA_SIGNAL|FEATURE_SIGNAL_DAY_CLOSE_MINUS_SIGNAL_DAY_OPEN_DIV_UPDATED_ATR14_GTE_0",
        "adjacent_sign_persistence": "ACTION_DRA_ENTRY_ADMISSION|CLOCK_COMPLETE_UTC_DAY|FEATURE_ADJACENT_H1_RETURN_SAME_SIGN_SHARE_TO_PRIOR_20D_MEDIAN_GTE_1",
...
class ParentSignalPositiveHourBreadthObserver(capacity.EqualCapitalCapacityEngine):
    def __init__(self) -> None:
        super().__init__(slot_capacity_usdt=SLOT_CAPACITY_USDT, initial_equity_usdt=INITIAL_EQUITY_USDT)
        self.previous_h1_close: D | None = None
        self.signal_day_return_count = 0
        self.signal_day_positive_count = 0
    ...
    def _indicators(self, bar: base.Bar) -> None:
        if bar.open_time.hour == 0:
            self.signal_day_return_count = 0
            self.signal_day_positive_count = 0
        if self.previous_h1_close is not None:
            self.signal_day_return_count += 1
            if bar.close > self.previous_h1_close:
                self.signal_day_positive_count += 1
        self.previous_h1_close = bar.close
    ...
    def _signal(self, bar: base.Bar) -> bool:
        ...
        breadth = positive_hour_breadth(self.signal_day_return_count, self.signal_day_positive_count)
        self.signal_snapshots.append({
            "signal_time": bar.open_time.isoformat(),
            "signal_day_complete_h1_return_count": self.signal_day_return_count,
            "positive_h1_return_count": None if breadth is None else breadth[0],
            "positive_h1_return_share": None if breadth is None else str(breadth[1]),
            "capacity_admissible": open_cost + LOT_COST_USDT <= self.cap,
        })
        return True


def simulate_parent_signals(bars: list[base.Bar], window: tuple[datetime, datetime]) -> tuple[list[dict[str, Any]], bool]:
    start, end = window
    ...
    parent = capacity.EqualCapitalCapacityEngine(slot_capacity_usdt=SLOT_CAPACITY_USDT, initial_equity_usdt=INITIAL_EQUITY_USDT)
    observer = ParentSignalPositiveHourBreadthObserver()
    for bar in selected:
    ...
    return observer.signal_snapshots, parent_result == observer_result
...
def _median(values: list[int]) -> str:
    ...
    value = D(ordered[midpoint]) if len(ordered) % 2 else D(ordered[midpoint - 1] + ordered[midpoint]) / D("2")
    return str(value)
...
def summarize_window(snapshots: list[dict[str, Any]], parent_path_parity: bool) -> dict[str, Any]:
    capacity_events = [row for row in snapshots if row["capacity_admissible"]]
    available = [row for row in capacity_events if row["positive_h1_return_count"] is not None]
    counts = [row["positive_h1_return_count"] for row in available]
    ...
    for role, minimum_count, variant_id in VARIANTS:
        ...
        variants[role] = {
            ...
            "minimum_positive_h1_returns": minimum_count,
            "threshold_share": str((D(minimum_count) / D(SIGNAL_DAY_HOURS)).quantize(QUANTUM, rounding=ROUND_HALF_UP)),
    ...
    return {
        ...
        "positive_hour_breadth_summary": {
            "minimum_positive_count": min(counts),
            "median_positive_count": _median(counts),
            "maximum_positive_count": max(counts),
        },
        "variants": variants,
    }
...
def build_result(spec: dict[str, Any], spec_path: Path, bars: list[base.Bar]) -> dict[str, Any]:
    ...
    for name, window in (("design", DESIGN), ("validation", VALIDATION)):
        snapshots, parity = simulate_parent_signals(bars, window)
        windows[name] = summarize_window(snapshots, parity)
    annual: dict[str, Any] = {}
    for year, window in ANNUAL.items():
        snapshots, parity = simulate_parent_signals(bars, window)
        summary = summarize_window(snapshots, parity)
        annual[year] = {
    ...
    year_counts = {year: value["primary_actionable_veto_count"] for year, value in annual.items()}
    total_year_vetoes = sum(year_counts.values())
    active_years = sum(count > 0 for count in year_counts.values())
    top_year_share = _share(max(year_counts.values()), total_year_vetoes) if total_year_vetoes else "1.00000000"
    validation_times = windows["validation"]["variants"]["primary"]["veto_signal_times"]
    month_counts = Counter(value[:7] for value in validation_times)
    top_month_share = _share(max(month_counts.values()), len(validation_times)) if validation_times else "1.00000000"

    gates = spec["gates"]
    failed: list[str] = []
    if not all(windows[name]["parent_path_parity"] for name in windows) or not all(value["parent_path_parity"] for value in annual.values()):
    ...
    if any(D(windows[name]["feature_available_share"]) < D(gates["minimum_feature_available_signal_share"]) for name in windows):
    ...
    if any(windows["validation"]["variants"][role]["actionable_veto_count"] < gates["validation_minimum_each_neighbor_actionable_vetoes"] for role in ("lower_threshold_neighbor", "upper_threshold_neighbor")):
    ...
    if any(0 < count < gates["minimum_primary_vetoes_per_active_annual_fold"] for count in year_counts.values()):
    ...
    if D(top_month_share) > D(gates["maximum_validation_top_month_primary_veto_share"]):
        failed.append("maximum_validation_top_month_primary_veto_share")
    status = "PASS_PREOUTCOME_DEDUP_SUPPORT_ALLOW_ONE_FROZEN_HYPOTHESIS" if not failed else "NO_HYPOTHESIS_CLOSE_SIGNAL_DAY_POSITIVE_HOUR_BREADTH_AT_SUPPORT_GATE"
    return {
        "schema_version": "1",
        ...
        "windows": windows,
        "annual_support": {
            "primary_actionable_vetoes": year_counts,
            "active_annual_folds": active_years,
            "top_year_primary_veto_share": top_year_share,
            "validation_top_month_primary_veto_share": top_month_share,
        },
...
def main() -> int:
    ...
    result = build_result(load_spec(spec_path), spec_path, load_bars(args.data.resolve()))
