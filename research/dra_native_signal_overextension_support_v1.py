#!/usr/bin/env python3
"""Pre-economic support probe for ATR-normalized native DRA signal margin."""
...
SPEC_TYPE = "DRA_NATIVE_SIGNAL_OVEREXTENSION_PREOUTCOME_SPEC_V1"
RESULT_TYPE = "DRA_NATIVE_SIGNAL_OVEREXTENSION_PREOUTCOME_RESULT_V1"
FAMILY_ID = "dra-native-signal-overextension-entry-admission"
RUNNER_IDENTITY = "DRA_NATIVE_SIGNAL_OVEREXTENSION_SUPPORT_V1"
...
VARIANTS = (
    ("lower_threshold_neighbor", D("0.75"), "signal-margin-at-most-0-75-v1"),
    ("primary", D("1.00"), "signal-margin-at-most-1-00-v1"),
    ("upper_threshold_neighbor", D("1.25"), "signal-margin-at-most-1-25-v1"),
)
...
CLOSED_LATE_DAY_ACTIVITY_FINGERPRINT = (
    "ACTION_DRA_ENTRY_ADMISSION|CLOCK_DRA_SIGNAL|"
    "FEATURE_FINAL_SIX_UTC_HOUR_ABSOLUTE_LOG_RETURN_SHARE_TO_PRIOR_20D_MEDIAN"
)
CLOSED_INTRADAY_DRAWDOWN_FINGERPRINT = (
    "ACTION_DRA_ENTRY_ADMISSION|CLOCK_DRA_SIGNAL|"
    "FEATURE_COMPLETE_DAY_H1_CLOSE_PATH_MAX_DRAWDOWN_TO_PRIOR_20D_MEDIAN"
)
...
def load_spec(path: Path) -> dict[str, Any]:
    ...
    if value.get("action_fingerprints") != {
        ...
        "closed_late_day_activity": CLOSED_LATE_DAY_ACTIVITY_FINGERPRINT,
        "closed_intraday_drawdown": CLOSED_INTRADAY_DRAWDOWN_FINGERPRINT,
    }:
    ...
    if value.get("feature_policy") != {
        ...
        "formula": "SIGNAL_BAR_CLOSE_MINUS_CAUSALLY_UPDATED_EMA20_DIVIDED_BY_CAUSALLY_UPDATED_ATR14",
        "relation": "ADMIT_AT_OR_BELOW_THRESHOLD_VETO_ABOVE_THRESHOLD",
        "primary_threshold": "1.00",
        "rejection_only_neighbors": ["0.75", "1.25"],
...
def signal_margin(close: D, ema20: D | None, atr14: D | None) -> D | None:
    if ema20 is None or atr14 is None:
        return None
    if atr14 <= 0:
        raise SupportReject("DATA_REJECT:NONPOSITIVE_ATR14")
    return ((close - ema20) / atr14).quantize(QUANTUM, rounding=ROUND_HALF_UP)


def actionable_veto(snapshot: dict[str, Any], threshold: D) -> bool:
    if not snapshot["capacity_admissible"]:
        return False
    value = snapshot["signal_margin_atr"]
    return value is None or D(value) > threshold


class ParentSignalMarginObserver(capacity.EqualCapitalCapacityEngine):
    ...
    def _entry_lifecycle(self, bar: base.Bar) -> None:
        ...
        if self.armed_at is not None and bar.open_time > self.armed_at and self._signal(bar):
            ...
            margin = signal_margin(bar.close, self.ema20, self.atr14)
            self.signal_snapshots.append(
                {
                    "signal_time": bar.open_time.isoformat(),
                    "signal_close_usdt": str(bar.close),
                    "updated_ema20_usdt": None if self.ema20 is None else str(self.ema20),
                    "updated_atr14_usdt": None if self.atr14 is None else str(self.atr14),
                    "signal_margin_atr": None if margin is None else str(margin),
...
def simulate_parent_signals(
    ...
    engine = ParentSignalMarginObserver()
    for bar in selected:
...
def _median(values: list[D]) -> D:
    ...
    if not ordered:
        raise SupportReject("DATA_REJECT:EMPTY_MARGIN_VALUES")
...
def summarize_window(
    ...
    available = [row for row in snapshots if row["signal_margin_atr"] is not None]
    margins = [D(row["signal_margin_atr"]) for row in available]
        ...
        action_rows = [
            {
                ...
                "signal_margin_atr": row["signal_margin_atr"],
            }
            for row in actions
        ]
        variants[role] = {
            "variant_id": variant_id,
            "admit_at_or_below_threshold": str(threshold),
    ...
    return {
        ...
        "signal_margin_atr_summary": {
            "minimum": str(min(margins)),
            "median": str(_median(margins).quantize(QUANTUM, rounding=ROUND_HALF_UP)),
            "maximum": str(max(margins)),
        },
...
def analyze(bars: list[base.Bar], spec: dict[str, Any]) -> dict[str, Any]:
    ...
    gate_results = {
        ...
        "action_fingerprints_distinct_from_closed_families": len(
            {NEW_ACTION_FINGERPRINT, CLOSED_LONG_TREND_FINGERPRINT, CLOSED_LATE_DAY_ACTIVITY_FINGERPRINT, CLOSED_INTRADAY_DRAWDOWN_FINGERPRINT}
        ) == 4,
    ...
    return {
        ...
        "family_id": FAMILY_ID,
        "status": "PASS_PREOUTCOME_DEDUP_SUPPORT_ALLOW_ONE_FROZEN_HYPOTHESIS" if not failed else "NO_HYPOTHESIS_CLOSE_NATIVE_SIGNAL_OVEREXTENSION_AT_SUPPORT_GATE",
        "runner_identity": RUNNER_IDENTITY,
        "runner_sha256": sha256(Path(__file__).resolve()),
        "dataset": {"canonical_sha256": base.data_hash(bars), "hourly_rows": len(bars), "selection_cutoff": spec["dataset"]["selection_cutoff"]},
