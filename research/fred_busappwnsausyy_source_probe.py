#!/usr/bin/env python3
"""Seal official present-vintage FRED BUSAPPWNSAUSYY without BTC outcomes."""
...
SPEC_PATH = REPO_ROOT / "research_pipeline/examples/btc-fred-business-applications-yoy-growth-source-feasibility.v1.spec.json"
PRIOR_PATH = REPO_ROOT / "research_pipeline/examples/btc-fred-business-applications-yoy-growth-long-cash-primary-prior.v1.json"
YEARS = tuple(range(2017, 2025))
SOURCE_URLS = {
    year: f"https://fred.stlouisfed.org/graph/fredgraph.csv?id=BUSAPPWNSAUSYY&cosd={year}-01-01&coed={year}-12-31"
    for year in YEARS
}
...
def load_and_validate_spec() -> dict[str, Any]:
    ...
    if spec.get("status") != "FROZEN_BEFORE_2017_2024_BUSAPPWNSAUSYY_FACTOR_ACCESS":
        raise SourceReject("SPEC_REJECT:STATUS")
    if spec.get("family_id") != "btc-fred-business-applications-yoy-growth-long-cash":
        raise SourceReject("SPEC_REJECT:FAMILY")
    source = spec.get("source_contract", {})
    expected_source = {
        "series": "BUSAPPWNSAUSYY",
        "years": list(YEARS),
    ...
    if feature.get("publication_lag_calendar_days") != PUBLICATION_LAG_DAYS:
        raise SourceReject("SPEC_REJECT:PUBLICATION_LAG")
...
def fetch_part(year: int) -> bytes:
    request = Request(
        ...
        headers={
            "Accept": "text/csv",
            "User-Agent": "AgoraResearchFredBusappwnsausyySourceProbe/1.0",
        },
    )
    try:
...
def parse_annual(raw: bytes, expected_year: int) -> list[tuple[date, Decimal]]:
    try:
    ...
    if not rows or rows[0] != ["observation_date", "BUSAPPWNSAUSYY"]:
    ...
    for index, row in enumerate(rows[1:]):
        ...
        if not value.is_finite() or value < Decimal("-1000") or value > Decimal("1000"):
...
def feature_feasibility(weekly: list[tuple[date, Decimal]]) -> dict[str, Any]:
    ...
    return {
        ...
        "zero_value_weeks": sum(value == Decimal("0") for _, value in weekly),
        "transitions": sum(
            current[1] != prior[1]
            for prior, current in zip(states, states[1:], strict=False)
        ),
        "first_evaluable_week": weekly[0][0].isoformat(),
    ...
    with zipfile.ZipFile(output, "w", compression=zipfile.ZIP_STORED) as archive:
        for year, raw in parts:
            info = zipfile.ZipInfo(
                f"fred-busappwnsausyy-{year}.csv", date_time=(1980, 1, 1, 0, 0, 0)
            )
...
def main() -> int:
    ...
    normalized = (
        "week_ending_saturday,business_applications_yoy_pct\n"
        + "".join(
    ...
    bundle = {
        "schema_version": "1",
        "document_type": "FRED_BUSAPPWNSAUSYY_WEEKLY_SOURCE_BUNDLE_V1",
        ...
        "normalized_weekly_subset": {
            ...
            "columns": ["week_ending_saturday", "business_applications_yoy_pct"],
        },
        "pre_outcome_feature_feasibility": feasibility,
        "publication_timing_boundary": "Week ending Saturday D is usable only at D plus 45 calendar days 00:00 UTC and remains valid for at most 168 hours.",
        "measurement_boundary": "BUSAPPWNSAUSYY is not seasonally adjusted and measures application intent, not realized business formation, employment, revenue or investment.",
        "revision_boundary": "The exact annual FRED responses are a sealed present-vintage snapshot. Original releases, revision vintages and exact historical publication timestamps remain MISSING_PROOF.",
