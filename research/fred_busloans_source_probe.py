#!/usr/bin/env python3
"""Seal official present-vintage FRED BUSLOANS history without BTC outcomes."""
...
SPEC_PATH = REPO_ROOT / "research_pipeline/examples/btc-fred-busloans-yoy-growth-acceleration-source-feasibility.v1.spec.json"
PRIOR_PATH = REPO_ROOT / "research_pipeline/examples/btc-fred-busloans-yoy-growth-acceleration-long-cash-primary-prior.v1.json"
SOURCE_URL = "https://fred.stlouisfed.org/graph/fredgraph.csv?id=BUSLOANS&cosd=2017-01-01&coed=2024-12-31"
EXPECTED_ROWS = 96
...
SUPPORT_GATES = {
    "design": {"minimum_evaluations": 48, "minimum_per_state": 10, "minimum_transitions": 4},
    "validation": {"minimum_evaluations": 24, "minimum_per_state": 6, "minimum_transitions": 3},
}
...
def load_and_validate_spec() -> dict[str, Any]:
    ...
    if spec.get("status") != "FROZEN_BEFORE_2017_2024_BUSLOANS_FACTOR_ACCESS":
        raise SourceReject("SPEC_REJECT:STATUS")
    family_id = "btc-fred-busloans-yoy-growth-acceleration-long-cash"
    ...
    if (
        ...
        or feature.get("availability") != "FIRST_DAY_OF_SECOND_FOLLOWING_CALENDAR_MONTH_AT_00_00_UTC"
        or feature.get("formula")
        != "(BUSLOANS_t/BUSLOANS_t_minus_12-1)-(BUSLOANS_t_minus_1/BUSLOANS_t_minus_13-1)"
        or feature.get("accelerating_condition") != "ADJACENT_YOY_GROWTH_CHANGE_STRICTLY_GREATER_THAN_ZERO"
    ):
...
def fetch() -> bytes:
    request = Request(
        SOURCE_URL,
        method="GET",
        headers={"Accept": "text/csv", "User-Agent": "AgoraResearchFredBusloansSourceProbe/1.0"},
    )
    try:
...
def parse_rows(raw: bytes) -> list[tuple[date, Decimal, str]]:
    ...
    if not rows or rows[0] != ["observation_date", "BUSLOANS"]:
    ...
    for index, row in enumerate(rows[1:]):
        ...
        if day.day != 1 or not value.is_finite() or value <= 0 or value > 100_000:
...
def _summarize(states: list[tuple[datetime, bool]], start: datetime, end: datetime, gate: dict[str, int]) -> dict[str, Any]:
    ...
    accelerating = sum(flags)
    nonaccelerating = len(flags) - accelerating
    ...
    for effective, state in selected:
        counts = annual.setdefault(str(effective.year), {"accelerating": 0, "nonaccelerating": 0})
        counts["accelerating" if state else "nonaccelerating"] += 1
    passed = (
        len(selected) >= gate["minimum_evaluations"]
        and accelerating >= gate["minimum_per_state"]
        and nonaccelerating >= gate["minimum_per_state"]
        and transitions >= gate["minimum_transitions"]
    )
    return {
        "evaluations": len(selected),
        "accelerating_months": accelerating,
        "nonaccelerating_months": nonaccelerating,
        "transitions": transitions,
        "longest_accelerating_run_months": _longest_run(flags, True),
        "longest_nonaccelerating_run_months": _longest_run(flags, False),
...
def feature_feasibility(rows: list[tuple[date, Decimal, str]]) -> dict[str, Any]:
    ...
    return {
        ...
        "accelerating_months": sum(state[1] for state in states),
        "nonaccelerating_months": sum(not state[1] for state in states),
        ...
        "admission_status": (
            "PASS_MONTHLY_COVERAGE_BOTH_BUSLOANS_ACCELERATION_STATE_SUPPORT_AND_TRANSITIONS_BEFORE_BTC_OUTCOME_ACCESS"
            if passed
            else "DATA_REJECT_INADEQUATE_BUSLOANS_ACCELERATION_STATE_SUPPORT_OR_TRANSITIONS_BEFORE_BTC_OUTCOME_ACCESS"
        ),
    }
...
def main() -> int:
    ...
    validate_rows(rows)
    normalized = (
        "observation_date,busloans_billions_usd_seasonally_adjusted\n"
    ...
    bundle = {
        "schema_version": "1",
        "document_type": "FRED_BUSLOANS_MONTHLY_SOURCE_BUNDLE_V1",
        ...
        "normalized_subset": {
            ...
            "columns": ["observation_date", "busloans_billions_usd_seasonally_adjusted"],
        },
        ...
        "revision_boundary": "The exact FRED CSV is sealed present-vintage seasonally adjusted history. Original releases, weekly aggregation, benchmark revisions, mergers, seasonal factors and vintage values remain MISSING_PROOF.",
        "interpretation_boundary": "Acceleration in C&I loan growth can reflect credit supply, borrower demand or distressed credit-line drawdowns; it is not proof of investable liquidity or independent Bitcoin causality.",
        "license_boundary": "Federal Reserve public data are attributed through FRED; exact sealed bytes are retained as internal reproducibility evidence.",
