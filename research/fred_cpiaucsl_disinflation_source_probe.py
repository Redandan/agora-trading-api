#!/usr/bin/env python3
"""Seal official present-vintage FRED CPIAUCSL history without BTC outcomes."""
...
SPEC_PATH = REPO_ROOT / "research_pipeline/examples/btc-fred-cpiaucsl-yoy-disinflation-source-feasibility.v1.spec.json"
PRIOR_PATH = REPO_ROOT / "research_pipeline/examples/btc-fred-cpiaucsl-yoy-disinflation-long-cash-primary-prior.v1.json"
SOURCE_URL = "https://fred.stlouisfed.org/graph/fredgraph.csv?id=CPIAUCSL&cosd=2017-01-01&coed=2024-12-31"
EXPECTED_ROWS = 96
...
SUPPORT_GATES = {
    "design": {"minimum_evaluations": 48, "minimum_per_state": 10, "minimum_transitions": 6},
    "validation": {"minimum_evaluations": 24, "minimum_per_state": 6, "minimum_transitions": 4},
}
...
def load_and_validate_spec() -> dict[str, Any]:
    ...
    if spec.get("status") != "FROZEN_BEFORE_2017_2024_CPIAUCSL_FACTOR_ACCESS":
        raise SourceReject("SPEC_REJECT:STATUS")
    if spec.get("family_id") != "btc-fred-cpiaucsl-yoy-disinflation-long-cash" or prior.get("family_id") != spec.get("family_id"):
        raise SourceReject("SPEC_REJECT:FAMILY")
    source = spec.get("source_contract", {})
    expected = {
        "series": "CPIAUCSL", "frequency": "MONTHLY", "request_url": SOURCE_URL,
...
def fetch() -> bytes:
    request = Request(SOURCE_URL, method="GET", headers={"Accept": "text/csv", "User-Agent": "AgoraResearchFredCpiaucslSourceProbe/1.0"})
    try:
...
def parse_rows(raw: bytes) -> list[tuple[date, Decimal, str]]:
    ...
    if not rows or rows[0] != ["observation_date", "CPIAUCSL"]:
    ...
    for index, row in enumerate(rows[1:]):
        ...
        try:
            day = date.fromisoformat(row[0]); value = Decimal(row[1])
        except (ValueError, InvalidOperation) as error:
            raise SourceReject(f"SOURCE_REJECT:VALUE_OR_DATE:{index}") from error
        if day.day != 1 or not value.is_finite() or value <= 0 or value > 1000:
...
def _summarize(states: list[tuple[datetime, bool]], start: datetime, end: datetime, gate: dict[str, int]) -> dict[str, Any]:
    ...
    disinflation = sum(flags); other = len(flags) - disinflation
    ...
    for effective, state in selected:
        counts = annual.setdefault(str(effective.year), {"disinflation": 0, "other": 0})
        counts["disinflation" if state else "other"] += 1
    passed = len(selected) >= gate["minimum_evaluations"] and disinflation >= gate["minimum_per_state"] and other >= gate["minimum_per_state"] and transitions >= gate["minimum_transitions"]
    return {
        "evaluations": len(selected), "disinflation_months": disinflation, "other_months": other,
        "transitions": transitions, "longest_disinflation_run_months": _longest_run(flags, True),
        "longest_other_run_months": _longest_run(flags, False), "annual_state_counts": annual,
...
def feature_feasibility(rows: list[tuple[date, Decimal, str]]) -> dict[str, Any]:
    ...
    for index in range(LOOKBACK, len(rows)):
        ...
        current_yoy = current / rows[index - 12][1] - Decimal(1)
        prior_yoy = rows[index - 1][1] / rows[index - 13][1] - Decimal(1)
        change = current_yoy - prior_yoy
        changes.append(change)
        states.append((datetime.combine(add_months(day, 2), time.min, tzinfo=timezone.utc), change < 0))
    ...
    return {
        ...
        "disinflation_months": sum(state[1] for state in states), "other_months": sum(not state[1] for state in states),
        "transitions": sum(current[1] != prior[1] for prior, current in zip(states, states[1:], strict=False)),
        "minimum_adjacent_yoy_change": str(min(changes)), "maximum_adjacent_yoy_change": str(max(changes)),
        "first_effective_time": states[0][0].isoformat().replace("+00:00", "Z"), "last_effective_time": states[-1][0].isoformat().replace("+00:00", "Z"),
        "design": design, "validation": validation,
        "admission_status": "PASS_MONTHLY_COVERAGE_BOTH_DISINFLATION_STATE_SUPPORT_AND_TRANSITIONS_BEFORE_BTC_OUTCOME_ACCESS" if passed else "DATA_REJECT_INADEQUATE_DISINFLATION_STATE_SUPPORT_OR_TRANSITIONS_BEFORE_BTC_OUTCOME_ACCESS",
    }
...
def main() -> int:
    ...
    bundle = {
        "schema_version": "1", "document_type": "FRED_CPIAUCSL_MONTHLY_SOURCE_BUNDLE_V1",
        "authorization": "RESEARCH_ONLY_NOT_SHADOW_PAPER_OR_LIVE", "status": status,
        "captured_at": datetime.now(timezone.utc).isoformat().replace("+00:00", "Z"),
        "publisher": "U.S. Bureau of Labor Statistics via Federal Reserve Bank of St. Louis FRED", "series": "CPIAUCSL",
        ...
        "normalized_subset": {"path": normalized_path.relative_to(REPO_ROOT).as_posix(), "bytes": len(normalized), "sha256": sha256(normalized), "rows": len(rows), "first_date": rows[0][0].isoformat(), "last_date": rows[-1][0].isoformat(), "columns": ["observation_date", "cpiaucsl_index_1982_84_100"]},
        "pre_outcome_feature_feasibility": feasibility,
        "publication_timing_boundary": "Each observation is conservatively usable only on the first day of the second following calendar month at 00:00 UTC and remains valid for one calendar month.",
        "revision_boundary": "The exact FRED CSV is sealed present-vintage seasonally adjusted history. BLS can revise the previous five years of seasonal factors; original releases and vintage values remain MISSING_PROOF.",
        "interpretation_boundary": "A declining year-over-year CPI rate is disinflation, not deflation, monetary easing or independent Bitcoin causality.",
        "license_boundary": "BLS public data are attributed through FRED; exact sealed bytes are retained as internal reproducibility evidence.",
