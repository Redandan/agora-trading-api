#!/usr/bin/env python3
"""Seal official present-vintage FRED DSPIC96 history without BTC outcomes."""
...
SPEC_PATH = REPO_ROOT / "research_pipeline/examples/btc-fred-dspic96-yoy-growth-acceleration-source-feasibility.v1.spec.json"
PRIOR_PATH = REPO_ROOT / "research_pipeline/examples/btc-fred-dspic96-yoy-growth-acceleration-long-cash-primary-prior.v1.json"
SOURCE_URL = "https://fred.stlouisfed.org/graph/fredgraph.csv?id=DSPIC96&cosd=2017-01-01&coed=2024-12-31"
EXPECTED_ROWS = 96
...
def load_and_validate_spec() -> dict[str, Any]:
    ...
    if spec.get("status") != "FROZEN_BEFORE_2017_2024_DSPIC96_FACTOR_ACCESS":
        raise SourceReject("SPEC_REJECT:STATUS")
    family_id = "btc-fred-dspic96-yoy-growth-acceleration-long-cash"
    ...
    if (
        ...
        != "(DSPIC96_t/DSPIC96_t_minus_12-1)-(DSPIC96_t_minus_1/DSPIC96_t_minus_13-1)"
...
def fetch() -> bytes:
    request = Request(
        SOURCE_URL,
        method="GET",
        headers={"Accept": "text/csv", "User-Agent": "AgoraResearchFredDspic96SourceProbe/1.0"},
    )
    try:
...
def parse_rows(raw: bytes) -> list[tuple[date, Decimal, str]]:
    ...
    if not rows or rows[0] != ["observation_date", "DSPIC96"]:
...
def feature_feasibility(rows: list[tuple[date, Decimal, str]]) -> dict[str, Any]:
    ...
    return {
        ...
        "admission_status": (
            "PASS_MONTHLY_COVERAGE_BOTH_DSPIC96_ACCELERATION_STATE_SUPPORT_AND_TRANSITIONS_BEFORE_BTC_OUTCOME_ACCESS"
            if passed
            else "DATA_REJECT_INADEQUATE_DSPIC96_ACCELERATION_STATE_SUPPORT_OR_TRANSITIONS_BEFORE_BTC_OUTCOME_ACCESS"
        ),
    }
...
def main() -> int:
    ...
    validate_rows(rows)
    normalized = (
        "observation_date,dspic96_billions_chained_2017_dollars_saar\n"
    ...
    bundle = {
        "schema_version": "1",
        "document_type": "FRED_DSPIC96_MONTHLY_SOURCE_BUNDLE_V1",
        "authorization": "RESEARCH_ONLY_NOT_SHADOW_PAPER_OR_LIVE",
        "status": status,
        "captured_at": datetime.now(timezone.utc).isoformat().replace("+00:00", "Z"),
        "publisher": "U.S. Bureau of Economic Analysis via Federal Reserve Bank of St. Louis FRED",
        "series": "DSPIC96",
        "request_contract": {
            "method": "GET",
            "url": SOURCE_URL,
        ...
        "normalized_subset": {
            ...
            "columns": ["observation_date", "dspic96_billions_chained_2017_dollars_saar"],
        },
        ...
        "revision_boundary": "The exact FRED CSV is sealed present-vintage seasonally adjusted history. Original releases, annual revisions, seasonal factors and vintage values remain MISSING_PROOF.",
        "interpretation_boundary": "Acceleration in real disposable-income year-over-year growth is a household-resource proxy, not proof of investable liquidity, allocation to Bitcoin or independent causality.",
        "license_boundary": "BEA data are attributed through FRED; exact sealed bytes are retained as internal reproducibility evidence subject to the source terms.",
