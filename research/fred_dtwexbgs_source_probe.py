#!/usr/bin/env python3
"""Validate and seal present-vintage FRED DTWEXBGS annual responses without opening BTC outcomes."""
...
SOURCE_URLS = {
    year: f"https://fred.stlouisfed.org/graph/fredgraph.csv?id=DTWEXBGS&cosd={year}-01-01&coed={year}-12-31"
    for year in YEARS
}
...
def parse_annual(raw: bytes, expected_year: int) -> tuple[list[tuple[date, Decimal]], list[date], int]:
    if not raw or len(raw) > MAX_PART_BYTES:
        raise SourceReject(f"SOURCE_REJECT:BYTES:{expected_year}:{len(raw)}")
    try:
    ...
    if not rows or rows[0] != ["observation_date", "DTWEXBGS"]:
    ...
    for index, row in enumerate(rows[1:]):
        ...
        try:
            ...
            raise SourceReject(f"SOURCE_REJECT:VALUE:{expected_year}:{index}") from error
...
def feature_feasibility(weekly: list[tuple[date, Decimal]]) -> dict[str, Any]:
    ...
    return {
        ...
        "dollar_weakness_weeks": sum(sign > 0 for sign in signs),
        "dollar_nonweakness_weeks": sum(sign <= 0 for sign in signs),
...
def deterministic_zip(parts: list[tuple[int, bytes]]) -> bytes:
    ...
    with zipfile.ZipFile(output, "w", compression=zipfile.ZIP_STORED) as archive:
        for year, raw in parts:
            info = zipfile.ZipInfo(f"fred-dtwexbgs-{year}.csv", date_time=(1980, 1, 1, 0, 0, 0))
...
def main() -> int:
    ...
    source_dir = state_path(args.source_dir, must_not_exist=False)
    ...
    if not source_dir.is_dir() or len({bundle_path, archive_path, normalized_path}) != 3:
        raise SourceReject("PATH_REJECT:SOURCE_OR_DUPLICATE")

    all_rows: list[tuple[date, Decimal]] = []
    parts: list[tuple[int, bytes]] = []
    response_parts: list[dict[str, Any]] = []
    for year in YEARS:
        part_path = source_dir / f"fred-dtwexbgs-{year}.csv"
        if not part_path.is_file() or part_path.is_symlink():
            raise SourceReject(f"SOURCE_REJECT:MISSING_PART:{year}")
    ...
    normalized = (
        "week_ending_friday,dtwexbgs_mean\n"
        + "".join(f"{week_end.isoformat()},{format(value, 'f')}\n" for week_end, value in weekly)
    ).encode("utf-8")
    bundle = {
        "schema_version": "1",
        "document_type": "FRED_DTWEXBGS_WEEKLY_SOURCE_BUNDLE_V1",
        ...
        "normalized_weekly_subset": {
            ...
            "columns": ["week_ending_friday", "dtwexbgs_mean"],
        ...
        "revision_boundary": "The exact annual FRED responses are a sealed present-vintage snapshot. Original H.10 release values and revision vintages remain MISSING_PROOF.",
        "license_boundary": "FRED tags DTWEXBGS as Public Domain: Citation Requested.",
