#!/usr/bin/env python3
"""Seal official present-vintage FRED PERMIT history without BTC outcomes."""
...
SPEC_PATH = REPO_ROOT / "research_pipeline/examples/btc-fred-housing-permits-yoy-growth-source-feasibility.v1.spec.json"
PRIOR_PATH = REPO_ROOT / "research_pipeline/examples/btc-fred-housing-permits-yoy-growth-long-cash-primary-prior.v1.json"
SOURCE_URL = "https://fred.stlouisfed.org/graph/fredgraph.csv?id=PERMIT&cosd=2017-01-01&coed=2024-12-31"
EXPECTED_ROWS = 96
...
SUPPORT_GATES = {
    "design": {"minimum_evaluations": 48, "minimum_per_state": 12, "minimum_transitions": 4},
...
def load_and_validate_spec() -> dict[str, Any]:
    ...
    if spec.get("status") != "FROZEN_BEFORE_2017_2024_PERMIT_FACTOR_ACCESS":
        raise SourceReject("SPEC_REJECT:STATUS")
    if spec.get("family_id") != "btc-fred-housing-permits-yoy-growth-long-cash" or prior.get("family_id") != spec.get("family_id"):
        raise SourceReject("SPEC_REJECT:FAMILY")
    source = spec.get("source_contract", {})
    expected = {
        "series": "PERMIT", "frequency": "MONTHLY", "request_url": SOURCE_URL,
...
def fetch() -> bytes:
    request = Request(SOURCE_URL, method="GET", headers={"Accept": "text/csv", "User-Agent": "AgoraResearchFredHousingPermitsSourceProbe/1.0"})
    try:
...
def parse_rows(raw: bytes) -> list[tuple[date, Decimal, str]]:
    ...
    if not rows or rows[0] != ["observation_date", "PERMIT"]:
    ...
    for index, row in enumerate(rows[1:]):
        ...
        if day.day != 1 or not value.is_finite() or value <= 0 or value > 5000:
...
def validate_rows(rows: list[tuple[date, Decimal, str]]) -> None:
    ...
    for index, ((prior_day, _, _), (current_day, _, _)) in enumerate(zip(rows, rows[1:], strict=False), start=1):
...
def main() -> int:
    ...
    bundle = {
        "schema_version": "1", "document_type": "FRED_HOUSING_PERMITS_MONTHLY_SOURCE_BUNDLE_V1", "authorization": "RESEARCH_ONLY_NOT_SHADOW_PAPER_OR_LIVE",
        "status": status, "captured_at": datetime.now(timezone.utc).isoformat().replace("+00:00", "Z"),
        "publisher": "U.S. Census Bureau and U.S. Department of Housing and Urban Development via Federal Reserve Bank of St. Louis FRED", "series": "PERMIT",
        ...
        "normalized_subset": {"path": normalized_path.relative_to(REPO_ROOT).as_posix(), "bytes": len(normalized), "sha256": sha256(normalized), "rows": len(rows), "first_date": rows[0][0].isoformat(), "last_date": rows[-1][0].isoformat(), "columns": ["observation_date", "permit_thousands_saar"]},
        ...
        "revision_boundary": "The exact FRED CSV is sealed present-vintage, seasonally adjusted history. Original releases, revisions and imputation vintages remain MISSING_PROOF; any historical pass still requires untouched independent OOS.",
        "interpretation_boundary": "Housing permits measure prospective construction activity, not crypto flows. Mortgage rates, supply and local policy can dominate the series.",
        "license_boundary": "FRED marks the Census/HUD series public domain with citation requested. Exact sealed bytes are retained as internal reproducibility evidence.",
    ...
    try:
        write_create_once(raw_path, raw); created.append(raw_path); write_create_once(normalized_path, normalized); created.append(normalized_path); write_create_once(bundle_path, bundle_raw); created.append(bundle_path)
    except Exception:
