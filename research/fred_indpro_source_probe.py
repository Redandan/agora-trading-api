#!/usr/bin/env python3
"""Seal official present-vintage FRED INDPRO history without BTC outcomes."""
...
SPEC_PATH = REPO_ROOT / "research_pipeline/examples/btc-fred-indpro-yoy-growth-source-feasibility.v1.spec.json"
PRIOR_PATH = REPO_ROOT / "research_pipeline/examples/btc-fred-indpro-yoy-growth-long-cash-primary-prior.v1.json"
SOURCE_URL = "https://fred.stlouisfed.org/graph/fredgraph.csv?id=INDPRO&cosd=2017-01-01&coed=2024-12-31"
EXPECTED_ROWS = 96
...
def load_and_validate_spec() -> dict[str, Any]:
    ...
    if spec.get("status") != "FROZEN_BEFORE_2017_2024_INDPRO_FACTOR_ACCESS":
        raise SourceReject("SPEC_REJECT:STATUS")
    if spec.get("family_id") != "btc-fred-indpro-yoy-growth-long-cash" or prior.get("family_id") != spec.get("family_id"):
        raise SourceReject("SPEC_REJECT:FAMILY")
    source = spec.get("source_contract", {})
    expected = {
        "series": "INDPRO", "frequency": "MONTHLY", "request_url": SOURCE_URL,
...
def fetch() -> bytes:
    request = Request(SOURCE_URL, method="GET", headers={"Accept": "text/csv", "User-Agent": "AgoraResearchFredIndproSourceProbe/1.0"})
    try:
...
def parse_rows(raw: bytes) -> list[tuple[date, Decimal, str]]:
    ...
    if not rows or rows[0] != ["observation_date", "INDPRO"]:
...
def main() -> int:
    ...
    bundle = {
        "schema_version": "1", "document_type": "FRED_INDPRO_MONTHLY_SOURCE_BUNDLE_V1",
        "authorization": "RESEARCH_ONLY_NOT_SHADOW_PAPER_OR_LIVE", "status": status,
        "captured_at": datetime.now(timezone.utc).isoformat().replace("+00:00", "Z"),
        "publisher": "Board of Governors of the Federal Reserve System via Federal Reserve Bank of St. Louis FRED", "series": "INDPRO",
        ...
        "normalized_subset": {"path": normalized_path.relative_to(REPO_ROOT).as_posix(), "bytes": len(normalized), "sha256": sha256(normalized), "rows": len(rows), "first_date": rows[0][0].isoformat(), "last_date": rows[-1][0].isoformat(), "columns": ["observation_date", "indpro_index_2017_100"]},
        ...
        "revision_boundary": "The exact FRED CSV is sealed present-vintage, seasonally adjusted history. Original releases, estimated components, seasonal factors and revisions remain MISSING_PROOF; any historical pass still requires untouched independent OOS.",
        "interpretation_boundary": "INDPRO measures U.S. industrial real output, not global liquidity or crypto-native demand. Broad risk-on co-movement is not independent causality.",
        "license_boundary": "FRED marks the Board of Governors series public domain with citation requested. Exact sealed bytes are retained as internal reproducibility evidence.",
