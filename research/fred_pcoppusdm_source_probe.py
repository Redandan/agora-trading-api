#!/usr/bin/env python3
"""Seal official present-vintage FRED PCOPPUSDM history without BTC outcomes."""
...
SPEC_PATH = REPO_ROOT / "research_pipeline/examples/btc-fred-pcoppusdm-3m-uptrend-source-feasibility.v1.spec.json"
PRIOR_PATH = REPO_ROOT / "research_pipeline/examples/btc-fred-pcoppusdm-3m-uptrend-long-cash-primary-prior.v1.json"
SOURCE_URL = "https://fred.stlouisfed.org/graph/fredgraph.csv?id=PCOPPUSDM&cosd=2018-01-01&coed=2024-12-31"
EXPECTED_ROWS = 84
EXPECTED_FIRST = date(2018, 1, 1)
EXPECTED_LAST = date(2024, 12, 1)
LOOKBACK = 3
AVAILABILITY_DAYS = 75
...
SUPPORT_GATES = {
    "design": {"minimum_uptrend": 12, "minimum_other": 12, "minimum_transitions": 6},
    "validation": {"minimum_uptrend": 6, "minimum_other": 6, "minimum_transitions": 3},
}
...
def load_and_validate_spec() -> dict[str, Any]:
    ...
    family_id = "btc-fred-pcoppusdm-3m-uptrend-long-cash"
    if (
        spec.get("document_type")
        != "BTC_FRED_PCOPPUSDM_3M_UPTREND_SOURCE_FEASIBILITY_SPEC_V1"
        or prior.get("document_type")
        != "BTC_FRED_PCOPPUSDM_3M_UPTREND_LONG_CASH_PRIMARY_PRIOR_V1"
    ...
    expected_source = {
        "provider": "FRED",
        "upstream_source": "INTERNATIONAL_MONETARY_FUND_PRIMARY_COMMODITY_PRICES",
        "series": "PCOPPUSDM",
        "request_url": SOURCE_URL,
        "transport": "HTTPS_GET_NO_REDIRECT_NO_RETRY_NO_CREDENTIAL",
        "maximum_response_bytes": MAX_RESPONSE_BYTES,
        "expected_header": "observation_date,PCOPPUSDM",
        ...
        "missing_markers": ["", "."],
        "required_valid_positive_values": EXPECTED_ROWS,
    ...
    if (
        factor.get("formula") != "PCOPPUSDM_MONTH_t-PCOPPUSDM_MONTH_t_minus_3"
    ...
    for label, gate in SUPPORT_GATES.items():
        frozen_keys = {
            "minimum_uptrend": f"minimum_{label}_uptrend_states",
...
def fetch() -> bytes:
    request = Request(
        ...
        headers={
            "Accept": "text/csv",
            "User-Agent": "AgoraResearchFredPcoppusdmSourceProbe/1.0",
        },
    )
    try:
...
def parse_rows(raw: bytes) -> list[tuple[date, Decimal, str]]:
    ...
    if not rows or rows[0] != ["observation_date", "PCOPPUSDM"]:
...
def _summarize(
    ...
    uptrend = sum(flags)
    other = len(flags) - uptrend
    transitions = sum(a != b for a, b in zip(flags, flags[1:], strict=False))
    annual: dict[str, dict[str, int]] = {}
    for effective, state in selected:
        counts = annual.setdefault(str(effective.year), {"uptrend": 0, "other": 0})
        counts["uptrend" if state else "other"] += 1
    passed = (
        uptrend >= gate["minimum_uptrend"]
        and other >= gate["minimum_other"]
        and transitions >= gate["minimum_transitions"]
    )
    return {
        "evaluations": len(selected),
        "uptrend_months": uptrend,
        "other_months": other,
        "transitions": transitions,
        "longest_uptrend_run_months": _longest_run(flags, True),
...
def feature_feasibility(rows: list[tuple[date, Decimal, str]]) -> dict[str, Any]:
    ...
    return {
        ...
        "admission_status": (
            "PASS_EXACT_MONTHLY_LATTICE_BOTH_COPPER_DIRECTION_STATES_AND_TRANSITIONS_BEFORE_BTC_OUTCOME_ACCESS"
            if passed
            else "DATA_REJECT_INADEQUATE_COPPER_DIRECTION_STATE_SUPPORT_OR_TRANSITIONS_BEFORE_BTC_OUTCOME_ACCESS"
        ),
    }
...
def main() -> int:
    ...
    validate_rows(rows)
    normalized = (
        "observation_date,pcoppusdm_usd_per_metric_ton_period_average\n"
    ...
    bundle = {
        "schema_version": "1",
        "document_type": "FRED_PCOPPUSDM_MONTHLY_SOURCE_BUNDLE_V1",
        "authorization": "RESEARCH_ONLY_NOT_SHADOW_PAPER_OR_LIVE",
        "status": status,
        "captured_at": datetime.now(timezone.utc).isoformat().replace("+00:00", "Z"),
        "publisher": "International Monetary Fund via Federal Reserve Bank of St. Louis FRED",
        "series": "PCOPPUSDM",
        "request_contract": {
            "method": "GET",
            "url": SOURCE_URL,
        ...
        "publication_timing_boundary": "Each monthly period average is conservatively usable only from observation-month start plus 75 calendar days at 00:00 UTC and remains effective until the next state becomes available.",
        "revision_boundary": "The exact FRED CSV is sealed present-vintage history. Original release vintages, later revisions and exact publication timestamps remain MISSING_PROOF; a historical pass would still require untouched independent OOS.",
        "interpretation_boundary": "Copper direction can reflect global demand, supply, inventories, the dollar or expectations; it is not proof of independent Bitcoin causality.",
        "license_boundary": "IMF-sourced FRED observations are retained as internal reproducibility evidence with source attribution; redistribution or commercial benchmark rights are not claimed.",
