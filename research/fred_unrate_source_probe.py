#!/usr/bin/env python3
"""Seal official present-vintage FRED UNRATE history without BTC outcomes."""
...
SPEC_PATH = REPO_ROOT / "research_pipeline/examples/btc-fred-unrate-yoy-nondeterioration-source-feasibility.v1.spec.json"
PRIOR_PATH = REPO_ROOT / "research_pipeline/examples/btc-fred-unrate-yoy-nondeterioration-long-cash-primary-prior.v1.json"
SOURCE_URL = "https://fred.stlouisfed.org/graph/fredgraph.csv?id=UNRATE&cosd=2017-01-01&coed=2024-12-31"
EXPECTED_ROWS = 96
...
def load_and_validate_spec() -> dict[str, Any]:
    ...
    if spec.get("status") != "FROZEN_BEFORE_2017_2024_UNRATE_FACTOR_ACCESS":
        raise SourceReject("SPEC_REJECT:STATUS")
    family_id = "btc-fred-unrate-yoy-nondeterioration-long-cash"
    ...
    if (
        ...
        or feature.get("nondeterioration_condition")
...
def fetch() -> bytes:
    request = Request(
        SOURCE_URL,
        method="GET",
        headers={"Accept": "text/csv", "User-Agent": "AgoraResearchFredUnrateSourceProbe/1.0"},
    )
    try:
...
def parse_rows(raw: bytes) -> list[tuple[date, Decimal, str]]:
    ...
    if not rows or rows[0] != ["observation_date", "UNRATE"]:
    ...
    for index, row in enumerate(rows[1:]):
        ...
        if day.day != 1 or not value.is_finite() or value <= 0 or value > 100:
...
def _summarize(states: list[tuple[datetime, bool]], start: datetime, end: datetime, gate: dict[str, int]) -> dict[str, Any]:
    ...
    nondeteriorating = sum(flags)
    deteriorating = len(flags) - nondeteriorating
    transitions = sum(current != prior for prior, current in zip(flags, flags[1:], strict=False))
    annual: dict[str, dict[str, int]] = {}
    for effective, is_nondeteriorating in selected:
        counts = annual.setdefault(str(effective.year), {"nondeteriorating": 0, "deteriorating": 0})
        counts["nondeteriorating" if is_nondeteriorating else "deteriorating"] += 1
    passed = (
        len(selected) >= gate["minimum_evaluations"]
        and nondeteriorating >= gate["minimum_per_state"]
        and deteriorating >= gate["minimum_per_state"]
        and transitions >= gate["minimum_transitions"]
    )
    return {
        "evaluations": len(selected),
        "nondeteriorating_months": nondeteriorating,
        "deteriorating_months": deteriorating,
        "transitions": transitions,
        "longest_nondeteriorating_run_months": _longest_run(flags, True),
        "longest_deteriorating_run_months": _longest_run(flags, False),
...
def feature_feasibility(rows: list[tuple[date, Decimal, str]]) -> dict[str, Any]:
    ...
    return {
        ...
        "nondeteriorating_months": sum(state[1] for state in states),
        "deteriorating_months": sum(not state[1] for state in states),
        ...
        "admission_status": (
            "PASS_MONTHLY_COVERAGE_BOTH_UNRATE_STATE_SUPPORT_AND_TRANSITIONS_BEFORE_BTC_OUTCOME_ACCESS"
            if passed
            else "DATA_REJECT_INADEQUATE_BOTH_UNRATE_STATE_SUPPORT_OR_TRANSITIONS_BEFORE_BTC_OUTCOME_ACCESS"
        ),
    }
...
def main() -> int:
    ...
    validate_rows(rows)
    normalized = (
        "observation_date,unrate_percent_seasonally_adjusted\n"
    ...
    bundle = {
        "schema_version": "1",
        "document_type": "FRED_UNRATE_MONTHLY_SOURCE_BUNDLE_V1",
        "authorization": "RESEARCH_ONLY_NOT_SHADOW_PAPER_OR_LIVE",
        "status": status,
        "captured_at": datetime.now(timezone.utc).isoformat().replace("+00:00", "Z"),
        "publisher": "U.S. Bureau of Labor Statistics via Federal Reserve Bank of St. Louis FRED",
        "series": "UNRATE",
        "request_contract": {
            "method": "GET",
            "url": SOURCE_URL,
        ...
        "normalized_subset": {
            ...
            "columns": ["observation_date", "unrate_percent_seasonally_adjusted"],
        },
        ...
        "revision_boundary": "The exact FRED CSV is sealed present-vintage seasonally adjusted history. Original releases, seasonal-factor revisions, population-control revisions and vintage values remain MISSING_PROOF; any historical pass still requires untouched independent OOS.",
        "interpretation_boundary": "UNRATE is a realized household-survey labor-market outcome, not a leading indicator or Bitcoin predictor. Strong labor can support demand while also delaying monetary easing.",
