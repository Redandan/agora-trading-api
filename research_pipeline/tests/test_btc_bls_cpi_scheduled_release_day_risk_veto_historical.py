...
from research.btc_bls_cpi_scheduled_release_day_risk_veto_historical import (
    DESIGN,
...
MANIFEST = REPO_ROOT / "research_pipeline/examples/btc-bls-cpi-scheduled-release-day-risk-veto-historical.v1.manifest.json"
HYPOTHESIS = REPO_ROOT / "research_pipeline/examples/btc-bls-cpi-scheduled-release-day-risk-veto-v1.hypothesis.json"
HYPOTHESIS_SCHEMA = REPO_ROOT / "research_pipeline/hypothesis.schema.json"


class BlsCpiScheduledReleaseDayRiskVetoHistoricalTest(unittest.TestCase):
    ...
    def test_only_frozen_official_release_dates_are_event_days(self) -> None:
        self.assertTrue(is_event_day(datetime(2020, 1, 14, 12)))
        self.assertFalse(is_event_day(datetime(2020, 1, 15, 12)))
        self.assertTrue(is_event_day(datetime(2020, 4, 10, 23)))
        self.assertTrue(is_event_day(datetime(2024, 11, 13, 23)))
        self.assertFalse(is_event_day(datetime(2024, 11, 14)))
