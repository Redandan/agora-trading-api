...
class CandidateFunnelTest(unittest.TestCase):
    def test_catalog_is_schema_valid_and_all_evidence_hashes_verify(self) -> None:
        ...
        self.assertEqual(len(catalog["closed_families"]), 140)
        self.assertEqual(
            {
                family["family_id"]
        ...
        stochastic = next(
            ...
            if family["family_id"]
            == "btc-daily-stochastic-5-3-3-kd-cross-long-cash"
        )
        self.assertEqual(stochastic["base_stage"], "PREREGISTRATION_READY")
        self.assertEqual(
            [binding["role"] for binding in stochastic["evidence_bindings"]],
            [
    ...
    def test_closed_microstructure_source_does_not_block_open_family_ranking(self) -> None:
        ...
        self.assertEqual(snapshot["summary"]["closed_family_count"], 141)
