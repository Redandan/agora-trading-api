...
class CandidateFunnelTest(unittest.TestCase):
    def test_catalog_is_schema_valid_and_all_evidence_hashes_verify(self) -> None:
        ...
        self.assertEqual(len(catalog["closed_families"]), 167)
        closed_nr7 = next(
            family
            for family in catalog["closed_families"]
            if family["family_id"]
            == "closed-btc-daily-nr7-volatility-contraction-breakout-v1"
        )
        self.assertEqual(
            closed_nr7["disposition"],
        ...
        closed_paxg_static = next(
            family
            for family in catalog["closed_families"]
            if family["family_id"]
            == "closed-btc-paxg-static-50-50-diversification-v1"
        )
        self.assertEqual(
            closed_paxg_static["disposition"],
        ...
        closed_peg_dislocation = next(
            ...
            for family in catalog["closed_families"]
            if family["family_id"]
            == "closed-btc-usdcusdt-peg-dislocation-risk-veto"
        )
        self.assertEqual(
            closed_peg_dislocation["disposition"],
            "DATA_REJECT_CLOSE_BTC_USDCUSDT_PEG_DISLOCATION_RISK_VETO_FAMILY",
        )
        self.assertTrue(closed_peg_dislocation["prohibited_reopen"])
        ...
        closed_fixed_maturity_carry = next(
            ...
            for family in catalog["closed_families"]
            if family["family_id"]
            == "closed-btc-binance-fixed-maturity-delivery-cash-and-carry"
        )
        self.assertEqual(
            closed_fixed_maturity_carry["disposition"],
            "DATA_REJECT_CLOSE_FIXED_MATURITY_DELIVERY_CASH_AND_CARRY_FAMILY_BEFORE_ARCHIVE_OR_ECONOMIC_ACCESS",
        )
        self.assertTrue(closed_fixed_maturity_carry["prohibited_reopen"])
        ...
        closed_basis_convergence = next(
            ...
            for family in catalog["closed_families"]
            if family["family_id"]
            == "closed-btc-binance-usdm-perpetual-basis-convergence-market-neutral"
        )
        self.assertEqual(
            closed_basis_convergence["disposition"],
            "NO_HYPOTHESIS_CLOSE_BTC_BINANCE_USDM_BASIS_CONVERGENCE_AT_EXECUTION_SURVIVAL_GATE",
        )
        self.assertTrue(closed_basis_convergence["prohibited_reopen"])
        ...
        monday_calendar = next(
            family
            for family in catalog["closed_families"]
            if family["family_id"] == "closed-btc-utc-monday-long-cash-v1"
        )
        self.assertEqual(
            monday_calendar["disposition"],
            "NO_CANDIDATE_CLOSE_BTC_UTC_MONDAY_LONG_CASH_FAMILY",
        )
        self.assertEqual(
            monday_calendar["duplicate_family_key"],
            "btc-utc-monday-00-to-tuesday-00-full-capital-long-cash",
        )
        self.assertTrue(monday_calendar["prohibited_reopen"])
        hash_ribbon = next(
            family
            for family in catalog["closed_families"]
            if family["family_id"]
            == "closed-btc-coinmetrics-hash-ribbon-health-state-long-cash-v1"
        )
        self.assertEqual(
            hash_ribbon["disposition"],
            "NO_CANDIDATE_CLOSE_BTC_COINMETRICS_HASH_RIBBON_HEALTH_STATE_FAMILY_PRE_ECONOMIC",
        )
        self.assertEqual(
            hash_ribbon["duplicate_family_key"],
            "btc-coinmetrics-hashrate-30d-above-60d-plus3d-full-capital-long-cash",
        )
        self.assertEqual(
            [binding["role"] for binding in hash_ribbon["evidence_bindings"]],
            [
    ...
    def test_closed_microstructure_source_does_not_block_open_family_ranking(self) -> None:
        ...
        self.assertEqual(snapshot["summary"]["closed_family_count"], 168)
