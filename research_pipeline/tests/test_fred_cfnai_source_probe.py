...
MODULE_PATH = REPO_ROOT / "research/fred_cfnai_source_probe.py"
SPEC = importlib.util.spec_from_file_location("fred_cfnai_source_probe", MODULE_PATH)
...
def monthly_rows() -> list[tuple[date, Decimal, str]]:
    pattern = [Decimal("0.5"), Decimal("0.2"), Decimal("-0.1"), Decimal("-0.4")]
    rows = []
...
class FredCfnaiSourceProbeTest(unittest.TestCase):
    def test_frozen_spec_matches_probe_contract(self) -> None:
        spec = probe.load_and_validate_spec()
        self.assertEqual(spec["source_contract"]["expected_unique_ordered_month_rows"], 84)
        self.assertEqual(spec["factor_contract"]["formula"], "CFNAI_MONTH_t")
        self.assertEqual(
            spec["factor_contract"]["effective_time"],
            "OBSERVATION_MONTH_START_PLUS_60_CALENDAR_DAYS_AT_0000_UTC",
        )

    def test_fixture_passes_lattice_and_expected_windows(self) -> None:
        rows = monthly_rows()
    ...
    def test_negative_value_is_accepted_but_missing_and_wrong_header_reject(self) -> None:
        parsed = probe.parse_rows(b"observation_date,CFNAI\n2018-01-01,-0.35\n")
        self.assertEqual(parsed[0][1], Decimal("-0.35"))
        with self.assertRaisesRegex(probe.SourceReject, "SOURCE_REJECT:MISSING_OR_ROW"):
            probe.parse_rows(b"observation_date,CFNAI\n2018-01-01,.\n")
        with self.assertRaisesRegex(probe.SourceReject, "SOURCE_REJECT:HEADER"):
            probe.parse_rows(b"DATE,CFNAI\n2018-01-01,0.5\n")
