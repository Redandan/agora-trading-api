...
MODULE_PATH = REPO_ROOT / "research/fred_cpiaucsl_disinflation_source_probe.py"
SPEC = importlib.util.spec_from_file_location("fred_cpiaucsl_disinflation_source_probe", MODULE_PATH)
assert SPEC is not None and SPEC.loader is not None
probe = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(probe)


class FredCpiaucslDisinflationSourceProbeTest(unittest.TestCase):
    ...
    def test_fixture_passes_lattice_and_produces_expected_evaluations(self) -> None:
        ...
        for index in range(96):
            value = Decimal("240") + Decimal(index) / Decimal("10") + Decimal((index // 6) % 2)
    ...
    def test_wrong_header_is_rejected(self) -> None:
        with self.assertRaisesRegex(probe.SourceReject, "SOURCE_REJECT:HEADER"):
            probe.parse_rows(b"DATE,CPIAUCSL\n2017-01-01,240\n")
