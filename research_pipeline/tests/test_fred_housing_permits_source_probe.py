...
MODULE_PATH = REPO_ROOT / "research/fred_housing_permits_source_probe.py"
SPEC = importlib.util.spec_from_file_location("fred_housing_permits_source_probe", MODULE_PATH)
assert SPEC is not None and SPEC.loader is not None
probe = importlib.util.module_from_spec(SPEC); SPEC.loader.exec_module(probe)


class FredHousingPermitsSourceProbeTest(unittest.TestCase):
    ...
    def test_fixture_passes_support(self) -> None:
        rows = []; current = date(2017, 1, 1)
        for index in range(96):
            value = Decimal(1000 + ((index // 6) % 2) * 200 + index)
            rows.append((current, value, str(value))); current = probe.add_months(current, 1)
    ...
    def test_wrong_header_is_rejected(self) -> None:
        with self.assertRaisesRegex(probe.SourceReject, "SOURCE_REJECT:HEADER"):
            probe.parse_rows(b"DATE,PERMIT\n2017-01-01,1000\n")
