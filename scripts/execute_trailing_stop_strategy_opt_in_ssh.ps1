...
function Invoke-RemoteSetTrailingStopOptIn {
    param([int]$TargetStrategyId, [string]$TargetNotes)
...
export PORT MCP_KEY STRATEGY_ID='$TargetStrategyId' NOTES='$TargetNotes'
python3 - <<'PY'
import json
import os
import urllib.error
...
enabled_marker = "trailingStopEnabled=true" in text
...
if (-not $usingReviewLog -or ($Execute.IsPresent -and -not $usingPostLog)) {
...
$expectedConfirmText = "EXECUTE_TRAILING_STOP_OPT_IN_$StrategyId"
...
if ($currentGlobalEnabled -ne "false") { Add-MissingRequirement -List $preMissing -Value "global trailing remains disabled before env diff" }
if ($currentGlobalDryRun -ne "true") { Add-MissingRequirement -List $preMissing -Value "global dry-run remains true" }
if ($recommendedStrategyId -ne $StrategyId) { Add-MissingRequirement -List $preMissing -Value "recommended strategy matches requested StrategyId" }
if ($proposedWrite -notmatch "setTrailingStopOptIn\(strategyId=$StrategyId, enabled=true") {
    Add-MissingRequirement -List $preMissing -Value "proposed write is setTrailingStopOptIn enabled=true for requested StrategyId"
}
...
if ($Execute.IsPresent -and $preReady) {
    $writeResult = Invoke-RemoteSetTrailingStopOptIn -TargetStrategyId $StrategyId -TargetNotes $Notes
...
if ($Execute.IsPresent) {
    ...
    if ($writeTrailingEnabled -ne "true") { Add-MissingRequirement -List $missingRequirements -Value "setTrailingStopOptIn confirms trailingStopEnabled=true" }
...
$status = if (-not $Execute.IsPresent -and $preReady) {
...
$decision = if ($status -eq "DRY_RUN_READY_FOR_SEPARATE_EXECUTION_AUTHORIZATION_NOT_MUTATION") {
...
$packet = [pscustomobject]@{
    ...
    nextRequiredAuthorization = if ($status -eq "DRY_RUN_READY_FOR_SEPARATE_EXECUTION_AUTHORIZATION_NOT_MUTATION") {
