...
if sudo grep -q "location[[:space:]]*/api/trading/" "$NGINX_CONF"; then
  tmp_file="$(mktemp)"
  awk -v port="$TRADING_PORT" -f scripts/rewrite_nginx_trading_routes.awk "$NGINX_CONF" > "$tmp_file"
  ...
  tmp_file="$(mktemp)"
  awk -v port="$TRADING_PORT" -v insert_trading_path=1 -f scripts/rewrite_nginx_trading_routes.awk "$NGINX_CONF" > "$tmp_file" || {
    rm -f "$tmp_file"
