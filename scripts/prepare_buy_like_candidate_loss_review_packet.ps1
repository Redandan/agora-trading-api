...
foreach ($path in @($BuyLike14dLogPath, $BuyLike30dLogPath, $DataFreshnessReadinessLogPath, $SignalCorrectnessLogPath)) {
...
foreach ($log in @($buyLike14d, $buyLike30d, $dataFreshnessReadiness, $signalCorrectness)) {
    if (-not $log.Fresh) {
...
$nextEvidenceTarget = if ($summary30d.noTerminalFollowupRows -gt 0) {
...
$packet = [pscustomobject]@{
    ...
    sourceLogFreshness = @(@($buyLike14d, $buyLike30d, $dataFreshnessReadiness, $signalCorrectness) | ForEach-Object {
    ...
    interpretation = [pscustomobject]@{
        ...
        noTerminalFollowup = "candidate-audit continuity gap; needs replayable continuity evidence before closure"
