...
if ($null -ne $entryDedupPacket) {
    ...
    $readyReviewItems = @($readyReviewItems) + [pscustomobject]@{
        proposalId = "entry-dedup-semantics-shadow-operator-review"
        sourceProposalId = "entry-dedup-semantics-shadow-experiment-review"
        lane = "entry-filter-shadow"
        priority = "P1"
        status = "READY_FOR_OPERATOR_REVIEW_NOT_LIVE"
        decisionClass = "review-only-shadow-experiment"
...
foreach ($requiredReviewItem in @("trailing-stop-dry-run-operator-review", "strategy485-risk-reduction-shadow-operator-review", "entry-dedup-semantics-shadow-operator-review")) {
...
$packet = [pscustomobject]@{
    ...
    operatorReviewChecklist = @(
        ...
        "review EntryDedup shadow item as review-only evidence, not policy approval",
