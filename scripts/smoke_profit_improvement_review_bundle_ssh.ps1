...
Write-Host ("  deploy_required_before_profit_improvement_review=" + ([bool]$reviewDecision.deployRequired).ToString().ToLowerInvariant())
Write-Host ("  profit_improvement_missing_requirement_count=" + $reviewDecision.missingRequirementCount)
Write-Host ("  profit_improvement_missing_requirements=" + (ConvertTo-Json -Compress @($reviewDecision.missingRequirements)))
