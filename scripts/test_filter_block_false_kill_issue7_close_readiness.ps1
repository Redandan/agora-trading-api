...
$scriptPath = Join-Path $PSScriptRoot "prepare_filter_block_false_kill_issue7_close_readiness.ps1"
$packetScriptPath = Join-Path $PSScriptRoot "prepare_filter_block_false_kill_issue7_packet.ps1"
...
foreach ($marker in @(
        ...
        "RequireBlocked",
        "read-only issue #7 close-readiness packet only"
    )) {
    Assert-Contains -Name "Issue #7 close-readiness script marker" -Text $scriptText -Pattern ([regex]::Escape($marker))
}
...
foreach ($marker in @(
        "prepare_filter_block_false_kill_issue7_close_readiness.ps1",
        "issue7_close_allowed",
        "ISSUE7_CLOSE_READINESS_PACKET"
    )) {
    Assert-Contains -Name "Issue #7 close-readiness docs marker" -Text $docsText -Pattern ([regex]::Escape($marker))
}
...
function New-Issue7SourceLog {
    ...
    $collector = if ($Replayable) { "READY:2" } else { "N/A:27" }
    $hardGate = if ($Replayable) { "PREVIEW_READY:2" } else { "N/A:27" }
    @"
...
$tempDir = Join-Path ([System.IO.Path]::GetTempPath()) ("issue7-close-readiness-" + [guid]::NewGuid().ToString("N"))
...
try {
    ...
    $blocked = & $scriptPath -SourceLog $blockedSource -ObservationLog $blockedObservation -RequireBlocked *>&1
    $blockedText = $blocked -join "`n"
    Assert-Contains -Name "Issue #7 close-readiness blocked status" -Text $blockedText -Pattern "issue7_close_readiness_status=BLOCKED_NOT_CLOSABLE_REPLAY_EVIDENCE_MISSING"
    Assert-Contains -Name "Issue #7 close-readiness blocked flag" -Text $blockedText -Pattern "issue7_close_allowed=false"
    Assert-Contains -Name "Issue #7 close-readiness blocked missing replay rows" -Text $blockedText -Pattern "complete_replayable_candidate_rows > 0"
    Assert-Contains -Name "Issue #7 close-readiness blocked packet" -Text $blockedText -Pattern '"closeAllowed":false'
    ...
    $ready = & $scriptPath -SourceLog $readySource -ObservationLog $readyObservation *>&1
    $readyText = $ready -join "`n"
    Assert-Contains -Name "Issue #7 close-readiness ready status" -Text $readyText -Pattern "issue7_close_readiness_status=READY_TO_CLOSE_NOT_LIVE_RELAXATION"
    Assert-Contains -Name "Issue #7 close-readiness ready flag" -Text $readyText -Pattern "issue7_close_allowed=true"
    Assert-Contains -Name "Issue #7 close-readiness ready packet" -Text $readyText -Pattern '"closeAllowed":true'
    Assert-Contains -Name "Issue #7 close-readiness not live" -Text $readyText -Pattern "issue7_live_relaxation_allowed=false"
} finally {
...
Write-Host "[filter-block-false-kill-issue7-close-readiness-test] OK"
