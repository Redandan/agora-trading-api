...
$scriptPath = Join-Path $PSScriptRoot "prepare_filter_block_false_kill_issue7_collector_activation_review_packet.ps1"
...
foreach ($marker in @(
        ...
        "READY_FOR_ISSUE7_EVIDENCE_COLLECTOR_ACTIVATION_REVIEW_NOT_LIVE",
        "REVIEW_SEPARATE_EVIDENCE_ONLY_COLLECTOR_ENV_DIFF",
        "TRADING_DATAFRESHNESS_SHADOW_REPLAY_COLLECTOR_ENABLED=true",
        "collectorActivationAllowed",
        "deployOrEnvChangeAllowed",
        "orderAllowed",
        "telegramSendAllowed",
        "requiredBeforeIssueClose",
        "stopConditions"
    )) {
    Assert-Contains -Name "Issue #7 collector review script marker" -Text $scriptText -Pattern ([regex]::Escape($marker))
}
...
foreach ($marker in @(
        "prepare_filter_block_false_kill_issue7_collector_activation_review_packet.ps1",
        "issue7_evidence_collector_activation_review_status",
        "ISSUE7_EVIDENCE_COLLECTOR_ACTIVATION_REVIEW_PACKET"
    )) {
    Assert-Contains -Name "Issue #7 collector review docs marker" -Text $docsText -Pattern ([regex]::Escape($marker))
}
Assert-Contains -Name "Issue #7 collector review verify marker" -Text (Get-Content -Raw -LiteralPath $verifyPath) -Pattern "test_filter_block_false_kill_issue7_collector_activation_review_packet.ps1"
...
$tempDir = Join-Path ([System.IO.Path]::GetTempPath()) ("issue7-collector-review-" + [guid]::NewGuid().ToString("N"))
...
try {
    ...
    $output = & $scriptPath -SourceLog $sourceLog -ObservationLog $observationLog -ReadinessLogPath $readinessLog -RequireReady *>&1
    $text = $output -join "`n"
    Assert-Contains -Name "Issue #7 collector review status" -Text $text -Pattern "issue7_evidence_collector_activation_review_status=READY_FOR_ISSUE7_EVIDENCE_COLLECTOR_ACTIVATION_REVIEW_NOT_LIVE"
    Assert-Contains -Name "Issue #7 collector review decision" -Text $text -Pattern "issue7_evidence_collector_activation_review_decision=REVIEW_SEPARATE_EVIDENCE_ONLY_COLLECTOR_ENV_DIFF"
    Assert-Contains -Name "Issue #7 collector review env diff" -Text $text -Pattern "proposed_separate_env_diff=TRADING_DATAFRESHNESS_SHADOW_REPLAY_COLLECTOR_ENABLED=true"
    Assert-Contains -Name "Issue #7 collector review close blocked" -Text $text -Pattern "issue7_close_allowed=false"
    Assert-Contains -Name "Issue #7 collector review no deploy" -Text $text -Pattern "deploy_or_env_change_allowed=false"
    Assert-Contains -Name "Issue #7 collector review no orders" -Text $text -Pattern "order_allowed=false"
    Assert-Contains -Name "Issue #7 collector review packet" -Text $text -Pattern '"packetType":"ISSUE7_EVIDENCE_COLLECTOR_ACTIVATION_REVIEW_PACKET"'
} finally {
...
Write-Host "[filter-block-false-kill-issue7-collector-activation-review-packet-test] OK"
