...
function Get-LiveReadinessBundleBlockers {
    ...
    $runtimeOrderSentBlockerCount = 0
    if ($RuntimeEvidence -match "(?m)^\s*orderSentEvidenceBlockerCount=([0-9]+)") {
        $runtimeOrderSentBlockerCount = [int]$Matches[1]
    } elseif ($RuntimeEvidence -match "(?m)^\s*orderSentEvidence=([0-9]+)") {
        $runtimeOrderSentBlockerCount = [int]$Matches[1]
    } else {
        $runtimeOrderSentBlockerCount = -1
    }
...
function Assert-ReviewPacketMinimumGuarded {
    ...
    foreach ($pattern in @(
            ...
            'orderSentEvidenceBlockerCount=0',
...
targetStrategyShadowLikeRows=3
shadowIntentCount=3
orderSentEvidence=0
...
Assert-BlockerCase -Name "audit missing readiness detail field summary fails closed" -Inputs (Merge-Inputs $cleanInputs @{ Audit = $readyAudit.Replace("missing_readiness_detail_fields=[]`n", "") }) -ExpectedBlockers @("MCP_AUDIT_TOOL_ERROR")
...
Assert-BlockerCase -Name "audit missing readiness detail fields fail closed" -Inputs (Merge-Inputs $cleanInputs @{ Audit = "$($readyAudit.Replace("verdict=READY_FOR_OPERATOR_REVIEW_NOT_LIVE_ENABLED", "verdict=NOT_READY"))`nmissing_readiness_detail_fields=[`"scoreBuyPostScoutAdd.executionEligible`"]`nblockers=[`"READINESS_DETAILS_MISSING_FIELDS`"]" }) -ExpectedBlockers @("LIVE_READINESS_NOT_READY", "MCP_AUDIT_TOOL_ERROR")
...
Assert-BlockerCase -Name "audit missing execution eligibility marker fails closed" -Inputs (Merge-Inputs $cleanInputs @{ Audit = $readyAudit.Replace('"scoreBuyPostScoutAdd":{"dryRun":true,"enabled":false,"executionEligible":true,"orderSent":false}', '"scoreBuyPostScoutAdd":{"dryRun":true,"enabled":false,"orderSent":false}') }) -ExpectedBlockers @("EXECUTION_ELIGIBILITY_NOT_READY")
...
Assert-BlockerCase -Name "background missing review plan fails closed" -Inputs (Merge-Inputs $cleanInputs @{ Background = "verdict=OK_BACKGROUND_AUTOMATION_DISABLED`nbackgroundAutomationClear=true`nbackground_automation_blockers=[]`nhigh_risk_background_automation_true=[]" }) -ExpectedBlockers @("BACKGROUND_AUTOMATION_REVIEW")
Assert-BlockerCase -Name "background true review plan fails closed when otherwise clear" -Inputs (Merge-Inputs $cleanInputs @{ Background = "verdict=OK_BACKGROUND_AUTOMATION_DISABLED`nbackgroundAutomationClear=true`nbackground_automation_blockers=[]`nhigh_risk_background_automation_true=[]`nbackground_automation_review_plan=[{`"state`":`"TRUE`"}]" }) -ExpectedBlockers @("BACKGROUND_AUTOMATION_REVIEW")
...
Assert-BlockerCase -Name "runtime config disabled" -Inputs (Merge-Inputs $cleanInputs @{ RuntimeEvidence = "diagnosis=CONFIG_DISABLED`ntargetStrategyShadowLikeRows=3`nshadowIntentCount=3`norderSentEvidence=0`nruntime_evidence_review_plan=[{`"state`":`"BLOCKED`"}]" }) -ExpectedBlockers @("RUNTIME_EVIDENCE_CONFIG_DISABLED")
Assert-BlockerCase -Name "runtime no canonical rows" -Inputs (Merge-Inputs $cleanInputs @{ RuntimeEvidence = "diagnosis=NO_CANONICAL_ROWS`ntargetStrategyShadowLikeRows=3`nshadowIntentCount=3`norderSentEvidence=0`nruntime_evidence_review_plan=[{`"state`":`"BLOCKED`"}]" }) -ExpectedBlockers @("RUNTIME_EVIDENCE_NO_CANONICAL_ROWS")
Assert-BlockerCase -Name "runtime no target strategy canonical rows" -Inputs (Merge-Inputs $cleanInputs @{ RuntimeEvidence = "diagnosis=NO_TARGET_STRATEGY_CANONICAL_ROWS`nruntimeEvidenceRows=200`ntargetStrategyId=485`ntargetStrategyEvidenceRows=0`ntargetStrategyShadowLikeRows=3`nshadowIntentCount=3`norderSentEvidence=0`nruntime_evidence_review_plan=[{`"state`":`"BLOCKED`"}]" }) -ExpectedBlockers @("RUNTIME_EVIDENCE_NO_TARGET_STRATEGY_CANONICAL_ROWS")
Assert-BlockerCase -Name "runtime review required" -Inputs (Merge-Inputs $cleanInputs @{ RuntimeEvidence = "diagnosis=REVIEW_RUNTIME_EVIDENCE_STATUS`ntargetStrategyShadowLikeRows=3`nshadowIntentCount=3`norderSentEvidence=0`nruntime_evidence_review_plan=[{`"state`":`"BLOCKED`"}]" }) -ExpectedBlockers @("RUNTIME_EVIDENCE_REVIEW_REQUIRED")
Assert-BlockerCase -Name "runtime missing core field list fails closed" -Inputs (Merge-Inputs $cleanInputs @{ RuntimeEvidence = 'diagnosis=CANONICAL_SHADOW_READY missing_runtime_evidence_fields=["runtimeEvidenceStatus"] targetStrategyShadowLikeRows=3 shadowIntentCount=3 orderSentEvidence=0 runtime_evidence_review_plan=[{"state":"READY_FOR_OTHER_BLOCKER_REVIEW"}]' }) -ExpectedBlockers @("RUNTIME_EVIDENCE_REVIEW_REQUIRED")
Assert-BlockerCase -Name "runtime no shadow intent" -Inputs (Merge-Inputs $cleanInputs @{ RuntimeEvidence = "diagnosis=CANONICAL_SHADOW_READY`ntargetStrategyShadowLikeRows=0`nshadowIntentCount=3`norderSentEvidence=0`nruntime_evidence_review_plan=[{`"state`":`"READY_FOR_OTHER_BLOCKER_REVIEW`"}]" }) -ExpectedBlockers @("RUNTIME_EVIDENCE_NO_SHADOW_INTENT")
Assert-BlockerCase -Name "runtime canonical rows no shadow intent" -Inputs (Merge-Inputs $cleanInputs @{ RuntimeEvidence = "diagnosis=CANONICAL_ROWS_NO_SHADOW_INTENT`ntargetStrategyShadowLikeRows=0`nshadowIntentCount=3`norderSentEvidence=0`nruntime_evidence_review_plan=[{`"state`":`"BLOCKED`"}]" }) -ExpectedBlockers @("RUNTIME_EVIDENCE_NO_SHADOW_INTENT")
Assert-BlockerCase -Name "runtime order sent evidence" -Inputs (Merge-Inputs $cleanInputs @{ RuntimeEvidence = "diagnosis=CANONICAL_SHADOW_READY`ntargetStrategyShadowLikeRows=3`nshadowIntentCount=3`norderSentEvidence=1`nruntime_evidence_review_plan=[{`"state`":`"HARD_BLOCKED`"}]" }) -ExpectedBlockers @("LOCAL_TRADINGVIEW_ORDER_SENT_EVIDENCE", "RUNTIME_EVIDENCE_ORDER_SENT", "RUNTIME_EVIDENCE_REVIEW_REQUIRED")
Assert-BlockerCase -Name "runtime non-target order sent evidence is scoped out" -Inputs (Merge-Inputs $cleanInputs @{ RuntimeEvidence = "diagnosis=CANONICAL_SHADOW_READY`ntargetStrategyShadowLikeRows=3`nshadowIntentCount=3`norderSentEvidence=1`norderSentEvidenceTargetStrategy=0`norderSentEvidenceOtherStrategy=1`norderSentEvidenceBlockerCount=0`nruntime_evidence_review_plan=[{`"state`":`"REVIEWED_NON_TARGET_ORDER_EVIDENCE`"}]" }) -ExpectedBlockers @()
Assert-BlockerCase -Name "runtime missing diagnosis fails closed" -Inputs (Merge-Inputs $cleanInputs @{ RuntimeEvidence = "targetStrategyShadowLikeRows=3`nshadowIntentCount=3`norderSentEvidence=0" }) -ExpectedBlockers @("RUNTIME_EVIDENCE_REVIEW_REQUIRED")
Assert-BlockerCase -Name "runtime missing review plan fails closed" -Inputs (Merge-Inputs $cleanInputs @{ RuntimeEvidence = "diagnosis=CANONICAL_SHADOW_READY`ntargetStrategyShadowLikeRows=3`nshadowIntentCount=3`norderSentEvidence=0" }) -ExpectedBlockers @("RUNTIME_EVIDENCE_REVIEW_REQUIRED")
Assert-BlockerCase -Name "runtime blocked review plan fails closed when otherwise ready" -Inputs (Merge-Inputs $cleanInputs @{ RuntimeEvidence = "diagnosis=CANONICAL_SHADOW_READY`ntargetStrategyShadowLikeRows=3`nshadowIntentCount=3`norderSentEvidence=0`nruntime_evidence_review_plan=[{`"state`":`"BLOCKED`"}]" }) -ExpectedBlockers @("RUNTIME_EVIDENCE_REVIEW_REQUIRED")
...
Assert-BlockerCase -Name "runtime missing shadow intent fails closed" -Inputs (Merge-Inputs $cleanInputs @{ RuntimeEvidence = "diagnosis=CANONICAL_SHADOW_READY`ntargetStrategyShadowLikeRows=N/A`nshadowIntentCount=3`norderSentEvidence=0`nruntime_evidence_review_plan=[{`"state`":`"READY_FOR_OTHER_BLOCKER_REVIEW`"}]" }) -ExpectedBlockers @("RUNTIME_EVIDENCE_NO_SHADOW_INTENT")
Assert-BlockerCase -Name "runtime missing order sent evidence fails closed" -Inputs (Merge-Inputs $cleanInputs @{ RuntimeEvidence = "diagnosis=CANONICAL_SHADOW_READY`ntargetStrategyShadowLikeRows=3`nshadowIntentCount=3`nruntime_evidence_review_plan=[{`"state`":`"READY_FOR_OTHER_BLOCKER_REVIEW`"}]" }) -ExpectedBlockers @("RUNTIME_EVIDENCE_REVIEW_REQUIRED")
...
Assert-BlockerCase -Name "local tradingview runtime order sent evidence" -Inputs (Merge-Inputs $cleanInputs @{ RuntimeEvidence = "diagnosis=CANONICAL_SHADOW_READY`ntargetStrategyShadowLikeRows=3`nshadowIntentCount=3`norderSentEvidence=1`nruntime_evidence_review_plan=[{`"state`":`"READY_FOR_OTHER_BLOCKER_REVIEW`"}]" }) -ExpectedBlockers @("LOCAL_TRADINGVIEW_ORDER_SENT_EVIDENCE", "RUNTIME_EVIDENCE_ORDER_SENT")
Assert-BlockerCase -Name "local tradingview ignores scoped non-target runtime order evidence" -Inputs (Merge-Inputs $cleanInputs @{ RuntimeEvidence = "diagnosis=CANONICAL_SHADOW_READY`ntargetStrategyShadowLikeRows=3`nshadowIntentCount=3`norderSentEvidence=1`norderSentEvidenceBlockerCount=0`nruntime_evidence_review_plan=[{`"state`":`"REVIEWED_NON_TARGET_ORDER_EVIDENCE`"}]" }) -ExpectedBlockers @()
