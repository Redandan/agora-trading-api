...
"@ | Set-Content -LiteralPath $watchLog -Encoding UTF8
    ...
    $blocked = & $scriptPath -ProfitImprovementLog $profitLog -Issue7BundleLog $issue7Log -ProfitEvidenceWatchLog $watchLog -RequireBlocked *>&1
    ...
    $missing = & $scriptPath -ProfitImprovementLog (Join-Path $tempDir "missing-profit.log") -Issue7BundleLog $issue7Log -ProfitEvidenceWatchLog $watchLog -RequireBlocked *>&1
