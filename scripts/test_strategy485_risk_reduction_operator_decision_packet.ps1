...
$scriptPath = Join-Path $PSScriptRoot "prepare_strategy485_risk_reduction_operator_decision_packet.ps1"
...
foreach ($marker in @(
        "[strategy485-risk-reduction-operator-decision-packet] read-only packet",
        "scope=READ_ONLY",
        ...
        "strategy485_risk_reduction_priority_rank",
        ...
        "notAuthorization=read-only strategy485 risk-reduction operator decision packet only",
        "RequireReady"
    )) {
    Assert-Contains -Name "strategy485 risk decision marker" -Text $scriptText -Pattern ([regex]::Escape($marker))
}
...
foreach ($marker in @(
    ...
    Assert-Contains -Name "priority brief supports strategy485 packet" -Text $priorityText -Pattern ([regex]::Escape($marker))
}
...
foreach ($marker in @(
    ...
    Assert-Contains -Name "exit-side packet supports strategy485 packet" -Text $exitSideText -Pattern ([regex]::Escape($marker))
}
...
foreach ($marker in @(
        ...
        "STRATEGY485_RISK_REDUCTION_OPERATOR_DECISION_PACKET",
        "strategy485_risk_reduction_operator_decision_packet",
        "strategy485 risk-reduction operator decision packet",
        "close_position_allowed=false"
    )) {
    Assert-Contains -Name "docs mention strategy485 risk decision packet" -Text $docsText -Pattern ([regex]::Escape($marker))
}

$tempMatrixPath = Join-Path ([System.IO.Path]::GetTempPath()) ("strategy485-risk-matrix-" + [guid]::NewGuid().ToString("N") + ".log")
$tempReviewDir = Join-Path ([System.IO.Path]::GetTempPath()) ("strategy485-risk-review-" + [guid]::NewGuid().ToString("N"))
try {
    ...
    if ($null -eq $powerShell) {
        throw "Unable to find powershell or pwsh for strategy485 risk decision packet test"
    }
    ...
    if ($exitCode -ne 0) {
        throw "strategy485 risk decision packet failed latest-pointer reuse:`n$text"
    }
    foreach ($marker in @(
            ...
            '"status":"READY_FOR_STRATEGY485_RISK_REDUCTION_OPERATOR_DECISION_NOT_MUTATION"',
        ...
        Assert-Contains -Name "strategy485 risk latest pointer reuse" -Text $text -Pattern ([regex]::Escape($marker))
    }
    if ($text -match "child_start|Could not resolve hostname|Connection timed out|Permission denied|remote command failed") {
        throw "strategy485 risk decision unexpectedly invoked SSH or a fresh child run:`n$text"
    }
} finally {
...
Write-Host "[strategy485-risk-reduction-operator-decision-packet-test] OK"
