...
$scriptPath = Join-Path $PSScriptRoot "prepare_strategy574_tiny_live_governance_preflight_review_packet.ps1"
$operatorPath = Join-Path $PSScriptRoot "prepare_strategy574_tiny_live_governance_operator_packet.ps1"
...
foreach ($marker in @(
        "[strategy574-tiny-live-governance-preflight-review-packet] read-only packet",
        "scope=READ_ONLY",
        ...
        "strategy574_tiny_live_governance_preflight_review_packet",
        "strategy574_tiny_live_governance_preflight_status",
        ...
        "notAuthorization=read-only strategy574/TinyLive governance preflight review packet only",
        "RequireReady"
    )) {
    Assert-Contains -Name "strategy574 TinyLive preflight marker" -Text $scriptText -Pattern ([regex]::Escape($marker))
}
...
foreach ($marker in @(
    ...
    Assert-Contains -Name "strategy574 TinyLive operator supports preflight" -Text $operatorText -Pattern ([regex]::Escape($marker))
}
...
foreach ($marker in @(
        ...
        "STRATEGY574_TINY_LIVE_GOVERNANCE_PREFLIGHT_REVIEW_PACKET",
        "strategy574_tiny_live_governance_preflight_review_packet",
        "strategy574/TinyLive governance preflight review packet",
        "READY_FOR_STRATEGY574_TINY_LIVE_GOVERNANCE_PREFLIGHT_REVIEW_NOT_LIVE"
    )) {
    Assert-Contains -Name "docs mention strategy574 TinyLive preflight" -Text $docsText -Pattern ([regex]::Escape($marker))
}

$tempDir = Join-Path ([System.IO.Path]::GetTempPath()) ("strategy574-tiny-live-preflight-" + [guid]::NewGuid().ToString("N"))
...
try {
    ...
    if ($null -eq $powerShell) { throw "Unable to find powershell or pwsh for strategy574 TinyLive preflight test" }
    ...
    if ($exitCode -ne 0) {
        throw "strategy574 TinyLive preflight failed temp-log reuse:`n$text"
    }
    foreach ($marker in @(
            "source_operator_packet_status=READY_FOR_STRATEGY574_TINY_LIVE_GOVERNANCE_OPERATOR_REVIEW_NOT_LIVE",
            "strategy574_tiny_live_preflight_risk_posture=BLOCKED_NEAR_THRESHOLD_FALSE_POSITIVE_RISK_HIGH",
            ...
            '"preflightDecision":"PREPARE_REVIEW_ONLY_STRATEGY574_TINY_LIVE_GOVERNANCE_REVIEW"',
            '"sourceNearThresholdRecommendation":"STRATEGY574_NEAR_THRESHOLD_FALSE_POSITIVE_RISK_HIGH"',
            '"tinyLiveOrderAllowed":false',
            '"livePolicyChangeAllowed":false',
            '"telegramSendAllowed":false',
            "notAuthorization=read-only strategy574/TinyLive governance preflight review packet only"
        )) {
        Assert-Contains -Name "strategy574 TinyLive preflight temp log reuse" -Text $text -Pattern ([regex]::Escape($marker))
    }
    if ($text -match "child_start|Could not resolve hostname|Connection timed out|Permission denied|remote command failed") {
        throw "strategy574 TinyLive preflight unexpectedly invoked SSH or a fresh child run:`n$text"
    }
} finally {
    if (Test-Path -LiteralPath $tempDir) { Remove-Item -LiteralPath $tempDir -Recurse -Force }
}

Write-Host "[strategy574-tiny-live-governance-preflight-review-packet-test] OK"
