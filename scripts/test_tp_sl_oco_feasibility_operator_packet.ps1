...
$scriptPath = Join-Path $PSScriptRoot "prepare_tp_sl_oco_feasibility_operator_packet.ps1"
...
foreach ($marker in @(
        "[tp-sl-oco-feasibility-operator-packet] read-only packet",
        "scope=READ_ONLY",
        ...
        "order_allowed=false",
        "notAuthorization=read-only TP/SL/OCO feasibility operator packet only",
        "RequireReady"
    )) {
    Assert-Contains -Name "TP/SL/OCO feasibility packet marker" -Text $scriptText -Pattern ([regex]::Escape($marker))
}
...
foreach ($marker in @(
        ...
        "TP_SL_OCO_FEASIBILITY_OPERATOR_PACKET",
        "tp_sl_oco_feasibility_operator_packet",
        "TP/SL/OCO feasibility operator packet",
        "READY_FOR_TP_SL_OCO_FEASIBILITY_OPERATOR_REVIEW_NOT_MUTATION"
    )) {
    Assert-Contains -Name "docs mention TP/SL/OCO feasibility packet" -Text $docsText -Pattern ([regex]::Escape($marker))
}

$tempLogPath = Join-Path ([System.IO.Path]::GetTempPath()) ("tp-sl-oco-feasibility-" + [guid]::NewGuid().ToString("N") + ".log")
try {
    $briefPacket = [pscustomobject]@{
        ...
        decisionLanes = @(
            ...
            [pscustomobject]@{
                ...
                status = "READY_FOR_OPERATOR_REVIEW_NOT_MUTATION"
            }
        )
    ...
    if ($null -eq $powerShell) {
        throw "Unable to find powershell or pwsh for TP/SL/OCO feasibility packet test"
    }
    ...
    if ($exitCode -ne 0) {
        throw "TP/SL/OCO feasibility packet failed temp-log reuse:`n$text"
    }
    foreach ($marker in @(
            ...
            '"primaryDecision":"PREPARE_SEPARATE_TP_SL_OCO_FEASIBILITY_REVIEW"',
            ...
            '"negativeEvPositionCount":3',
            "notAuthorization=read-only TP/SL/OCO feasibility operator packet only"
        )) {
        Assert-Contains -Name "TP/SL/OCO feasibility temp log reuse" -Text $text -Pattern ([regex]::Escape($marker))
    }
...
Write-Host "[tp-sl-oco-feasibility-operator-packet-test] OK"
