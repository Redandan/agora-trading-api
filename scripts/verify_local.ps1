...
try {
    ...
    foreach ($pattern in @("TelegramServiceImpl", "ExecutionEventScheduler", "unknown runtime errors fail live readiness log smoke", "ERROR category telegram_service=0 execution_event_scheduler=0 unknown=1", "runtime ERROR lines present: count=2", "high-risk operation-like log lines present", "okx auto-trade enabled config echo is not an operation", "OKX auto-trade enabled startup config echo present", "pyth transient timeout is classified warn baseline", "Pyth network warnings exceeded threshold", "okx ws transient null is classified warn baseline", "OKX WS transient warnings exceeded threshold", "runtime error allow flag is diagnostic only", "unknown warn allow flag is diagnostic only", "high risk allow flag is diagnostic only", "ALLOW_RUNTIME_ERROR", "ALLOW_UNKNOWN_WARN", "ALLOW_HIGH_RISK_LOG", "MAX_PYTH_NETWORK_WARN", "MAX_OKX_WS_TRANSIENT_WARN", "runtime-log-smoke-classification-test")) {
