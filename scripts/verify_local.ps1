...
try {
    ...
    Assert-RgNoMatch -Pattern "pending trading Flyway baseline|V10[0-9] migration|follow-up Flyway migration|Generate and review the Flyway baseline|until a baseline migration is ready|Before replacing Hibernate schema update with Flyway validation|before generating Flyway baseline" -Paths @("src/main/java", "src/test/java", "README.md", "docs", "SPLIT_PROGRESS.md") -Description "schema comments and docs must not imply pre-baseline or legacy V10x migration state"
    ...
    Assert-RgMatch -Pattern "Reviewed shared-DB baseline" -Paths @("src/main/resources/db/migration/V1__baseline.sql") -Description "baseline migration header reflects reviewed shared-DB status"
    Assert-RgMatch -Pattern "Future Trading schema changes should be V2__" -Paths @("src/main/resources/db/migration/V1__baseline.sql", "docs/schema-baseline.md") -Description "baseline migration points future schema changes to V2 migrations"
    Assert-RgNoMatch -Pattern "until this file is reviewed|Generate and review the Flyway baseline" -Paths @("src/main/resources/db/migration/V1__baseline.sql", "docs/schema-baseline.md") -Description "baseline migration/docs must not imply pre-review baseline state"
    Assert-RgNoMatch -Pattern "^[[:space:]]*DROP TABLE|CREATE TABLE ``?(flyway_schema_history|trading_flyway_schema_history)``?" -Paths @("src/main/resources/db/migration/V1__baseline.sql") -Description "reviewed baseline must not drop tables or create Flyway history tables"
