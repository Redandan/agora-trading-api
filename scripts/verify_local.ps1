...
try {
    ...
    foreach ($pattern in @("Assert-LogContains", "Assert-LogNotContains", "AGORA_MARKET_INTERNAL_API_KEY", "TRADING_OKX_API_KEY", "TRADING_BINANCE_API_KEY", "TELEGRAM_BOT_TOKEN", "GEMINI_API_KEY", "GROQ_API_KEY", "ANTHROPIC_API_KEY", "JINA_API_KEY", "EXTERNAL_COINALYZE_API_KEY", "EXCHANGE_RATE_COINMARKETCAP_API_KEY", "META_CONTROL_STARTUP_BACKFILL_COINALYZE_ENABLED", "META_CONTROL_STARTUP_BACKFILL_COMPOSITE_INDICATOR_ENABLED", "META_CONTROL_STARTUP_BACKFILL_DEX_FLOW_ENABLED", "META_CONTROL_STARTUP_BACKFILL_HYPERLIQUID_FUNDING_ENABLED", "MARKET_WS_AUTO_SUBSCRIBE_ENABLED", "OKX_EARN_TOPUP_ENABLED", "POLYMARKET_MONITOR_ENABLED", "TRAILING_STOP_ENABLED", "TRADING_SHORT_SQUEEZE_ALERT_ENABLED", "TRADING_SHORT_SQUEEZE_ALERT_TAKER_BUY_COLLECTOR_ENABLED", "spring-boot.run.arguments", "gemini.api.key=", "groq.api.key=", "trading.okx.api-key=", "telegram.bot.token=", "meta-control.startup-backfill.coinalyze.enabled=false", "meta-control.startup-backfill.composite-indicator.enabled=false", "meta-control.startup-backfill.dex-flow.enabled=false", "meta-control.startup-backfill.hyperliquid-funding.enabled=false", "market.ws.auto-subscribe.enabled=false", "okx.earn-topup.enabled=false", "polymarket.monitor.enabled=false", "trailing-stop.enabled=false", "trading.short-squeeze-alert.enabled=false", "trading.short-squeeze-alert.taker-buy-collector-enabled=false", "Scheduling disabled for local-smoke profile", "Auto-trade enabled", "API Key configured", "Trading disabled.*private WS skipped", "AiTaskRouter.*initialized with 0 providers", "Jina embedding client initialised: enabled=false", "MarketWS.*auto-subscribe config: enabled=false", "EarnTopUp.*config: enabled=false", "PolymarketMonitor.*config: enabled=false", "TrailingStop.*config: enabled=false", "ShortSqueezeAlert.*config: enabled=false takerBuyCollectorEnabled=false", "DexFlowBackfill", "HLFundingBackfill", "CoinalyzeBackfill", "CMIBackfill", "Auto subscribed via", "Warming up MarketSignalCache", "Trading buffer topped from Earn", "Simple Earn", "modifyOco", "ShortSqueezeAlert.*FIRED", "SpotTakerBuy.*15m taker buy", "order placed", "send telegram")) {
