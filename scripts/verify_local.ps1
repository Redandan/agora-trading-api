...
try {
    ...
    foreach ($pattern in @("read-only server audit", "order_capable_flags", "dry_run_flags", "secret_presence", "readiness_details", "autonomousOpportunity", "primaryBlockers", "blockingInterpretation", "terminalBlockers", "runtimeEvidenceStatus", "validateAutonomousOpportunityReadiness", "blocker_classification", "next_actions", "market_condition_wait", "runtime_evidence_gap", "risk_hard_stop", "execution_disabled_guard", "background_automation_review", "runtime_log_status", "verdict=READY_FOR_OPERATOR_REVIEW_NOT_LIVE_ENABLED", "verdict=NOT_READY", "getTinyLiveAutoExecutionTriggerStatus", "getScoreBuyPrePositionAutoExecutionStatus", "getScoreBuyConfirmedDeployAutoExecutionStatus", "getScoreBuyPostScoutAutoAddStatus", "getTrailingStopStatus", "getGuardianSnapshot")) {
    ...
    foreach ($pattern in @("Live Background Automation Env Diff Proposal", "not authorization", "BACKGROUND_AUTOMATION_REVIEW", "HIGH_RISK_BACKGROUND_AUTOMATION_TRUE", "TRADING_MARKET_DATA_MCP_EXTERNAL_HEALTH_PROBES_ENABLED=false", "TRADING_MARKET_DATA_MCP_EXTERNAL_BACKFILLS_ENABLED=false", "MARKET_WS_AUTO_SUBSCRIBE_ENABLED=false", "EVENT_SCAN_NOTIFICATION_ENABLED=false", "EXECUTION_EVENT_ENABLED=false", "TRADING_DAILY_TG_REPORT_ENABLED=false", "TRADING_AUTONOMOUS_DIGEST_ENABLED=false", "TRADING_AUTONOMOUS_DIGEST_TELEGRAM_ENABLED=false", "TRADING_LIVE_SIGNAL_RETRY_NOTIFICATION_ENABLED=false", "background_automation_true=\[\]", "high_risk_background_automation_true=\[\]", "OK_BACKGROUND_AUTOMATION_DISABLED", "order_capable_flags", "Rollback Criteria", "not live approval")) {
