...
try {
    ...
    foreach ($pattern in @("tiny-live-loss-rca", "hardStopDetected", "AUTO_APPROVAL_DISABLED_CONSECUTIVE_TINY_LIVE_LOSSES", "listTinyLiveExecutionReadiness", "previewTinyLiveAutoApproval", "previewTinyLiveAutoExecution", "listTinyLiveExecutions", "getAutonomousExecutionAttribution", "getAutonomousExplorationMonitorStatus", "getExplorationRolloutStatus", "getMissedOpportunityRegressionReport", "getNoBuyReasonTruthTable", "KEEP_DISABLED", "read-only check complete")) {
    ...
    foreach ($script in @("scripts/smoke_mcp_parity_ssh.ps1", "scripts/smoke_guardrail_acceptance_ssh.ps1", "scripts/smoke_trailing_stop_pnl_replay_ssh.ps1", "scripts/smoke_signal_correctness_ssh.ps1", "scripts/audit_live_readiness_ssh.ps1", "scripts/smoke_tiny_live_loss_rca_ssh.ps1")) {
