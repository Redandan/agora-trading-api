...
try {
    ...
    foreach ($pattern in @("SSH_AUTH_FAILED", "SSH_CONNECT_FAILED", "SSH_COMMAND_FAILED", "complete evidence", "not live-readiness evidence")) {
    ...
    foreach ($pattern in @("Live Background Automation Env Diff Proposal", "not authorization", "BACKGROUND_AUTOMATION_REVIEW", "HIGH_RISK_BACKGROUND_AUTOMATION_TRUE", "TRADING_MARKET_DATA_MCP_EXTERNAL_HEALTH_PROBES_ENABLED=false", "TRADING_MARKET_DATA_MCP_EXTERNAL_BACKFILLS_ENABLED=false", "MARKET_WS_AUTO_SUBSCRIBE_ENABLED=false", "EVENT_SCAN_NOTIFICATION_ENABLED=false", "EXECUTION_EVENT_ENABLED=false", "TRADING_DAILY_TG_REPORT_ENABLED=false", "TRADING_AUTONOMOUS_DIGEST_ENABLED=false", "TRADING_AUTONOMOUS_DIGEST_TELEGRAM_ENABLED=false", "TRADING_LIVE_SIGNAL_RETRY_NOTIFICATION_ENABLED=false", "background_automation_true=\[\]", "high_risk_background_automation_true=\[\]", "background_automation_false", "lists all nine reviewed background flags", "OK_BACKGROUND_AUTOMATION_DISABLED", "order_capable_flags", "Rollback Criteria", "not live approval")) {
