...
try {
    ...
    Assert-RgMatch -Pattern "no production deploy has been run for those commits" -Paths @("SPLIT_PROGRESS.md") -Description "split progress documents production currentness gap after local cleanup"
    ...
    Assert-RgMatch -Pattern "META_CONTROL_BTC_PRICE_MOVE_INDICATOR_ENABLED" -Paths @(".env.trading.secrets.example", "scripts/validate_env_template.ps1", "docs/deploy-runbook.md") -Description "BTC price-move indicator opt-in key is documented and validated"
    Assert-RgMatch -Pattern "META_CONTROL_ETF_PRESSURE_REFRESH_ENABLED" -Paths @(".env.trading.secrets.example", "scripts/validate_env_template.ps1", "docs/deploy-runbook.md") -Description "ETF pressure refresh opt-in key is documented and validated"
    ...
    Assert-RgMatch -Pattern "META_CONTROL_HOURLY_ORCHESTRATOR_ENABLED" -Paths @(".env.trading.secrets.example", "scripts/validate_env_template.ps1", "scripts/smoke_local_health.ps1", "docs/deploy-runbook.md") -Description "hourly orchestrator opt-in key is documented, validated, and cleared in local smoke"
    Assert-RgMatch -Pattern "META_CONTROL_COMPOSITE_INDICATOR_SCHEDULER_ENABLED" -Paths @(".env.trading.secrets.example", "scripts/validate_env_template.ps1", "scripts/smoke_local_health.ps1", "docs/deploy-runbook.md") -Description "composite indicator scheduler opt-in key is documented, validated, and cleared in local smoke"
    ...
    Assert-RgMatch -Pattern "META_CONTROL_MARKET_INDICATOR_ATTENTION_ENABLED" -Paths @(".env.trading.secrets.example", "scripts/validate_env_template.ps1", "scripts/smoke_local_health.ps1", "docs/deploy-runbook.md") -Description "market-indicator attention opt-in key is documented, validated, and cleared in local smoke"
    ...
    Assert-RgMatch -Pattern "if \(!props\.enabled\(\)\) return" -Paths @("src/main/java/com/agora/service/trading/GridManagerService.java") -Description "grid manager scheduled check has method-level opt-in guard"
    ...
    Assert-RgMatch -Pattern 'agora\.alpha-tracker\.enabled:false' -Paths @("src/main/java/com/agora/service/diagnostic/AlphaPromotionTracker.java") -Description "alpha promotion scheduled tracker defaults off"
