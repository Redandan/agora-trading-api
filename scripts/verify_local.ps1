...
function Assert-McpParityToolCoverage {
    ...
    foreach ($marker in @("Invoke-McpTool", "getEventRiskControlStatus", "analyzeSpotAntiWickPolicyCoverage", "analyzeTrailingStopPnlReplay", "getEntryDedupGovernanceDashboard", "getMissedOpportunityRegressionReport", "analyzeStrategy508HoldCounterfactual", "orderSent", "ocoModified", "writesRuntimeEvidence", "operatorControls=CONFIG_ONLY_NO_RUNTIME_MUTATION", "ULTRA_LOW_DISASTER", "acceptanceTarget: total trailing PnL improvement >= 5%", "ambiguousSameBar rows are excluded", "acceptanceBlocker=", "acceptanceBlockerDetail=")) {
