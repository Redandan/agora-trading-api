...
try {
    ...
    foreach ($pattern in @("read-only server audit", "order_capable_flags", "dry_run_flags", "secret_presence", "readiness_details", "primaryBlockers", "blockingInterpretation", "terminalBlockers", "runtimeEvidenceStatus", "runtime_log_status", "verdict=READY_FOR_OPERATOR_REVIEW_NOT_LIVE_ENABLED", "verdict=NOT_READY", "getTinyLiveAutoExecutionTriggerStatus", "getScoreBuyPrePositionAutoExecutionStatus", "getScoreBuyConfirmedDeployAutoExecutionStatus", "getScoreBuyPostScoutAutoAddStatus", "getTrailingStopStatus", "getGuardianSnapshot")) {
