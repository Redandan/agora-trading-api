...
try {
    ...
    Assert-RgMatch -Pattern "verify_nginx_mcp_block_bodies" -Paths @("scripts/verify_server.sh") -Description "server verification checks nginx public MCP block bodies"
    Assert-RgMatch -Pattern "must not proxy_pass" -Paths @("scripts/verify_server.sh") -Description "server verification rejects public MCP proxy_pass in exact blocks"
    ...
    foreach ($pattern in @("verifyStrategyExecution", "analyzeBlockedSignalOutcomes", "getSignalCorrectnessDashboard", "getEntryDedupGovernanceDashboard", "getMissedOpportunityRegressionReport", "getGovernanceDriftDashboard", "findGovernanceRelaxationCandidates", "findGovernanceTighteningCandidates", "read-only production MCP check", "verifyStrategyExecution machine status marker", "MACHINE_STATUS", "executionMachineStatus", "missing_signal_policy_fields", "signal_policy_review_plan", "notAuthorization", "requiredEvidence", "missing no-missed-evaluation/no-missed-order marker", "sys.exit\(1\)", "OK read-only check complete")) {
    ...
    foreach ($pattern in @("live-background-automation", "READ_ONLY", "background_automation_true", "high_risk_background_automation_true", "missing_background_automation_flags", "background_automation_review_plan", "riskCategory", "requiredReview", "background_automation_blockers", "backgroundAutomationClear", "MISSING_BACKGROUND_AUTOMATION_FLAG", "BACKGROUND_AUTOMATION_REVIEW_BEFORE_LIVE", "NOT_READY_BACKGROUND_AUTOMATION_REVIEW", "OK_BACKGROUND_AUTOMATION_DISABLED", "TRADING_MARKET_DATA_MCP_EXTERNAL_BACKFILLS_ENABLED", "TRADING_AUTONOMOUS_DIGEST_TELEGRAM_ENABLED", "TRADING_LIVE_SIGNAL_RETRY_NOTIFICATION_ENABLED", "Assert-RemotePathSafe", "Assert-SshHostSafe", "read-only check complete")) {
