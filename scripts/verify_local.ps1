...
try {
    ...
    foreach ($pattern in @("Assert-LogContains", "Assert-LogNotContains", "AGORA_MARKET_INTERNAL_API_KEY", "TRADING_OKX_API_KEY", "TRADING_BINANCE_API_KEY", "TELEGRAM_BOT_TOKEN", "GEMINI_API_KEY", "GROQ_API_KEY", "ANTHROPIC_API_KEY", "JINA_API_KEY", "EXTERNAL_COINALYZE_API_KEY", "EXCHANGE_RATE_COINMARKETCAP_API_KEY", "MARKET_WS_AUTO_SUBSCRIBE_ENABLED", "OKX_EARN_TOPUP_ENABLED", "POLYMARKET_MONITOR_ENABLED", "spring-boot.run.arguments", "gemini.api.key=", "groq.api.key=", "trading.okx.api-key=", "telegram.bot.token=", "market.ws.auto-subscribe.enabled=false", "okx.earn-topup.enabled=false", "polymarket.monitor.enabled=false", "Scheduling disabled for local-smoke profile", "Auto-trade enabled", "API Key configured", "Trading disabled.*private WS skipped", "AiTaskRouter.*initialized with 0 providers", "Jina embedding client initialised: enabled=false", "MarketWS.*auto-subscribe config: enabled=false", "EarnTopUp.*config: enabled=false", "PolymarketMonitor.*config: enabled=false", "Auto subscribed via", "Warming up MarketSignalCache", "Trading buffer topped from Earn", "Simple Earn", "order placed", "send telegram")) {
