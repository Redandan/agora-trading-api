...
try {
    ...
    Assert-RgMatch -Pattern "require_env_value SPRING_JPA_HIBERNATE_DDL_AUTO update" -Paths @("scripts/preflight_server.sh", "scripts/verify_server.sh") -Description "server verification keeps ddl-auto update until Flyway baseline exists"
    Assert-RgMatch -Pattern "require_env_value SPRING_FLYWAY_ENABLED false" -Paths @("scripts/preflight_server.sh", "scripts/verify_server.sh") -Description "server verification keeps Flyway disabled until baseline exists"
    ...
    Assert-RgMatch -Pattern "Server preflight and verification fail if" -Paths @("docs/split-audit.md", "docs/deploy-runbook.md") -Description "schema bootstrap env guard is documented"
