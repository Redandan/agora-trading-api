...
$remote = "sed '1s/^\xEF\xBB\xBF//' | tr -d '\r' | EXPECTED_CONTROL_RELEASE_ID='$ExpectedControlReleaseId' EXPECTED_DATA_RELEASE_ID='$ExpectedDataReleaseId' EXPECT_MICROSTRUCTURE_SOURCE='$ExpectMicrostructureSource' MICROSTRUCTURE_INTAKE_PREFLIGHT='$intakePreflightFlag' EXPECT_TIMER='$ExpectTimer' RUN_HEARTBEAT='$runFlag' RUN_SOURCE_PROBE='$probeFlag' bash -s"
